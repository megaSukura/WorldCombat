 ...
 * 碎岩 / rocksmash 的客户端表现。
 *
 * 一句话：拳上聚起薄土，随后拳头沿施法者到目标的一条短线一记记打出去，命中处砸出拳印与碎石；砸开缺口时目标身上
 * 崩开一圈石屑。
 * 色相家族：土黄与岩灰（large_rock／impact_rock 原色、tinydust 中性）＋一处近白高光（glowingsparkle）。
 * 拍子：起（windup 聚土）→ 击（strike 每记拳）→ 收（crack 缺口崩开）。
 * 范围：strike 用 path 画出服务端 trace 的同一段线，拳够到哪、画面就是那段；命中爆开在同一个落点上。
 * 运动：拳沿 path 由近及远打出，碎石从落点朝外飞、受重力落地，土屑慢速下沉。
