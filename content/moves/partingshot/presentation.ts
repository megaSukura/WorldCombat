 ...
 *
 * 一句话：施法者嘴边聚起一缕暗紫话音 → 一句带刺的狠话（暗紫惊叹镖）沿直线飞向对手 → 扎中后对手身上炸开暗紫的羞辱色、
 *   一句话的重量压下去，施法者向反方向拖出退步线走开。
 * 色相家族：暗紫 0x6A3FA0 画主线，近白紫 0xD9C2F0 只给「扎中」那一下，暗酒红 0x3A2140 作余韵。
 * 起击收：起 barb 10t ／ 飞 launch 沿机制轨迹 ／ 击 drain 26t ／ 退 step 20t ／ 空 whiff 18t。
 * 范围：本招是一条「施法者→对手」的线；投射物本体（`sprite` 惊叹镖）就是那句话说走的路，玩家一眼看出打向谁。
 * 运动：镖沿机制弹道直线飞行；drain 的暗色从对手身上向内压、余韵向下沉；step 的速度线沿 `data.direction` 拖出。
 * 数：launch／drain 的粒子数量直接读本招算出的 `motes`（特攻派生），drain 的暗色爆发读 `drop`（削掉的等级数），
 *   强度读 `intensity`（削得越多越重）。
 *
...
const PartingShotDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        step: {
            ...
            emitters: [
                {
                    name: "back", bind: "source", offset: [0, 0.35, 0], height: 0.5, fit: "body",
