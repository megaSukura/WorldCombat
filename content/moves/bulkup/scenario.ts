 ...
 *   所以 AI 只能先涨一轮。有威胁且在练身距离内时，它会先绷紧再考虑交战。
 * 必然事实：本招被提交过；施术者身上出现过共享身份 world_combat:status/bulkup 的涨身窗口。
 *   物攻与防御各抬了几级、涨身撑多久、窗口结束时收回多少写进 note 供读轨迹判断（私有装配没有读取原生能力等级的读取原语）。
 */
Smoke.scenario("bulkup", function (stage) {
    ...
    stage.until(1200, function () {
        ...
        stage.after(80, function () {
            stage.note("the lean choice sets Attack/Defense stages (+2/+1 or +1/+2); both are native for a Pokemon and unreadable here, and the window takes them back by its stored mark when it ends.", {
