 ...
 *
 * 一句话：甩出一发廉价的远程细针，真的扎出伤害后，短命托管效果才在伤口里慢慢渗毒。
 *
 * 场面：一只只会毒针的线球（spinarak，L30，原生学习者）对一只只会跃起、站桩的卡比兽（snorlax，L30），
 *   相隔 6 格——在射程内，AI 可以直接点射；地面铺平、白天晴天。
 *
 * 必然事实：本招被提交过（`stage.casts`）；目标受到过针尖伤害（`stage.damageTo`）。
 * 随机量写进 note：中毒概率（原生 30%%，受特攻与倒钩针影响）、暴击、以及托管效果到 `seep` 后的渗毒结果；
 *   飞针动作不等渗完就释放（托管效果自己走完），留待人工观察。
 */
