 ...
 *
 * 一句话：光在施法者身前收成一枚宝石焦点、碎晶朝它聚拢 → 一条又细又长的宝石光线沿瞄准方向短闪射出，
 *   线两侧闪着彩虹碎光 → 光贯穿到谁，谁身上只崩出一枚碎晶；射程尽头留下一圈余光。
 * 色相家族：宝石的青白（0xBFE8FF / 0xEDF7FF）为主体，彩虹只做小面积的折射碎光，灰蓝（0x9FB6C8）收在碎屑。
 * 拍子：起 gather（聚晶）→ 射 beam（短闪贯穿）→ 击 hit（一枚碎晶）→ 收 fade（尽头余光）。
 * 范围：beam 用与判定同一起止的 `data.path` 画这条线；玩家一眼看出站在哪条走廊上会被贯穿。
 * 运动：光线沿 `data.direction` 笔直射出，附着的碎光沿同方向掠过，尽头停在被方块挡住的位置。
 * 数：束宽直接绑定 `data.width`（粗细，碰撞箱换算），线上的碎光量绑定 `data.shards`（特攻与等级换算，只落在线上、不落到击点），亮度绑定 `data.intensity`（威力换算）。
 */
const PowerGemDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        hit: {
            ...
            emitters: [
                {
                    name: "hit_flash", bind: "target", offset: [0, 0.1, 0], height: 0.5,
                    particle: "world_combat_core:cobblemon/generic/impact/impact_rock_white",
                    burst: { count: 2, at: 0 },
                    shape: { kind: "sphere", radius: 0.2 },
                    direction: "outward", speed: [0.0, 0.04],
                    lifetime: 6, size: [0.24, 0.04], sizeMode: "index",
                    color: 0xEDF7FF, alpha: [0.9, 0], light: "full", bloom: 0.3, maxParticles: 6
                },
                {
                    name: "hit_shard", bind: "target", offset: [0, 0.1, 0], height: 0.5,
                    particle: "world_combat_core:cobblemon/generic/spike",
                    burst: { count: 1, at: 0 },
                    shape: { kind: "sphere_surface", radius: 0.18 },
                    direction: "outward", speed: [0.1, 0.24], spread: 20,
                    gravity: 0.07, drag: 0.9,
                    lifetime: [12, 20], size: [0.16, 0.03],
                    color: 0x9FB6C8, alpha: [0.85, 0], light: "world", maxParticles: 4
                },
                {
                    name: "hit_glint", bind: "target", offset: [0, 0.1, 0], height: 0.5,
                    particle: "world_combat_core:cobblemon/generic/sparkle/shinesparkle_rainbow",
                    burst: { count: 1, at: 0 },
                    shape: { kind: "sphere_surface", radius: 0.2 },
                    direction: "outward", speed: [0.05, 0.14],
                    lifetime: [8, 14], size: [0.11, 0.02],
                    alpha: [0.95, 0], light: "full", bloom: 0.3, maxParticles: 4
                }
            ]
        },
        fade: {
