 ...
 * 数：`data.flames`（速度派生）决定火星与命中的密度，`data.intensity`（本次伤害派生）决定命中强度，
 * `data.order`（第几个被碾到）决定命中是否递减变淡，`data.thawed`（1 表示化掉了自身的冰）决定是否多一圈白热光环。
 */
        ...
        roll: {
            ...
            emitters: [
                ...
                },
                {
                    ...
                    color: 0xF08030, alpha: [0.8, 0], light: "full", bloom: 0.3, maxParticles: 130
                },
                {
