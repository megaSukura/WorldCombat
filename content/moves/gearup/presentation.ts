 ...
 *
 * 一句话：施法者体内的齿轮「咔」地啮合、越转越快，转速到顶时几道齿链把动力甩向身边的伙伴，
 *   伙伴身上迸出钢屑、物攻与特攻一起抬起来。
 *
 * 色相家族：钢白（0xC8CDD3）画齿轮与齿屑，近白（0xF2F4F6）做高光，暖铜（0xE0A060）只作为齿轮中心那一撮
 *   小面积强调——一个冷色家族加一点暖光，没有第二个色相。
 * 层次：起（空转的齿环）／啮（合拢的钢爆＋齿链）／传（伙伴身上的动力迸发）／收（锁定散去）。
 * 起击收：spin（起）→ mesh（击）→ drive（落在谁身上）→ fade（收）。
 * 范围：mesh 的齿环绑 `data.scale`（实际齿链半径 / 2.6），画面里的环就是动力能传到的范围。
 * 运动：齿屑绕身高速旋转；啮合瞬间沿 `data.path` 的齿链直线甩向伙伴；伙伴身上钢屑向外迸开再落回。
