 ...
 * 范围：surge 用 `data.path`（与服务端同一个扇环多边形）画成面，玩家看到泥浪铺到哪就知道站哪会被糊到。
 * 运动：泥浪沿地面一步步向前推，`data.front`／`data.inner` 决定当前这一道的位置，泥点带轻微重力往下沉。
 * 数：`data.drops`（特攻＋等级换算的泥点数量）绑定发射量，`data.intensity`（泥浪威力 / 90）放大整幕，
 *   `data.murkStages` 让糊眼那一下更密，`data.scale` 让大个子的泥浪更粗。
 */
