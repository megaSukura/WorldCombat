...
namespace PokemonSkills {
    ...
    function muddywaterWants(context: WorldBehavior.Context, capability: WorldBehavior.Capability, target: CompanionBehavior.Entity): boolean {
        ...
        if (!muddywaterReachHeight(CompanionBehavior.source(context), target)) return false;
    ...
    CompanionBehavior.registerUse(muddywaterId, {
        ...
        accepts: function (context, capability, target) {
            return !target.friendly && target.health > 0 && target.visible
                && muddywaterReachHeight(CompanionBehavior.source(context), target);
        },
    ...
    addPreferences(muddywaterId, {}, [
        field(pathOf("silted"), "淤积式", "boolean", {
            help: "开启：扇面 ×1.35、糊眼概率 +12%、可掉 2 级命中、糊眼与地面泥膜都更久，适合把一片区域封住；代价是威力 ×0.85、射程 −1.5 格、推进更慢、起手 +3 刻、冷却 +6 刻。关闭（急流）：威力 ×1.12、射程 +1.5 格、推进更快，代价是糊眼概率 −6%、糊眼与泥膜都更短（泥膜 ×0.75），打完就走。"
        }),
