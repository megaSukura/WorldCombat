 ...
 * 范围：`kick` 绑命中点、以 `impact_electric` 的炸开尺度画出脚掌判定（`data.intensity` 随威力放大），
 *   `blink` 的残影沿 `data.direction`（这一步实际位移方向）拉出电光条，玩家看得见绕步绕向哪一侧。
 * 运动：电光从脚边窜起、残影沿位移方向拖尾、踢实整片电火外爆、护架缺口从接触点向上崩开。
 * 数：`data.feints`（实际绕步次数）绑定残影层的重复量，`data.stages`（实际踢开级数）绑定缺口火花量，
        ...
        blink: {
            ...
            emitters: [
                ...
                },
                {
                    ...
                    burst: { count: { data: "feints", fallback: 2 }, at: 0 },
