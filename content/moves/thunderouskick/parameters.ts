 ...
 * 电光般的绕步（`feints`）在目标身侧来回换位，最后从它没在看的那一侧踢出一脚。脚踢把目标的护架踢开：
 * `NativeEffects.boost(...,"def",-N)` + 共享身份 `world_combat:status/guardbroken`（与破防一族同一身份）。
 * 「戏耍」体现在两处：绕步次数越多、目标越是正忙着打别人（`attacking` 不是自己），护架被踢得越开——
 * 这是本招可被读出、也可被反制的部分：背对或分神时挨得更狠。
 *
 * 数据分散：
 *   kick         踢击威力：物攻定脚劲，速度定出脚多快多狠；配置两向分配。
 *   feints       绕步次数：速度定能在目标身边闪几次，配置再补一次。
 *   blinkRange   绕步距离：身高定每一步跨多远。
 *   standoff     站位距离：目标体宽定侧身站位要多远才踢得到。
 *   kickSpeed    出脚速度：速度定最后一脚冲得多快。
 *   guardStages  踢开等级：配置决定踢开一级还是两级。
 *   feintBonus   戏耍加成：绕步达到三次以上时额外多踢开一级。
