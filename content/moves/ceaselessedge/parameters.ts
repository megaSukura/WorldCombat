 ...
 *   `world_combat:hazard/shellshards` 由本单元注册）：踏进来/跨过边界的贴地敌人按当前锋利度吃一记重的 `shard`，
 *   留在圈里按 `treadInterval` 间隔再吃一记轻的（`standShare`），飞行对象不会被割。每次斩击——斩中实体或刀锋落在
 *   没被墙挡住的空地上——都在落点重新铺一次，并把同片地上的旧碎片并入、锋利度 +1（最多 `sharpMax` 层），所以持续
 *   在同一片地上斩会越割越狠。自由瞄准：目标可为 null，按方向斩，被墙挡住就不在墙后摆碎片。
 *
