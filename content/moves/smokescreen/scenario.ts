// 烟幕的可执行设计说明：一只宝可梦对挤在一起的两只僵尸只说这一招，把烟云铺在它们脚下。
// 必然事实：烟幕被放出来过；云里的目标带上共享的「被烟呛」身份；它的攻击属性随之走低。
// 云同时罩住几人、套多久、命中瞬间的随机结果都不是必然事实，写进 note 供读轨迹判断。
Smoke.scenario("smokescreen", function (stage) {
    ...
    stage.until(800, function () {
        ...
        stage.note("smokescreen lays a lingering field at a point; the cloud keeps impairing whoever stays inside and lingers after leaving", {
