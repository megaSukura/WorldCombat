 ...
 *   身边的敌人会不断把攻击目标锁到施法者身上（world.target），把原本打向同伴的火力牵过来。它不限属性、
 *   不限目标类型（不像愤怒粉怕草属性），是纯粹的一嗓子。优先度 +2 落成很短的起手。
 *
 * 数值来源（每个参数读不同的精灵数据，分散到不同参数上）：
 *   callRadius  喊话半径：等级与特攻决定能喊到多远；喊话式更宽、招手式更窄。
 *   callTicks   喊住时长：等级与特防决定注意被拉住多久；喊话式更短、招手式更久。
 *   interval    牵引节奏：速度决定每隔多久重新把敌人拉回来一次。
