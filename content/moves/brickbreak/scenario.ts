 ...
 *
 * 场面：一只只会劈瓦的腕力（34 级）对一只只会撞击的卡比兽（40 级）开战。选厚血对手是为了让它挨过几记
 * 劈瓦仍活着；AI 只会用这一招，所以必然会被放出来。
 * 必然事实：本招被提交过、目标受到过伤害（手刀劈实）。
 * 碎壁依赖的反射壁／光墙／极光幕由别的招式单元生产，而单元自测只装配共享包与本单元——它们不在场，
 * 因此「屏障被劈碎」不进断言，写进 note 并在完整装配的集成试玩里核对。
 */
Smoke.scenario("brickbreak", function (stage) {
    stage.fill([-8, -1, -6], [8, -1, 6], "minecraft:stone");
    stage.time("day");
    stage.weather("clear");
    const caster = stage.pokemon({ species: "machop", level: 34, moves: ["brickbreak"], at: [-2, 0, 0] });
    const foe = stage.pokemon({ species: "snorlax", level: 40, moves: ["tackle"], at: [2, 0, 0] });
