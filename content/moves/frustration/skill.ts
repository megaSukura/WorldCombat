 ...
 *
 * 念头的形状：把憋着的不满在身前攥成一簇暗色刺团（windup，提交前只播预告）→ 朝对手扑近一小段（rush）→
 * 贴上去后连续抓出几爪，每爪独立结算一次接触伤害并把对手抓退一点（rake）→ 收势时吐出一口余恨（spite / miss）。
 * 亲密度越低，抓击越重、次数越多、间隔越短；它和报恩读同一个数、方向相反，靠「多段快抓」与「单次重击」分开。
 *
 * 两幕半：rush（扑近）→ rake × rakes（连抓）→ spite / miss（收势）。提交后才触碰世界。
 */
            ...
            function strike(current: CombatAction, left: number): void {
                ...
                if (hit.hitEntity()) {
                    ...
                    if (target !== null && !scope.friendly(target)) {
                        ...
                        if (landed) {
                            anyLanded = true;
                            if (scope.valid(target)) scope.displace(target, heading.scale(push));
                        }
                    }
                }
                WorldFeedback.emit(scope, frustrationScene, 1, origin.plus(heading.scale(0.65)),
                    { moment: landed ? "rake" : "swipe", index: rakes - left + 1, rakes: rakes, sparks: sparks, intensity: intensity, scale: scale }, 18);
                if (landed) sound(current, "cobblemon:impact.dark");
                if (left <= 1) { finish(current); return; }
