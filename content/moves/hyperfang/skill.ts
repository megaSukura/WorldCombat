...
namespace PokemonSkills {
    ...
    define({
        ...
        execute: function (action, move, config, done) {
            ...
            WorldFeedback.emit(world, hyperfangScene, 1, action.origin(),
                { moment: "pounce", direction: [direction.x(), direction.y(), direction.z()], scale: scale }, 26);
            ...
            function advance(current: CombatAction): void {
                ...
                const hit = current.trace(origin, origin.plus(delta.scale(p("hyperfang", "traceAhead", current))), radius);
                if (hit.hitEntity()) {
