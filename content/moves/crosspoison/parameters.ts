 ...
 *
 * 翻译：把「用毒刃劈开」落成**两片毒刃从左右同时合拢的一剪**——两刃在目标身上交叉剪出一个 X，
 *   两刃之间那条窄线上的人也被顺带划到；切口里的毒不会立刻走，过 `seepDelay` 刻再渗一次，
 *   按更高的 `seepChance` 把毒真正按进伤口。它是本族唯一**同时合拢、并留下一道会渗的毒痕**的斩击。
 *
 * 与同族分开：十字劈是两道劈击从斜上方**先后**落下、第一劈为第二劈撞开架势；十字毒刃是两刃从左右**同时**合拢的
 *   一剪，凭留在切口里的毒取胜。十字剪是合拢扫过两片半扇面，毒刃只剪一条窄线。
 *
 * 数据分散（每项读不同的精灵数据）：
 *   slit         剪击威力：物攻定刃劲，速度定合拢的快；腐蚀式把每一剪摊薄。
 *   reach        出手距离：速度与身高决定两刃能探多远；也是本招射程基准。
 *   spread       剪线半宽：碰撞箱宽度决定两刃之间那条线有多宽。
 *   share        副目标折扣：等级决定顺手划到旁人时留几分力。
 *   poisonChance 初毒概率：特攻（毒液）与物攻（按进伤口）派生。
 *   seepChance   渗毒概率：特攻与等级派生，比初毒高——切口里的毒会自己走。
 *   venomTicks   中毒时长：特攻与等级派生。
 *   seepDelay    渗毒延迟：速度决定两刃合拢到毒渗开之间的间隔。
 *   drops        毒滴数：物攻派生，表现按它发射。
 *   tempo/settle/recharge：速度与等级决定起手、收招与冷却。
 * 配置 corrode（腐蚀式）双向取舍：开启＝初毒 ×1.2、渗毒 ×1.3、中毒更久，但剪击 ×0.9——以毒取胜；
...
namespace PokemonSkills {
    actionParameters.define("crosspoison", {
        ...
        poisonChance: percent(
            F.base(0.10).plus(F.stat("specialAttack").minus(60).times(0.0016).clamp(-0.04, 0.2))
                ...
                .clamp(0.06, 0.5),
            "初毒概率", "两刃合拢当下就把毒按进伤口的概率；特攻越高、按得越准，腐蚀式更高。"),
        /** 渗毒概率：基础 0.22，特攻每比 60 多 1 加 0.002（夹 -0.06..0.24），等级每比 30 高 1 加 0.003（夹 0..0.15）；腐蚀 ×1.3；夹 0.1..0.7。 */
        seepChance: percent(
            F.base(0.22).plus(F.stat("specialAttack").minus(60).times(0.002).clamp(-0.06, 0.24))
                ...
                .clamp(0.1, 0.7),
            "渗毒概率", "切口里的毒在 `seepDelay` 刻后再渗一次、真正令目标中毒的概率；比初毒高，腐蚀式更高。"),
        ...
        venomTicks: seconds(
            ...
            "中毒时长", "毒渗进伤口后持续多久；特攻越高、等级越高挂得越久，腐蚀式更久。"),
        /** 渗毒延迟：基础 8 刻，速度每比 60 快 1 减 0.03（夹 -2..2）；夹 4..13。 */
        seepDelay: seconds(
            F.base(8).minus(F.stat("speed").minus(60).times(0.03).clamp(-2, 2)).clamp(4, 13).round(0),
            "渗毒延迟", "两刃合拢到毒渗开之间隔几刻；速度越快切口收得越快，毒也渗得越急。"),
    ...
    stages("crosspoison", [
        ...
        { level: 48, values: { slit: 88, seepChance: 0.32 } }
    ]);
    ...
    describe("crosspoison", [
        ...
        { key: "growth.1", values: ["tier.1.level", "tier.1.slit", "tier.1.seepChance"] }
    ]);
}
