 ...
 * 拍子：起（windup 聚风）→ 扫（burst 风圈铺到全半径）→ 净（clear 收势白环）→ 剥（strip 被扫中者身上）。
 * 范围：burst／clear 的圆环半径＝`data.scale`×定义半径（scale＝实际清扫半径 ÷ 6），画出的那圈就是判定圈。
 * 运动：风流沿圆环向外推开、贴地翻卷；被扫中者身上的碎屑向外一散即收。
 * 数：风丝数量由 `data.motes`（体宽与速度派生）驱动；`data.caught`／`data.clipped`／`data.fields` 只体现在收势一拍。
            ...
            emitters: [
                {
                    ...
                    burst: { count: 12 }, shape: { kind: "sphere_surface", radius: 0.32 },
                    direction: "outward", speed: [0.05, 0.2], spread: 30,
                    lifetime: [8, 16], size: [0.1, 0.02],
                    color: 0xF5FCFE, alpha: [0.85, 0], light: "full", bloom: 0.3, maxParticles: 34
                },
                {
