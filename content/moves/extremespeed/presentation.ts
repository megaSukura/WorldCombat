 ...
 *   贯穿式还会从对方身上再拉出一段残影停在它身后；冲空时只在前方地上留下一撮被带起的尘与散去的速度线。
 * 色相家族：冷白偏蓝（0xEAF2FF / 0xBFC8FF），亮心近白，尘用中性暖灰；没有第二组色相。
 * 拍子：起 charge（蹬地拉影）→ 冲 leap（整条残影直线）→ 击 ram（巨大白环）→ 穿 through（身后残影）／收 miss。
 * 范围：leap 的残影沿施法者实际走过的轨迹铺开，就是判定扫过的那条长线；ram 绑命中点画在接触处。
 * 运动：起手是贴地向外蹬开的环，冲刺是把残影与速度线甩在身后，命中是向外扩张的巨大白环，
 *   贯穿是身体带着残影从目标身上穿过去继续拉线。
 * 数：leap 的残影道数绑定 `data.wake`（速度换算），ram 的冲击量绑定 `data.count`（撞击威力换算），
 *   亮度绑定 `data.intensity`（撞击威力 / 85），贯穿的残影密度同样绑定 `data.wake`。
 * 参照节：视觉语言第二、三、四、七、九节。
 */
            ...
            emitters: [
                {
                    ...
                    direction: "away", speed: [0.06, 0.24],
                    lifetime: [6, 12], size: [0.28, 0.05], sizeMode: "index",
                    color: 0xBFC8FF, alpha: [0.7, 0], light: "full", maxParticles: 120
                },
                {
                    ...
                    color: 0xEAF2FF, alpha: [0.6, 0], light: "full", maxParticles: 80
                }
            ]
        },
        miss: {
