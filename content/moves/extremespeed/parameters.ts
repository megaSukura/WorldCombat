 ...
 *   一记撞击，并把它狠狠顶开。默认贯穿式（overrun）：撞上后不停，从对方身上穿过去，再真实扫掠一小段停在它身后——
 *   余势会被后排身体或墙截住，撞过的那个可继续穿过但不再补伤害；它同时是一招换位手段，代价是这一下略轻、
 *   收招更久、冷却极长（原生 5 PP）。伤害被拒时只在真实接触点显示拒绝，不推动目标。
 *   与最像的电光一闪分开：距离约两倍、速度更快、命中后贯穿并停到身后、冲击重得多、冷却高得多；
 *   电光一闪是点到为止的短促先手。
 *   自由瞄准 `kind: "aim"`：沿提交时锁定的方向冲，方向或世界点都行；目标横移或离场都不改变已锁定方向。
 *
...
namespace PokemonSkills {
    ...
    actionParameters.define(extremespeedId, {
        ...
        carry: formula(
            ...
            "穿过距离", {
                unit: "格",
                description: "贯穿式在撞中之后还能从对方身上再冲多远（停在它身后）；余势会被后排身体或墙截住，且不再补伤害。重撞式为 0，撞中即停。"
            }),
