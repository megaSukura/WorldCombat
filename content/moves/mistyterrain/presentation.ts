 ...
 *
 * 一句话：施法者脚边腾起雾絮、把雾压到地面 → 落点炸开一圈雾环、薄雾贴着地皮漫满一块地（surge）→
 * 站上去的活体被雾裹住：异常落不下来（ward，持续拒绝时每次亮）、龙招打进来被雾吃掉一半（veil）→
 * 净化雾在活体首次进入本次雾时把已有的异常洗掉一次（cleanse，退出再入不再触发）→ 雾散。
 ...
 * ward   挡下 giantring_white 环面向外 0.5-0.9 8-16 0.85→0 ≤30   （持续防异常，每次拒绝时亮）
 * veil   削龙 giantring_white 环面向外 0.4-0.8 8-16 0.7→0 ≤30
 * cleanse 洗异常 smallsparkle 球面上浮 0.08-0.02 12-22 0.9→0 ≤30 （首次进入本次雾清除一次）
 */
