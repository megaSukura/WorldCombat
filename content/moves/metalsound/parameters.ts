 ...
 *
 * 世界化：把「摩擦金属的讨厌声音」落成一次**分几次慢慢刮**——施法者用身上的金属相互摩擦，刺耳的高频声贴着墙
 *   送到对手耳里，在它体内拉出一条长长的回响；每刮过一次就剥掉对手一级特防，直到把预算刮完。它不飞、不铺地，
 *   也不需要看见对方，但要把音磨出来就得站在原地花时间：本组起手最长、回响最久的一招，作用于特防。
 *   与同族分开：刺耳声是一条能扫多人的走廊、假哭要贴脸通视、怪异电波是绕身一圈；金属音只对一个人，
 *   但穿墙、磨得最久、分级剥防。
 *
 * 数值来源（每个参数读不同的精灵数据）：
 *   reach   回响距离：基础 4 格 + 碰撞箱高度×1.2 + 等级×0.02；夹 4..9。身板越高、越老练，声磨得越远。
 *   drop    特防下降预算：基础 2 级，体重 ≥ 100 加 1 级（金属共鸣更沉），长磨再加 1 级；夹 2..3。每级分一次刮擦。
 *   gap     刮擦间隔：基础 12 刻 − (速度 − 60) × 0.03；夹 8..14。手越快，两次刮擦之间隔得越短。
...
namespace PokemonSkills {
    ...
    actionParameters.define(metalsoundId, {
        ...
        drop: formula(
            ...
            "特防下降", {
                unit: " 级",
                description: "被刮擦声磨掉的特防等级总数，也是分成几次刮擦的次数；每刮过一次只降一级，施法者体重达到 100 时 +1，长磨再 +1。"
            }),
        gap: seconds(
            F.base(12).minus(F.stat("speed").minus(60).max(0).times(0.03)).clamp(8, 14).round(0),
            "刮擦间隔", "两次刮擦之间隔多久；施法者速度越快，剥防的节奏越紧。目标在这段时间里离开射程，剩下的刮擦就落空。"),
        ring: seconds(
    ...
    describe(metalsoundId, [
        { key: "description.0", values: ["drop","ring","gap"] },
