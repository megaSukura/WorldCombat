 ...
 *   唯一按自身体重结算、且专打空中的近身招：跃到一个目标头上、用身体压下来，命中时若目标离地就用
...
namespace PokemonSkills {
    ...
    actionParameters.define("flyingpress", {
        ...
        /** 跃起高度：3.2 + 身高偏移[−0.3,1.2] + 高空 1.2；夹 2.4..5.5 格。实际升程受顶板截断。 */
        leapHeight: formula(
            F.base(3.2)
            ...
            "跃起高度", { base: 3.2,
                unit: "格",
                description: "从自身当前位置向上跃到多高再压下来（实际升程会被顶板、方块真实截短）；身量越高、高空配置跃得越高。"
            }),
        ...
        /** 撞开距离：0.4 + 自身体重偏移[0,0.6]；夹 0.2..1.0 格；走原生受击位移，抗位移目标不动。 */
        push: formula(
            F.base(0.4).plus(F.body("weight").minus(300).times(0.0006).clamp(0, 0.6)).clamp(0.2, 1.0).round(2),
            "撞开距离", {
                unit: "格",
                description: "压中后把目标沿水平方向撞开的距离；身体越沉撞得越远。走原生受击位移，抗击退/抗位移的目标不会被硬拽。"
            }),
        /** 向下原生冲量（内部参数）：命中离地目标时向下的速度补偿，尊重原生抗击退与权限。 */
        crush: hidden(0.3),
