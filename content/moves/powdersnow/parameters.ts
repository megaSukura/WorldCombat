 ...
 *   出手快、能反复放，朝瞄准方向罩住扇面里的几个敌人，各挨一次轻冻伤、被吹退半步，并各自掷一次冰冻；
 *   中间的方块会挡住雪，可以朝空地空放。它是全家最便宜的「连续掷冰冻」工具：单次很轻，但冷却短、能一直吹。
 *
 * 与同族分开：
 *   细雪     —— 近身瞬发、便宜可连放的宽扇雪霰。
 *   冰冻之风 —— 一堵会往前推、只扫过一次的走廊冷锋；范围更窄、更远、有减速（另一单元）。
 *   冰冻光束 —— 停留片刻的固定窄光束，射程长、可穿人（同组）。
 *
 * 数值来源（每项读不同的精灵数据，分散到不同参数上）：
 *   puff         每目标威力 34 + 特攻偏移 + 等级偏移，乱雪式 ×0.85、细雪式 ×1.15。
 *   range        吹拂距离 5.5 + 特攻偏移 + 碰撞箱高度偏移，乱雪 ×1.15（驱动实际射程）。
 *   angle        扇面半角 40 + 特攻偏移，乱雪 ×1.25（判定扇形）。
...
namespace PokemonSkills {
    actionParameters.define("powdersnow", {
        ...
        range: formula(
            ...
            "吹拂距离", {
                base: 5.5, unit: "格",
                description: "这口雪能吹多远；特攻与身高让它够到更远一点。它也是本招的实际射程。"
            }),
        ...
        angle: formula(
            ...
            "扇面半角", {
                base: 40, unit: "度",
                description: "扇面从瞄准线向两侧各张开多少度，决定它有多宽，也决定能同时罩住多散的人。"
            }),
