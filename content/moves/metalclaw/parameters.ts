 ...
 * 翻译：把「用钢铁之爪劈开对手，有时提高自己的攻击」落成**贴脸左右两爪各劈一下**——爪刃劈中活物就被磨出锋口，
 *   磨出的物攻让下一记劈得更重；它是四式里最近、最快、最便宜的一记，靠贴着目标连打把攻击一点点喂起来。
 *
 * 与同族分开：钢翼是横向一扇、把面前的人一起扫开、磨的是防御；金属爪是贴脸两点、磨的是攻击，越劈越重。
 *
 * 数值分散（每项依赖不同的精灵数据）：
 *   rake          爪击威力：物攻给出爪劲、速度给出挥速；磨爪式一记更重，连爪式每记更轻。
 *   reach         爪程：速度与体型共同决定够到多远；也是本招实际射程的来源。
 *   radius        爪刃判定：身高与体重决定爪面宽度。
 *   gap           两爪间隔：速度越快两爪接得越紧。
 *   sharpenChance 磨利几率：物攻与等级；磨爪式更专注，连爪式每记各掷一次。
 *   sharpenStages 单次施放最多磨起的物攻级数：等级 55 台阶抬到 2。
 *   knock         击退：物攻。
 *   sparks        火星数量：物攻与速度派生，表现按它发射。
 *   tempo/aftercast/recharge 速度与等级决定起手、收招与冷却；磨爪式以更慢更贵换更重的单记。
 *
 * 配置 `hone`（磨爪）双向取舍：开启＝只劈一记但更重、更高几率、击退更远，起手与冷却更久；
 * 关闭（连爪）＝两记各掷一次磨利，更快更便宜、单记更轻。两个方向分别对应「稳扎一记」与「多点机会」。
 */
namespace PokemonSkills {
    actionParameters.define("metalclaw", {
        ...
        rake: formula(
            ...
            "爪击威力", {
                unit: "威力",
                description: "每一记钢爪劈中的威力；物攻越重、挥得越快越狠。磨爪式合成一记更重的劈砍，连爪式把力分成两记。对手防御、相性与暴击在命中时另算。"
            }),
        ...
        reach: formula(
            ...
            "爪程", {
                unit: "格",
                description: "朝前劈出的距离，也是本招的实际射程来源；腿快、身长的个体够得更远，磨爪式探得更深一点。"
            }),
        ...
        gap: formula(
            ...
            "两爪间隔", {
                unit: "刻",
                description: "连爪式里左右两爪之间的间隔；速度越快接得越紧，也越难被反手打断。磨爪式只有一记，此项不生效。"
            }),
        ...
        sharpenChance: percent(
            ...
            "磨利几率", "劈中后爪刃被磨出锋口、物攻提升的几率；磨爪式捏得更准，连爪式每记各掷一次。"),
        ...
        knock: formula(
            ...
            "击退", {
                unit: "格",
                description: "劈中后把目标带开的方向偏移；物攻越高推得越远，磨爪式的一记掀得更开。"
            }),
        ...
        tempo: seconds(
            F.base(6).minus(F.stat("speed").minus(60).times(0.015).clamp(-1.5, 2))
                ...
                .clamp(4, 12).round(0),
            "起手", "张开双爪、把刃口亮出来的时间；速度越快越短，磨爪式多蓄一拍。"),
