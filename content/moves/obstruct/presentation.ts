 ...
 * 拦堵的客户端表现。
 *
 * 一句话：一圈深色尖桩从地面顶起围住自己，撞上来的每一击在桩面上爆开暗色冲击，撞的人被一根短刺扎退、防御数值当场下降。
 * 色相家族：深岩灰蓝为主体（spike / smoke / groundquake），惩罚层用 impact_dark 的原色与琥珀火星强调。
 * 拍子：起（raise 0–14t，尖桩自下而上顶出）→ 击（block 每次拦截、punish 每次扎退）→ 收（shatter 崩解）。
 * 范围：hold 的桩环按 `data.scale`（拒马宽度／1.6）铺开——画面就是被判定的那一圈。
 * 运动：起手尖桩向上顶出并扬尘；持罩几乎静止，只在被撞击处向内一震；惩罚时从接触点向攻击者射出短刺。
 * 数：`data.scale` 放大桩环与刺的尺寸，`data.punishCount`（降防级数 ×8）就是 dark 爆点的数量，`data.intensity`（剩余量／初始量）决定亮度。
...
const ObstructDefinition: ParticleDefinition = {
    ...
    moments: {
        raise: {
            ...
            emitters: [
                {
                    name: "stakes", bind: "source", offset: [0, 0.02, 0], height: 0, fit: "none",
                ...
                {
                    name: "puff", bind: "source", offset: [0, 0.02, 0], height: 0, fit: "none",
                ...
                {
                    name: "grit", bind: "source", offset: [0, 0.02, 0], height: 0, fit: "none",
        ...
        hold: {
            // 持续状态：深色低密度桩环，放在脚边，让玩家看清目标。
            emitters: [
                {
                    name: "stake_ring", bind: "source", offset: [0, 0.02, 0], height: 0, fit: "none",
                    particle: "world_combat_core:cobblemon/generic/spike",
                    rate: 8, shape: { kind: "ring", radius: { data: "scale", fallback: 1 } },
        ...
        block: {
            ...
            emitters: [
                ...
                {
                    name: "splinter", bind: "target", height: 0.5, fit: "none",
        ...
        punish: {
            ...
            emitters: [
                {
                    name: "stab", bind: "target", height: 0.45, fit: "none",
                    particle: "world_combat_core:cobblemon/generic/spike",
                ...
                {
                    name: "crumble", bind: "target", height: 0.45, fit: "none",
                    particle: "world_combat_core:cobblemon/generic/impact/impact_dark",
        ...
        shatter: {
            ...
            emitters: [
                {
                    name: "break", bind: "target", offset: [0, 0.03, 0], height: 0, fit: "none",
                ...
                {
                    name: "fall", bind: "target", offset: [0, 0.5, 0], height: 0.4, fit: "none",
                    particle: "world_combat_core:cobblemon/generic/spike",
                    burst: { count: 36 },
                ...
                {
                    name: "dust", bind: "target", offset: [0, 0.05, 0], height: 0, fit: "none",
                    particle: "world_combat_core:cobblemon/generic/smoke/smoke",
                    burst: { count: 20 },
