 ...
 *   **当前挨肘的这个人**打过（`world.observe(self).hurtAgo() <= window` 且 `lastAttacker` 就是挨肘者）。
 *   站住、侧步让开正面、把这一肘顶回去；只有正中那名最近出手的人，肘上的伤才会翻倍。
 *
 * 现实接触由动作层的 `action.trace(..., true)` 决定：第一个身体或实墙就是真实接触点。判读用现场那个
 *   被害人重建上下文（`withTarget`），**显式 target 优先于原锁定目标**；因此第三人挡住肘路时，读到的是
 *   那个挡路者的状态，不会把对原仇人的翻倍借过来。
 *
 * 数据分散（每项依赖不同的精灵数据）：
 *   retort   报复威力 58 + 物攻偏移 + 等级偏移；正中最近出手者时 ×2，硬扛式 ×0.92。
 *   window   记仇窗口 2.2 秒 − 速度偏移 + 硬扛 0.8 秒（快的个体只记得眼前这一下）。
 *   reach    横肘距离 2.2 格 + 物攻偏移；也是实际射程来源，短。
 *   shuffle  侧步 0.34 格 + 速度偏移；快的个体让位更多，上限半格。
 *   radius   判定半径 0.34 格 + 体型高度偏移。
 *   push     顶开 0.60 格 + 物攻偏移（把贴脸的人一肘送开）。
 *   smash    肘风粒子数 12 + 物攻偏移 + 等级偏移，驱动表现。
 *   tempo／settle／recharge 速度决定起手、收招、冷却。
 *
 * 配置 `endure`（硬扛）：开启＝记仇更久 +0.8 秒、顶开 ×1.20，但本击 ×0.92、起手 +2 刻、冷却 +6；
 *   关闭＝本击 ×1.06、出肘更快。两向各有局面：站住等这一口 vs 立刻还手。
 */
namespace PokemonSkills {
    ...
    /** 施法者是否最近被本次挨肘者本人打过；1 即这口仇能翻倍。显式 target 优先，动作原目标是兜底。 */
    ...
    actionParameters.define(revengeId, {
        /** 报复威力：58 + 物攻偏移[−14,34] + 等级偏移[−4,10]；正中最近出手者 ×2、硬扛式 ×0.92 / 常规 ×1.06；夹 34..150。 */
        retort: formula(
            F.base(58)
            ...
            "报复威力", {
                unit: "威力",
                description: "这一记横肘的基准威力；物攻与等级越高越重。若最近打你的正是这次挨肘的人，威力翻倍。对手防御、相性与暴击在命中时另算。"
            }),
        ...
        window: seconds(
            ...
            "记仇窗口", "这个人在伤到你之后这段时间内挨肘，这一记就翻倍；快的个体只记得眼前这一下，硬扛式记更久。"),
        /** 横肘距离：2.2 格 + 物攻偏移[−0.3,0.8]；夹 1.8..3.0；也是实际射程来源。 */
        reach: formula(
            F.base(2.2).plus(F.stat("attack").minus(58).times(0.010).clamp(-0.3, 0.8)).clamp(1.8, 3.0).round(2),
            "横肘距离", {
                unit: "格",
                description: "站定侧步后这一肘能够到的最大距离，也是本招的实际射程来源；臂力大的个体顶得更前。"
            }),
        /** 侧步：0.34 格 + 速度偏移[−0.08,0.16]；夹 0.16..0.50（上限半格）。 */
        shuffle: formula(
            F.base(0.34).plus(F.stat("speed").minus(58).times(0.005).clamp(-0.08, 0.16)).clamp(0.16, 0.50).round(2),
            "侧步", {
                unit: "格",
                description: "出肘前让开正面的一小步；越快让得越开，最多半格。墙挡住侧步时只挪到实际够到的位置。"
            }),
        ...
        radius: formula(
            ...
            "判定半径", {
                unit: "格",
                description: "这一肘能咬住多大一圈；身板大的个体肘面更宽。"
            }),
        ...
        tempo: seconds(
            ...
            "起手", "站定、侧步到顶出横肘之间的时间；速度快的个体起得更快，硬扛式先站稳。"),
        /** 收招：8 刻；夹 5..13。 */
        settle: seconds(F.base(8).clamp(5, 13).round(0), "收招", "一肘顶完后收住的时间。"),
        ...
        recharge: seconds(
            ...
            "冷却", "这一记横肘之后多久能再还一次；速度快的个体回得更快，硬扛式更费。")
    });
    ...
    describe(revengeId, [
        ...
        { key: "description.1", values: ["shuffle","reach","radius","push"] },
