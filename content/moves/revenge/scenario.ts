 ...
 * 报复 / revenge —— 可执行设计说明。
 *
 * 一句话：站定还手；最近被当前目标本人打过时，这一拳翻倍并把对手送开。
 *
 * 场面：一只只带这一招的格斗手对一只会还手、血厚的对手；平地、夜晚，避免日光与地形干扰读数。
 * 对手先打到施法者之后，接下来的回拳就在记仇窗口里翻倍，轨迹能读出那一段。
 * 断言只取必然事实：这招被提交过、对手吃到过伤害。
 * 具体哪几记翻倍取决于谁先动手，写进 note 供读轨迹判断。
 */
Smoke.scenario("revenge", function (stage) {
    ...
    stage.until(1600, function () {
        ...
        stage.after(40, function () {
            ...
            stage.expect(stage.damageTo(foe) > 0, "the retort punch dealt damage");
            stage.note("doubling needs the caster to have been hurt by this very target inside the window; read the trace for whether the foe struck first", {
