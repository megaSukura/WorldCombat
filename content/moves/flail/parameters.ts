 ...
 *
 * 翻译：把「抓狂般乱打」落成**以当前瞄准为中线左右交替连甩的一串乱拳**——血量越低，甩出的下数越多、
 *   每下也越重，残血时是一整片失控的乱打。原生的六档台阶在这里摊成连续的两条曲线（次数与单发威力都随缺失
 *   血量涨），玩家从挥出的下数与扇面密度就能读出自己还剩多少血。与同族分开：喷火／喷水／巨龙威能是满血越强
 *   的爆发，抓狂是唯一**越残越疯**的招，也是唯一的近身多段扇面；它不锁定一个人，左右两记轮着甩。
 *
 * 数据分散（每项读不同的精灵数据）：
