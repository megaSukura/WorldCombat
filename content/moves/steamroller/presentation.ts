 ...
 * 范围：roll 的 `path` 是与服务端同一组四个顶点的走廊，画出的就是会被压到的那条线；crush 绑在目标身上。
 * 运动：curl 的尘向里收；roll 的草屑贴地向外抛、土屑带重力；crush 的虫粉向外炸。
 * 数：roll 与 crush 的量绑定 `data.dirt`（物攻派生），`data.scale`（碾压半宽派生）决定球的体积，`data.intensity` 抬命中亮度。
...
const SteamrollerDefinition: ParticleDefinition = {
    ...
    moments: {
            ...
            emitters: [
                {
                    ...
                    particle: "world_combat_core:cobblemon/generic/earth",
                    shape: { kind: "polygon" }, rate: 30, direction: "outward", speed: [0.04, 0.16],
        ...
        crush: {
            ...
            emitters: [
                {
                    name: "pressed", bind: "point", offset: [0, 0.15, 0], height: 0.3, fit: "none",
                ...
                {
                    name: "crumbs", bind: "point", offset: [0, 0.1, 0], height: 0.2, fit: "none",
                    particle: "world_combat_core:cobblemon/generic/tinydust",
