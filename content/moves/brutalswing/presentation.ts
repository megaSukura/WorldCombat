 ...
 * 拍子：起 gather（收势蓄力）→ 扫 sweep（当前刃弧生长）→ 击 hit（扫中爆点）→ 收 settle（整圈尘环）。
 * 范围：sweep 用 `data.path`（服务端只给上一角到当前角这一小段，加很短过去轨迹）铺成薄扇面，刃到哪就扫到哪，
 *   不再从起始累积成满饼；settle 用 `data.radius` 画整圈，半径直接读机制。
