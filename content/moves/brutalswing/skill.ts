 ...
 *   扫（sweep → hit）：提交后从起始朝向横转 `lap` 刻；转到的方位上、还在半径内的敌人各挨一次 `sweep`，
 *       按体重推力的 `jolt` 被甩开。画面里那道弧实时长出来，玩家看得出扫到哪、还剩多少。
...
namespace PokemonSkills {
    ...
    define({
        ...
        execute: function (action, move, config, done) {
            ...
            const jolt = p("brutalswing", "jolt", action);
            ...
            /** 先把半径内每个敌人的方位量出来，按方位排序，扫到时再逐个结算。 */
            const foes: { ref: string; bearing: number }[] = [];
            WorldGeometry.selectEnemies(world, WorldGeometry.ring(centre, 0, radius, { below: 2.0, above: 2.5 }),
                function (enemy, facts) {
                    if (String(enemy.ref()) === selfRef) return;
                    const delta = facts.position().minus(centre);
                    let bearing = Math.atan2(delta.x(), delta.z()) - base;
                    bearing = ((bearing % full) + full) % full;
                    foes.push({ ref: String(enemy.ref()), bearing: bearing });
                });
            foes.sort(function (a, b) { return a.bearing - b.bearing; });
            let step = 0, index = 0, hits = 0;

            /** 已扫过的那段扇面顶点：圆心 + 从起始朝向到当前角度的采样点，画面与判定读同一份半径。 */
            function arcPath(angle: number): number[][] {
                const points: number[][] = [[centre.x(), centre.y() + 0.1, centre.z()]];
                const segments = Math.max(2, Math.ceil(angle / (full / 16)));
                for (let i = 0; i <= segments; i++) {
                    const a = base + angle * i / segments;
            ...
            /** 推进一段弧：扫过方位的人逐个结算；扫满 `lap` 刻后收招。 */
            function advance(current: CombatAction): void {
                const scope = current.world();
                const angle = full * (step + 1) / lap;
                while (index < foes.length && foes[index].bearing <= angle) {
                    const foe = foes[index]; index++;
                    const victim = scope.actor(foe.ref);
                    if (victim === null || !scope.valid(victim) || scope.friendly(victim)) continue;
                    const facts = scope.observe(victim);
                    if (facts === null || facts.position().minus(centre).length() > radius + 0.6) continue;
                    if (!hurt(current, victim, "brutalswing", power, { damage: damageSpec("brutalswing", "sweep"), contact: true, slice: true })) continue;
                    hits++;
                    WorldFeedback.emit(scope, brutalswingScene, 1, facts.position(),
                        { moment: "hit", target: foe.ref, scale: scale, intensity: intensity, count: Math.round(8 + power * 0.2) }, 18);
                    const away = WorldCombat.point(facts.position().x() - centre.x(), 0, facts.position().z() - centre.z());
                    if (scope.valid(victim) && away.length() > 0.15) scope.displace(victim, away.unit().scale(jolt));
                }
                WorldFeedback.keep(scope, "brutalswing:sweep:" + String(current.actor().ref()), brutalswingScene, 1, centre,
                    { moment: "sweep", path: arcPath(angle), progress: Math.min(1, (step + 1) / lap),
                        direction: [Math.sin(base), 0, Math.cos(base)], radius: radius, scale: scale, intensity: intensity }, 8);
                step++;
