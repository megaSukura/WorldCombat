 ...
 * - 即时战斗翻译：伸长喙一记快速啄击，够得远也够得高（能啄到浮在空中的对手）；若目标携带树果，
 *   当场啄下吞掉、立刻获得效果。与同为“吃果”的虫咬分开：啄食不贴近、不咀嚼，靠长喙与抬升的高度取胜，
 *   吞得快而浅（吸收系数低于 1）；虫咬则贴近咬合、咀嚼后吸收更充分。
 * - 参数分散到精灵数据：威力取物攻（喙尖）与速度（出喙快），扇长取速度与等级，抬升高度取速度与等级，
 *   判定半径取体型高度（喙尖窄），轻推取体重；吸收系数取速度，粒子数量取速度。
 *   树果是什么、吃下有什么基本效果，统一读 `NativeItems` 的共享树果注册库；本招只决定吸收系数与节奏。
 * 配置 swoop（俯冲）：够得更远更高（射程 ×1.3、抬升 ×1.4、冷却 +6），代价是本击 ×0.88；关闭则本击 ×1.08。
 *
...
namespace PokemonSkills {
    ...
    actionParameters.define("pluck", {
         ...
         *  等级每比 30 多 1 加 0.4（夹 -6..+12）；swoop 开 ×0.88、关 ×1.08；夹在 34..104。 */
        peck: formula(
            F.base(50)
                ...
                .times(F.when(F.pref("swoop"), F.const(0.88), F.const(1.08)))
         ...
         *  swoop 开 ×1.3；夹在 3.0..6.5 格。 */
        reach: formula(
            F.base(3.6)
                .plus(F.stat("speed").minus(60).times(0.02).clamp(-0.4, 1.6))
                .plus(F.level().minus(30).times(0.01).clamp(-0.2, 0.5))
                .times(F.when(F.pref("swoop"), F.const(1.3), F.const(1.0)))
                .clamp(3.0, 6.5).round(2),
            "长喙距离", {
                unit: "格",
                description: "从起喙到啄中的距离；快的个体伸得更远。它也是本招的实际射程来源，比虫咬长得多。"
            }),
        /** 抬升高度：基础 2.2 格，速度每比 60 多 1 加 0.02（夹 -0.4..+1.6），等级每比 30 多 1 加 0.01（夹 -0.2..+0.5）；
         *  swoop 开 ×1.4；夹在 1.8..5.0 格（判定走廊向上的高度带）。 */
        lift: formula(
            F.base(2.2)
                .plus(F.stat("speed").minus(60).times(0.02).clamp(-0.4, 1.6))
                .plus(F.level().minus(30).times(0.01).clamp(-0.2, 0.5))
                .times(F.when(F.pref("swoop"), F.const(1.4), F.const(1.0)))
                .clamp(1.8, 5.0).round(2),
            "抬升高度", {
                unit: "格",
                description: "这一啄向上够得到多高；快而熟练的个体能啄到浮在空中的对手（原生 distance 标记的翻译）。"
            }),
        ...
        radius: formula(
            ...
            "判定半径", {
                unit: "格",
                description: "喙尖够到活体的横向判定半径；身板越大略宽，但始终比虫咬的张口窄。"
            }),
        ...
        aftercast: seconds(
            ...
            "收招", "收喙的时间；快的个体几乎不停。"),
        /** 冷却：基础 22 刻，速度每比 60 多 1 减 0.06（夹 -2..+4）；swoop 开 +6；夹在 16..40 刻。 */
        recharge: seconds(
            F.base(22).minus(F.stat("speed").minus(60).times(0.06).clamp(-2, 4))
                .plus(F.when(F.pref("swoop"), F.const(6), F.const(0)))
                .clamp(16, 40).round(0),
    ...
    describe("pluck", [
        ...
        { key: "description.2", values: ["absorb", "motes"] },
