 ...
 * 啄食 / pluck 的客户端表现。
 *
 * 一句话：施法者抬头张开喙、翅下卷起细细的风，随后沿瞄准方向伸出一条长喙走廊，在前端啄中对手；
 * 若啄到树果，绿色的果汁与果屑向喙尖一收，紧接着暖绿的光顺着身体升起、把果子吞进肚子。
 * 色相家族：风白（gust / speedlines）作运动，飞行紫（impact_flying）作命中，果绿（smallleaf / glowingsparkle）
 * 只在“啄到树果”时进入；三者合起来读作“风—啄—吞”。
 * 拍子：起（raise 抬头）→ 啄（peck 长喙走廊）→ 击（hit 命中）→ 得（gain 吞果）／空（miss 风屑）。
 * 范围：peck 沿 `data.path` 的两个端点拉出一条长喙走廊，走廊画到哪就够到哪；`data.lift` 决定走廊向上抬多高。
 * 运动：抬头时风从翅下向上卷；啄出时速度线沿走廊向前射；命中是短促的紫白外爆；吞果是暖绿由下向上。
...
const PluckDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        peck: {
            ...
            emitters: [
                {
                    name: "lane", bind: "path", fit: "none", height: 0.35,
                    particle: "world_combat_core:cobblemon/generic/speedlines",
                ...
                {
                    name: "gust", bind: "path", fit: "none", height: 0.35,
