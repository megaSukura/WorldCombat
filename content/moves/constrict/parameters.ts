 ...
 *   tip         触手粗细：施法者体型与等级派生，是判定线的接触半径；内部参数，不单独面向玩家。
 ...
 *
 * 配置 latch（攀缠式，默认关）双向取舍：开＝缠得久、定得久、再压一级速度，并在束缚期间维持一根可被拉断
 * 或隔墙截断的实体藤连接，但伤害更低，且施法者要分出肢体按住目标（自身短暂 rooted）；关（绞缠式）＝一记
 * 更重更快的硬绞，束缚更短、定身更短、没有持续连接。两向各有局面（配合队友控场 vs 单点速伤）。
 *
...
namespace PokemonSkills {
    ...
    actionParameters.define("constrict", {
        ...
        /** 触手粗细：基础 0.32 格；施法者宽度每比 0.9 多 1 加 0.25（上限 +0.30）；等级每比 20 高 1 加 0.004（上限 +0.12）；夹在 0.22..0.7。 */
        tip: formula(
            F.base(0.32).plus(F.body("width").minus(0.9).times(0.25).clamp(-0.1, 0.30))
                .plus(F.level().minus(20).times(0.004).clamp(0, 0.12))
                .clamp(0.22, 0.7).round(2),
            "触手粗细", {
                unit: "格", visible: false,
                description: "触手前进时判定接触的半径；体型越大、等级越高越粗，能更快碰到贴边的目标。"
            }),
