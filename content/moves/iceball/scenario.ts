 ...
 * 冰球 / iceball —— 可执行设计说明。
 *
 * 一句话：蜷身抛出一颗会自己回头的冰球，每撞中一趟冻厚一层、下一趟更重，最后碎开留下冰面。
 *
 * 场面：一只只会冰球的冰宝（40 级，原生真实学习者之一）对一只被点住、不会还手的铁傀儡（厚血靶子）。
 * 断言只取必然事实：这招被提交过（`stage.casts`）、目标受过伤害。实际撞了几趟、是否每趟都接上、
 * 碎开有没有冻住地面，都写进 note 供读轨迹判断。
 */
Smoke.scenario("iceball", function (stage) {
    ...
    stage.until(1200, function () {
        ...
        stage.after(80, function () {
            ...
            stage.note("each landed pass exceeds the last by ramp (2x by default); the ball returns on a whiff instead of ending, and the caster stays rooted for the whole sequence. The ice patch is a real terrain lease that stage.changedBlocks() cannot see from here.", {
