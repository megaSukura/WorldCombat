 ...
 * 麻痹粉的可执行设计说明。
 *
 * 场面：一只只会麻痹粉的走路草对 3 格外的卡比兽抛粉，卡比兽没有招式、站在原地。AI 会把粉团抛向它的落点，
 *   距离在射程内、中间没有遮挡，云就落在它脚下。
 * 必然事实：本招被提交过；目标身上出现过共享麻痹（`world_combat:paralysis` 效果与 `world_combat:status/paralysis`
 *   身份）；麻痹把它的移动速度压低了。
 * 随机结果：粉团落地时卡比兽还在不在云里、以及它之后是否走出云外都写进 note 供读轨迹判断。
 *   云在施法动作结束后仍按自己的寿命存在、并在驱散/到期时同步收掉画面，属于跨动作过程，由源码承担；
 *   烟测只覆盖它在当场麻住目标。它不造成伤害（原生威力 0）。
 */
