 ...
 *         持续噼啪；走进来的人身上会被一层薄电罩住，走出去时薄电短续一下再碎成小火花脱落。
 * 色相家族：electric cyan 0x8FE8FF 作场地主体；暖白 0xDFFBFF 作边缘高光；黄白 0xFFF3C4 只给落点强调。
 * 起击收：起 windup 16t ／击 field 46t、随场效果续期 ／击 ionize 20t ／持 film 随膜续期 ／收 shed 18t。
 * 持续状态：field 贴地低密度呼吸，边界用一圈环画清「站哪里会被电离」，随场效果一起收走；film 只在膜还贴着时亮。
 ...
 * film  主体 electricity_white  球面贴附、sin        0.1-0.03  12-22 0.35→0 ≤40
 * film  细节 accessory_spark    环上浮             0.05-0.01 10-18 0.5→0  ≤30
...
const IonDelugeSceneDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        film: {
            exit: { drain: 24 },
            emitters: [
                { name: "film_shell", bind: "target", height: 0.5,
                    particle: "world_combat_core:cobblemon/generic/electricity/electricity_white",
                    rate: 10, shape: { kind: "sphere_surface", radius: 0.45 },
                    direction: "up", speed: [0.01, 0.04],
                    lifetime: [12, 22], size: [0.1, 0.03],
                    color: 0x8FE8FF, alpha: [0.35, 0], alphaMode: "sin", light: "full", maxParticles: 40 },
                { name: "film_spark", bind: "target", height: 0.5,
                    particle: "world_combat_core:cobblemon/generic/status/accessory_spark",
                    rate: 8, shape: { kind: "ring", radius: 0.38 },
                    direction: "up", speed: [0.005, 0.03],
                    lifetime: [10, 18], size: [0.05, 0.01],
                    color: 0xDFFBFF, alpha: [0.5, 0], light: "full", maxParticles: 30 }
            ]
        },
        shed: {
            duration: 18,
