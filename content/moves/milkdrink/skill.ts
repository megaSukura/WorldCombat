 ...
 *   最后一口结束后调用 CombatStatus.cure(world, self, "poison")，只要身上带着任一来源的中毒就被冲掉（refresh 一幕）。
 * 收势：饮完抹嘴（wipe），结束。
 *
 * 反制：连饮有节拍、要花几息——对手可以在这段时间继续压血，把每一口都吃掉；解毒是额外收益，不是保险。
 * 与同族分开：自我再生按刻连续、偷懒留下减速、生蛋外化为蛋；喝牛奶是有节拍的连饮，并额外清毒。
 */
namespace PokemonSkills {
    ...
    define({
        ...
        description: "仰头连饮几大口，把回复分成几口交付、总量约最大生命的一半；最后一口把身上的中毒冲掉。饮用期间可以继续走位，但每一口都要花掉几息。",
        uses: ["分几口补回生命", "顺手冲掉中毒", "满血中毒时也值得动用"],
        ...
        execute: function (action, _move, config, done) {
            ...
            function step(current: CombatAction): void {
                ...
                if (healed > 0) feedback(access, self, now.position(), "heal", { amount: Math.round(healed * 10) / 10 });
                left = left - 1;
                sound(current, "minecraft:entity.generic.drink");
                WorldFeedback.emit(access, milkdrinkScene, 1, now.position(),
                    { moment: "gulp", target: String(self.ref()), drops: drops, left: left, total: gulps, scale: scale }, 24);
                if (left <= 0) {
                    var cleaned = CombatStatus.cure(access, self, "poison");
                    if (cleaned) {
                        world.sound("minecraft:item.honey_bottle.drink", now.position(), 16, "{}");
                        WorldFeedback.emit(access, milkdrinkScene, 1, now.position(),
                    ...
                    WorldFeedback.emit(access, milkdrinkScene, 1, now.position(),
                        { moment: "wipe", target: String(self.ref()), scale: scale }, 22);
                    done(current);
                    return;
                }
