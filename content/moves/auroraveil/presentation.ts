 ...
 *
 * 一句话：施法者把天光拉下来，按落点上方实际能挂多高横起几道虹带、铺成一片极光，地面亮起一圈光边画出幕的范围；
 * 幕下的人由一条细丝连回幕心，来袭的攻击撞上时折出一道亮带，极光慢慢收拢、落下。
 *
 * 场地高度：`data.ceiling` 是机制实测的可挂高度，幕顶虹带与持续层都贴到它；`data.midHeight` 是垂带中心。
 *   屋顶低到 3.2 格以内时，机制把 `highRibbons` 置 0、`lowRibbons` 放正，横带换成幕内短垂带，避免穿出屋顶。
 *
 ...
 * 层次：拉光（起）／地边圈＋幕顶虹带（铺开）／幕顶极光与边圈（持续）／细丝连幕（受护）／折光挡下（事件）／收。
 * 起击收：windup（聚光）→ curtain（拉开）→ veil（持续）→ cover（接上）→ thread（连幕）→ block（挡下）→ fade（收）。
 * 数：虹带数绑定 data.ribbons；地边圈与天顶极光的尺度绑定 data.scale（半径/4）；挡下的爆发量读 data.burst。
 */
    ...
    moments: {
        ...
        curtain: {
            ...
            emitters: [
                ...
                { name: "bands", bind: "point", offset: [0, { data: "ceiling", fallback: 5.4 }, 0], height: 0,
                    particle: "world_combat_core:cobblemon/generic/sparkle/shinesparkle_rainbow",
                    burst: { count: { data: "highRibbons", fallback: 8 }, interval: 3, repeats: 5 }, shape: { kind: "circle", radius: 3.6 },
                ...
                { name: "ribbon_a", bind: "point", offset: [0, { data: "ceiling", fallback: 5.0 }, 0], height: 0,
                    particle: "world_combat_core:cobblemon/generic/orb/xsfadeorblite",
                    burst: { count: { data: "highRibbons", fallback: 12 }, interval: 4, repeats: 3 }, shape: { kind: "line", length: 4.6, rotation: [0, 0, 20] },
                ...
                { name: "ribbon_b", bind: "point", offset: [0, { data: "ceiling", fallback: 4.6 }, 0], height: 0,
                    particle: "world_combat_core:cobblemon/generic/orb/xsfadeorblite",
                    burst: { count: { data: "highRibbons", fallback: 12 }, interval: 4, repeats: 3 }, shape: { kind: "line", length: 4.2, rotation: [0, 0, -24] },
                    direction: "shape", speed: [0.02, 0.08], drag: 0.95,
                    lifetime: [20, 32], size: [0.3, 0.06],
                    color: 0x7FE6D8, alpha: [0.5, 0], light: "full", maxParticles: 90 },
                { name: "lowbands", bind: "point", offset: [0, { data: "midHeight", fallback: 2.7 }, 0], height: 0,
                    particle: "world_combat_core:cobblemon/generic/sparkle/shinesparkle_rainbow",
                    burst: { count: { data: "lowRibbons", fallback: 0 }, interval: 3, repeats: 5 },
                    shape: { kind: "line", length: { data: "ceiling", fallback: 5.4 } },
                    direction: "shape", speed: [0.01, 0.05], drag: 0.94,
                    lifetime: [22, 36], size: [0.22, 0.05], sizeMode: "sin",
                    alpha: [0.7, 0], light: "full", bloom: 0.2, maxParticles: 120 },
                { name: "lowrim", bind: "point", offset: [0, { data: "ceiling", fallback: 5.4 }, 0], height: 0,
                    particle: "world_combat_core:cobblemon/generic/ring/giantring_white",
                    burst: { count: { data: "lowRibbons", fallback: 0 } }, shape: { kind: "ring", radius: 3.6 },
                    direction: "outward", speed: [0.02, 0.08], drag: 0.94,
                    lifetime: [18, 30], size: [0.5, 0.9], sizeMode: "sin",
                    color: 0x7FE6D8, alpha: [0.5, 0], light: "full", maxParticles: 44 }
            ]
        },
        veil: {
            exit: { drain: 30 },
            emitters: [
                ...
                { name: "sky", bind: "point", offset: [0, { data: "ceiling", fallback: 5.2 }, 0], height: 0,
                    particle: "world_combat_core:cobblemon/generic/sparkle/shinesparkle_rainbow",
                    rate: { data: "highRibbons", fallback: 8 }, shape: { kind: "circle", radius: 3.6 },
                ...
                { name: "fall", bind: "point", offset: [0, { data: "ceiling", fallback: 4.0 }, 0], height: 0,
                ...
                { name: "lowbands", bind: "point", offset: [0, { data: "midHeight", fallback: 2.7 }, 0], height: 0,
                    particle: "world_combat_core:cobblemon/generic/sparkle/shinesparkle_rainbow",
                    rate: { data: "lowRibbons", fallback: 0 },
                    shape: { kind: "line", length: { data: "ceiling", fallback: 5.4 } },
                    direction: "shape", speed: [0.004, 0.016],
                    lifetime: [22, 36], size: [0.18, 0.04], sizeMode: "sin",
                    alpha: [0.4, 0], alphaMode: "sin", light: "full", maxParticles: 120 }
            ]
        },
        cover: {
            duration: 12,
