 ...
 *
 * 核心念头：召起一圈锋利叶片，在对手脚下立起旋转切割的旋风——它不是一颗弹丸，而是一块留在场上几秒的区域；
 * 每一拍割伤范围里的敌人，并按概率把叶屑扑进眼睛、削掉命中。对手可以走出旋风躲开后续切割。
 *
 * 三幕：
 *   起：叶片在施法者周围升起打旋（提交前 windup 预告）。
 *   裹：提交后在目标位置立起旋风（range 即落点距离），按 `interval` 一拍一拍地切割；
 *       每一拍罩住范围内所有敌人，命中下降每个目标整场只结算一次，避免多拍叠满。
 *   散：时间走完，叶片四散。
 *
 * 与同族分开：另外三招都是把东西送到脸上的弹丸；青草搅拌器不飞，它把对手围在一块会持续切割的区域里，
...
namespace PokemonSkills {
    ...
    define({
        ...
        cooldown: 48,
        style: "leaf",
        defaults: { tight: false, ai: { maxChase: 17, crowd: true, leaveStation: true } },
        fields: [
            flag("tight", "紧裹")
        ],
