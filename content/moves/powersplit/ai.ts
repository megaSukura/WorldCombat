 ...
 *   值不值得平看攻势底子之和（攻 + 特攻的原始值）：对方比自己高出至少 ai.edge 倍（默认 1.15）才出手——
 *   平完两人都落在同一个平均刻度上，差距越大你赚得越多；差距越大排序越靠前。
 * 自己反而更高时不参与候选（多半会被拉低），交给其他招；只剩本招时也不硬放。
...
namespace PokemonSkills {
    ...
    const powersplitEdge = field(pathOf("ai.edge"), "平分下限", "number", { min: 1.0, max: 2.5, step: 0.05,
        help: "对方的攻势底子之和要达到自己的这个倍数才出手；调高更挑剔、只在对手明显更强时平，调低更常出手。" });
    const powersplitStation = flag("ai.leaveStation", "驻守时允许离位");
    powersplitStation.help = "开启后，收到「驻守」指令时也会离开原位去平分。";

    addPreferences("powersplit", { ai: { maxChase: 12, edge: 1.15, leaveStation: false } },
        [powersplitChase, powersplitEdge, powersplitStation]);
}
