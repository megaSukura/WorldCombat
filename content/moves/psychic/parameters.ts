 ...
 *
 * 翻译：把「强大的念力」落成一记**抓住并挤压**的重手——先在目标身上收拢一团紫靛的念力、把它按在原地，
 * 拖向自己一段，再在片刻后狠狠一挤。它是本组最重、最慢、最贵的一发，身份是「握」：抓住、定住、拽动、
 * 压特防。相对于念力（便宜快发的骚扰弹），精神强念是看得见的控制重击。
 *
 * 数据分散（每项读不同的精灵数据）：
 ...
 *   drag          拖拽距离：特攻决定拽多远；大个子的抵抗在运行时按目标体型折减（见 skill.ts）。
 *   sunderChance  特防下降概率：原生 10% 起，特攻与等级提高，缠握式更容易。
 *   sunderStages  特防下降级数：固定 1 级，与原生一致。
 *   squeezeDelay  挤压延迟：速度决定抓稳后多快挤第二下。
 *   spirals       念力丝数：特攻与等级派生，驱动画面密度。
 *   起手／收招／冷却：速度决定。
 *
 * 配置 `hold`（缠握）双向取舍：开启＝定身 ×1.5、挤压 ×1.25、拖拽 ×1.2、特防下降 ×1.3，但擒压 ×0.94、
...
namespace PokemonSkills {
    ...
    actionParameters.define(psychicId, {
        ...
        drag: formula(
            ...
            "拖拽距离", {
                unit: "格",
                description: "抓住时把目标朝自己拽回来的距离；特攻越高拉得越远，缠握式更用力。体型越大的目标抵抗越强，运行时按它的身高折减。"
            }),
        ...
        squeezeDelay: seconds(
            F.base(10)
                ...
                .clamp(5, 18).round(0),
            "挤压延迟", "抓住后隔多久挤第二下；速度越快跟得越紧，缠握式多按一会儿再挤。"),
