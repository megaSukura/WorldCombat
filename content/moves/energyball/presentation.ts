 ...
 *
 * 一句话：从每一处真实植被点拉一条稀疏绿线把生机送进身前的草能球 → 球沿直线飞出、一路抛落草叶与光尘 →
 * 命中活物时炸开成一圈草种与光点，并在地面绽出一小片花草；打空也只留绽开。
 * 色相家族：草绿（0x7FBF3A / 0x8FD14A）为主，嫩绿高光（0xEFFFC8）只给球心与击点，暗绿（0x3E6B2A）做余韵。
 * 拍子：起 gather（真实植被点拉线＋球心成形）→ 行 travel（飞行）→ 击 burst（炸开）→ 绽 bloom（落地生长）／空 fizzle。
 * 范围：单发点射，由 travel 的直线轨迹读出；gather 的线读服务端真实取样点 `data.path`，不是均匀圆环。
 * 运动：每条线的两端是真实植被点与球心（polyline），球沿直线飞出（服务端速度），草种向外抛洒。
 * 数：gather 的线密度读 `data.sites`（保留的植被点数）、球大小随 `data.scale`；
...
const EnergyBallDefinition: ParticleDefinition = {
    ...
    moments: {
        gather: {
            ...
            emitters: [
                {
                    name: "gather_threads", bind: "path", fit: "none",
                    particle: "world_combat_core:cobblemon/generic/grass/xsseed",
                    shape: { kind: "polyline" },
                    rate: { data: "sites", fallback: 0 },
                    direction: "shape", speed: [0.01, 0.05], spread: 10, spin: 8,
                    lifetime: [10, 20], size: [0.09, 0.02],
                    color: 0x7FBF3A, alpha: [0.85, 0], light: "world", maxParticles: 70
                },
                {
                    name: "gather_leaves", bind: "path", fit: "none",
                    particle: "world_combat_core:cobblemon/generic/grass/leaf",
                    shape: { kind: "polyline" },
                    rate: 8, direction: "shape", speed: [0.02, 0.07], spread: 12, spin: 14,
            ...
            emitters: [
                {
                    ...
                    burst: { count: { data: "planted", fallback: 0 }, at: 1 },
