 ...
 * 装饰的可执行设计说明。
 *
 * 场面：一只只会装饰的霜奶仙（Alcremie）与一只僵尸队友（同队）在 5 格外，12 格外另有一只敌对僵尸。
 * 队友僵尸会去咬敌人（因此是共享观测里的「交战中」伙伴），AI 会先走过去把装饰送给它。
 * 必然事实：本招被提交过；队友身上出现过共享身份 world_combat:status/decorated；队友的物攻属性被抬高过。
 * 送了几件装饰、AI 是先送还是先挨打，随站位变化，写进 note 供读轨迹判断。
 */
Smoke.scenario("decorate", function (stage) {
    ...
    var ally = stage.mob({ type: "minecraft:zombie", at: [5, 0, 0] });
    var foe = stage.mob({ type: "minecraft:zombie", at: [12, 0, 0] });
