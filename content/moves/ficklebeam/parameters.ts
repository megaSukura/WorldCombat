 ...
 * 一起聚焦，这一发同时醒来 `heads` 股、每股各走一条近线并行覆盖更宽的一条线面；同一目标本次合计伤害封顶
 * 为原来的两倍，不按股无限乘。这一族里它是唯一**没有自损**的一招，风险和回报都在那一掷里。
 * 头部数量对多数生物不存在，所以翻成「分光股数」：体型越大，能同时聚起的光股越多，齐射时覆盖越宽。
 *
...
namespace PokemonSkills {
    ...
    actionParameters.define(ficklebeamId, {
        ...
        beam: formula(
            ...
            "光束威力", {
                unit: "威力",
                description: "单股光线的基础威力；特攻越高、等级越高越强，齐心式为了赌齐射压低基础。齐射时每股各走一条近线并行覆盖更宽，同一目标本次合计不超过基础的两倍。对手防御、相性与暴击在命中时由共享结算另算。"
            }),
