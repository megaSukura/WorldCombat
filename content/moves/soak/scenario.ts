...
Smoke.scenario("soak", function (stage) {
    ...
    stage.until(420, function () {
        ...
        stage.after(90, function () {
            stage.note("浸水把目标当前的全部属性冲成单一水属性（共享 NativeModifiers types 层，到期自动还原原生属性），并挂上共享身份的水属性标记；命中目标身上另有一个同寿命的托管效果持有水膜表现，水干时一起散去。只对宝可梦、且不是纯水的目标浇得进去（纯水预检直接拒绝），空点只泼水、不附带属性目标；漫流只波及与主目标同敌我类别的一圈人。属性是否真的带上水、水膜与漫流范围、水干还原由共享结算与世界读数决定，留给完整装配的人工试玩。浸透时长随等级与特攻、水柱条数随特攻、水花圈数随速度分别变化。", {
