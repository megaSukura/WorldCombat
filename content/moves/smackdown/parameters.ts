 ...
 *   价值全在落点之后：砸中一个离地的目标，就持续用向下的原生受击冲量把它拽向正下方地面（有限冲量、
 *   逐刻读真实高度，抗推 Boss 不动也不谎报），拔掉它身上新获得的浮空身份，并申请一段部分拘束身份
 *   `world_combat:status/partiallytrapped`／`world_combat:status/smackdown`（本单元 `world_combat:smackdown_pin`）；
 *   身份被拒就只结算主伤。贴地的普通对手只挨这一记。
 *
 * 与同族分开（同一族「垂直轴」，见报告）：
 ...
 *   pull            下坠速度 0.8 格/刻 + 物攻偏移：每刻把目标竖直速度补到这么快（不覆盖水平运动），也是坠地画面速度。
 *
 * 配置 `ai.flyersOnly`（只打空中的）：开启＝只对离地/飞行/浮空的目标投石（省 PP，专做防空）；
 *   关闭＝也当普通远程石击用。两向各有适用局面。参数 `pull`／`pinTicks` 在实际受击者上下文求值。
 *
...
namespace PokemonSkills {
    actionParameters.define("smackdown", {
        ...
        /** 下坠速度：0.8 + 物攻偏移[−0.1,0.5]；夹 0.45..1.4 格/刻。 */
        pull: formula(
            F.base(0.8).plus(F.stat("attack").minus(60).times(0.006).clamp(-0.1, 0.5)).clamp(0.45, 1.4).round(2),
            "下坠速度", {
                unit: "格/刻",
                description: "每刻把空中的目标竖直速度补到这么快，水平运动照旧；抗推/无敌的对手可能推不动。它也是目标坠地那一段的画面速度。"
            }),
