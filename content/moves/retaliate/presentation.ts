 ...
 * 报仇 / retaliate 的客户端表现。
 *
 * 一句话：施法者低头一瞬、身上压出灰白的哀气 → 带着这股劲直直撞向目标 → 撞上处炸开一圈惨白的光与冲击；
 *   带着哀兵的那一撞更亮更重。
 * 色相家族：灰白与银（0xD9D2C4 / 0xF2EEE4）为主体，惨白高光只给「哀兵」那一撞；不引入第二个色相。
 * 拍子：起 mourn（压低哀气）→ 撞 charge（拖速度线冲锋）→ 击 strike（冲击炸开）→ 空 miss。
 * 范围：strike 的环与外爆按 `data.scale`（判定半径 / 0.45）放大，撞到哪块区域一眼可读。
 * 运动：速度线沿身体轨迹拖在身后，冲击在命中点向外炸开。
 * 数：strike 的冲击点数绑定 `data.streaks`（物攻与等级换算），强度绑定命中威力，`data.avenge` 决定是否叠出惨白高光。
 */
            ...
            emitters: [
                {
                    ...
                    trail: { minDistance: 0.4 }, rate: 26, shape: { kind: "sphere", radius: 0.3 },
