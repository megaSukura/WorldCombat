 ...
 *   朝当刻的瞄准方向拨响三下（两下之间可转向），每一下都从当下位置沿一条走廊推出一道带电的声浪：
 *   走廊里的每个敌人各挨一次 `thrum` 并被打得后退半步；电声每一下都可能把人震到**麻痹**（Electre 属性免疫，与原作一致）。
 *   开启余响时会多出一记迟到的**余响**——那是原生的「巨大回声」，它不追人，而是在第三拨的原位置沿同一条
 *   走廊重放一次（更重，也多一次麻痹机会），代价是整段演出更长、冷却更久。
 *   与同族分开：
