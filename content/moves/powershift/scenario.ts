 ...
 *
 * 场面：一只只会「力量转换」的腕力（攻高防低，差距明显）与一只弱小的小拉达隔开 9 格、石质场地上开战；
 *   技能表里只有这一招，所以 AI 只能先转换。有威胁、差距够大且没贴身时它会换一轮。
 * 必然事实：本招被提交过；施术者身上出现过共享身份 world_combat:status/powershift 的交换窗口。
 *   换前换后的数值、差距比、窗口多长写进 note 供读轨迹判断（smoke 不能直接读宝可梦的原生攻防数值）。
 */
Smoke.scenario("powershift", function (stage) {
    stage.fill([-8, -1, -8], [8, -1, 8], "minecraft:stone");
    stage.time("day");
    stage.weather("clear");
    var caster = stage.pokemon({ species: "machop", level: 34, moves: ["powershift"], at: [-3, 0, 0] });
