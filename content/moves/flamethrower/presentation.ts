 ...
 *
 * 一句话：喉间先攒起一点白热的核心，随后一道火舌从身前喷出、一截一截向远处铺开，沿一条走廊（或一个
 * 扇面）燎过；被燎到的人身上炸开一圈火，喷完只剩余焰与烟慢慢散。
 * 色相家族：火苗橙（0xFF7A2E）与火心白黄（0xFFE2A0）为主体，深褐烟（0x3A2E2A）衬托。
 * 拍子：起 charge（攒气）→ 喷 jet／jetwide（火舌推进）→ 击 hit（燎到目标）→ 收 fade（余焰散去）。
 * 范围：jet／jetwide 沿服务端传的 `data.path`（与判定同一份走廊／扇面顶点）铺设，火墙画到哪里就烧到哪里。
 * 运动：粒子沿路径向前、逐段推进；火舌每刻由服务端的 `data.length` 决定长度。
            ...
            emitters: [
                {
                    ...
                    rate: { data: "density", fallback: 70 }, shape: { kind: "polyline" },
                ...
                },
                {
                    ...
                    particle: "world_combat_core:cobblemon/generic/fire/cloudyfire_white",
                    rate: { data: "density", fallback: 30 }, shape: { kind: "polyline" },
                ...
                },
                {
                    ...
                    particle: "world_combat_core:cobblemon/generic/fire/ember",
                    rate: { data: "density", fallback: 24 }, shape: { kind: "polyline" },
