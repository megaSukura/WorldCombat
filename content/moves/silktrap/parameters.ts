 ...
 *
 * 翻译：把“丝 + 陷阱”翻成一张**贴地绷起的丝网**——它像守住一样按总量挡下伤害招式，而每一次直接的接触
 * 让攻击者的脚被丝缠住：速度被压下去（共享能力等级），并当场被钉住一小段（世界已有的 world_combat:rooted）。
 * 变化招式不进 pool 的拦截，所以**会照常落在身上**，这是它与尖刺防守／碉堡的取舍。
 *
 * 与家族分开：同族里尖刺防守当场掉血、王者盾牌削攻击、碉堡灌毒，只有线阱**同时黏住脚**——
 * 它是一族里唯一会把接触者按在原地的，兑现“陷阱”这个名字。
 *
 * 数值分散到多类精灵数据（每项读不同的量）：
 *   raise      绷网快慢：体重。
 ...
 *   bind       缠足时间：速度 + 等级（腿快的个体丝绷得更紧、缠得更久）、配置取向。
 *   radius     丝网半径：碰撞箱高度。
 *   charge     冷却：等级。
 *   fizzle     连用失误率：本招自己的连用计数 + 等级。
 *
 * 配置 `snare`（缠缚／滑丝）双向取舍：开＝缠足 ×1.5、降速 +1 级，但总量 ×0.8、持续 ×0.85；
 * 关＝总量 ×1.25、持续 ×1.15，但缠足 ×0.6、降速不额外加。两向各有局面（钉住近战 vs 硬挡齐射）。
 */
namespace PokemonSkills {
    actionParameters.define("silktrap", {
        ...
        drop: formula(
            ...
            "接触降速", {
                unit: "级",
                description: "接触攻击撞上丝网时，攻击者下降的速度等级；等级越高、缠缚取向降得越多。"
            }),
        /** 缠足时间：基础 10 刻 + 速度 ×0.06 + 等级 ×0.1；缠缚 ×1.5、滑丝 ×0.6，夹在 6..40。 */
        bind: formula(
            F.base(10)
                .plus(F.stat("speed").times(0.06))
                .plus(F.level().times(0.1))
                .times(F.when(F.pref("snare"), F.const(1.5), F.const(0.6)))
                .clamp(6, 40).round(0),
            "缠足时间", {
                unit: "刻",
                description: "接触者被丝缠住脚、当场不能移动的时间；腿快、等级高的个体缠得越久，缠缚取向更甚。"
            }),
