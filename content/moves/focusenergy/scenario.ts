 ...
 *
 * 场面：一只会「聚气」与「撞击」的腕力（Machop，30 级）与一只弱小的小拉达在 6 格内、石质场地上开战；
 *   有威胁且在考虑距离内、还没贴身时，它会先聚气，再回到普通交战、用撞击兑现这口越来越深的气。
...
Smoke.scenario("focusenergy", function (stage) {
    ...
    stage.time("day");
    stage.weather("clear");
    var caster = stage.pokemon({ species: "machop", level: 30, moves: ["focusenergy", "tackle"], at: [-2, 0, 0] });
    var foe = stage.pokemon({ species: "rattata", level: 12, moves: ["tackle"], at: [4, 0, 0] });
    ...
    stage.until(1200, function () {
        ...
        stage.after(220, function () {
            stage.note("吐纳窗口走完即散，命中不消耗它：带身份者每次伤害结算按「已深化比例」抬升附加要害概率（满气附加比例 edge 由速度与特攻派生），落地伤害还会沿用一缕轻量气息。是否真的打出要害是随机结果，读轨迹与完整装配试玩核对；深呼吸与浅呼吸各有代价。", {
