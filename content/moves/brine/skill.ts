 ...
 * 盐水 / brine —— 注册与动作。
 *
 * 核心念头：把盐水压成一条锁人的细流射向对手；打在完好的身上只是冲一记，打在半血以下的伤口上，
 *   盐钻进伤口，威力翻倍、目标被浇得湿透，画面也随伤势更亮更狠。这一招只做「残血补刀」：
 *   一束盐卤、一次结算、命中即止，没有落地盐池、没有减速状态。
 *
 * 三幕：
 *   起（charge，提交前）：盐水在口边／掌中压成细线，白汽与水滴聚拢，只播预告。
 *   射（flight）：细流拖着水尾沿瞄准方向笔直飞出（原生投射物外观）。
 *   溅（burst / sting / miss）：命中活物时按该目标自己的血量结算 jet（残血 ×2）并浇上湿透身份；
 *       命中前它已在半血以下时，命中处炸出明显的盐晶裂口；打中墙或射空只散开水花。
 *
 * 与同族分开：热水是抛出的沸水并留下烫池；喷水是一圈推人的潮墙；盐水是一条锁人的细流，
 *   只对**已经残血**的目标翻倍，不制造残血、不留区域。
 */
namespace PokemonSkills {
    define({
        ...
        name: "盐水",
        description: "朝对手压出一束盐卤：目标血量在一半或以下时这一击威力翻倍，命中即把它浇得湿透。只有一条直射的细流，不留盐池、不额外减速，专门补掉已经残血的对手。",
        uses: ["对着已经残血的对手补上翻倍的一记", "在敌人退到半血以下时追加压力", "从远处直射，只求收掉一个残血目标"],
        kind: "aim",
        range: 10,
        maxRange: 18,
        prepare: 8,
        ...
        windup: function (action, config, prepare) {
            ...
            action.present("brine:charge", brineScene, 1, action.origin(),
                JSON.stringify({ moment: "charge", reach: reach, nozzle: radius, scale: brineScale(radius),
                    press: config && config.press === true ? 1 : 0 }));
            return prepare;
        },
