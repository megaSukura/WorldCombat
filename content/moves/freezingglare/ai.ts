 ...
 * 对谁出手：默认（ai.preferImmune 开）优先挑冰属性目标——这一记能穿透冰冻免疫，正是对付它们的手段；
 *   默认（ai.preferChains 开）也优先挑身边挨着别人的目标，好让念力线跳过去。
...
namespace CompanionBehavior {
    PokemonSkills.addPreferences("freezingglare", {}, [
        ...
        PokemonSkills.flag("ai.preferImmune", "优先冰冻免疫目标")
    ]);

    /** 目标是不是冰属性（本来免疫冰冻，正是这一招要点掉的对象）。 */
    function freezingglareIceType(context: WorldBehavior.Context, target: Entity): boolean {
    ...
    registerUse("freezingglare", {
        ...
        priority: function (context, capability, target) {
            ...
            if (ai<boolean>(capability, "preferImmune", true) && freezingglareIceType(context, target)) score += 16;
