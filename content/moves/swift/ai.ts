 ...
 * `ai.spread`（默认开）让它数一数目标身边的敌人：有第二个敌人在场时抬高 priority，因为散星能同时咬住两个。
 * 关掉后只按普通远程攻击使用。
 */
namespace PokemonSkills {
    CompanionBehavior.registerUse("swift", {
        ...
        priority: function (context, capability, target) {
            ...
            for (var i = 0; i < nearby.length; i++) {
                ...
                if (CompanionBehavior.distance(other.point, target.point) <= 5) return 40;
            }
            return base;
        }
    });
    ...
    addPreferences("swift", {}, [
        field(pathOf("scatter"), "散星", "boolean", {
            help: "开启：星分散到射程内每个对手，覆盖更广，但选定目标吃到的星更少、起手多 2 刻、冷却多 3 刻。关闭：全部星集中打选定目标，单体更重、更快。"
        }),
        ...
        field(pathOf("ai.spread"), "散星取材", "boolean", {
            help: "开启后，目标身边还有别的敌人时优先发射，让散星同时咬住两个；关闭则只按普通远程攻击排序。"
        }),
