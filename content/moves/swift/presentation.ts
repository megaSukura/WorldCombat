 ...
 * 拍子：起（charge 聚星）→ 击（launch 迸射、seek 追星、hit 命中）→ 收（fade 散尽）。
 * 范围：seek 的每颗星沿 projectile 锚点飞行、命中点由 hit 的落点决定；玩家沿着星的飞行弧看清它够到哪。
 * 运动：星从一圈迸出后逐刻转向目标，沿途拖出星屑尾迹。
 * 数：`data.count`（星数）绑定 launch 的迸射数量，`data.trail`（单星威力换算的尾迹量）绑定 seek 的发射率，
 * `data.notes`（威力换算的碎星数）绑定 hit 的爆开数量，`data.intensity` 抬高亮度，`data.scale` 缩放命中环。
