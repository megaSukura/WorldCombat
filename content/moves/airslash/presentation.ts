 ...
 * 拍子：起（gather 拢风）→ 斩（flight 月牙掠出、拖碎风）→ 中（cut 命中爆风屑）→ 懵（flinch）／空（miss）。
 * 范围：这一招只作用在刃线扫过的一条直线上；flight 的各层绑 `projectile` 锚点沿 projectile 轨迹铺开，画面即那条切割线。
 * 运动：gather 的风线向内收成一条；flight 的月牙沿 projectile 高速直行、碎风留在身后；cut 时风屑向外炸。
 * 数：`data.shards`（特攻与等级派生）决定碎风与命中风屑量，`data.hits` 让第几段切割可读，`data.intensity` 抬高命中亮度。
        ...
        flight: {
            ...
            emitters: [
                ...
                },
                {
                    ...
                    particle: "world_combat_core:cobblemon/vanilla/small_gust",
                    rate: 40, shape: { kind: "sphere", radius: 0.12 }, direction: "velocity", speed: [0.02, 0.12], spread: 10,
