 ...
 *
 * 翻译：把「点燃小石子，形成火球攻击对手」落成一记**从脚下踢出去的火球**——施法者先在小石球滚到
 * 脚前后把它点燃，再像抽射一样踢出；火球带着焰尾沿一道低弧飞出，命中炸开成一团火与碎石，并可能把
 * 目标引燃。原生 90 命中在这里是**这一脚踢得正不正**：火球出膛时带一点散布，速度快的个体踢得更直，
 * 蛮踢式则更野——玩家能从火球偏没偏读出这一脚。落点的焦痕只是短装饰，不代表地面持续燃烧。
 *
 * 数据分散（每项读不同的精灵数据）：
 ...
 *   heat       焦痕半径：物攻决定落点染出多大一片；表现按它画。
 *   scorchTicks 焦痕时长：等级决定落点短暂痕迹留多久。
 *   起手／收招／冷却：速度决定。
 *
...
namespace PokemonSkills {
    actionParameters.define("pyroball", {
        ...
        heat: formula(
            ...
            "焦痕半径", {
                unit: "格",
                description: "火球落地染出的焦痕大小；物攻越高、蛮踢式越大。表现里的焦痕按它铺开，它不造成伤害。"
            }),
        scorchTicks: seconds(
            F.base(50).plus(F.level().minus(30).times(0.6).clamp(0, 40)).clamp(40, 100).round(0),
            "焦痕时长", "落点那圈焦痕短暂停留多久；等级越高留得略久，到期自然散去。它只是装饰，不代表持续灼烧的地面。"),
        tempo: seconds(
