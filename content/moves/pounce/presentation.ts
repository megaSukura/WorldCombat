 ...
 * 范围：launch 的 `data.path` 只是这一扑会经过的弧线预告，不是身体本身；leap 的拖尾由服务端在身体逐刻实际经过时
 *   发出，接触一点后服务端停止该相，拖尾随即收束。玩家读得出这一扑落在哪、有没有真的碰到人。
 * 运动：身体沿服务端逐刻算出的弧线增量真实移动；虫翼与尘点沿身体当前所在处向后抛；命中后足印贴着目标腿边环绕。
