 ...
 *
 * 一句话：施法者屈膝起势、脚下先聚起粉色花点；随后一圈圈花瓣与叶片从身体向外卷成风暴，贴着地面转到风暴半径；
 *   被扫到的人身上再炸开一撮粉色花点，落脚处有花瓣扑簌簌地铺开；舞完头顶转起眩晕的气流。
 * 色相家族：粉色 0xE58FB0 与近白 0xFFF0F5 为主，叶片绿 0x7FB56A 只做外圈与余韵；花与叶是一组自然的双色。
 * 层次：聚拢起势（tempo）→ 花瓣风暴（bloom）→ 命中花点（lash）→ 落地花瓣（residue）→ 收束眩晕（spent）→ 持续眩晕（dizzy）。
 * 范围：bloom 的 `storm_ring` 用 `ring` 贴地画出一整圈，基准半径 5.0 格，服务端按实际风暴半径传 `data.scale`，
 *   画出的圈就是花瓣卷到、也是判定扫到的范围。
 * 运动：花瓣沿环向外卷出并带旋转；命中处向外炸开；落地花瓣向上翻起再落下。
