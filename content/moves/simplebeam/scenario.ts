...
Smoke.scenario("simplebeam", function (stage) {
    ...
    stage.until(420, function () {
        ...
        stage.after(90, function () {
            stage.note("单纯光束把目标当前生效的特性改写成 simple（共享 NativeModifiers ability 层，到期自动还原原生特性），并挂上共享身份的标记。只对宝可梦、且特性可被顶替（不是 simple／truant、不带 cantsuppress）的目标改得动（预检直接拒绝）。特性是否真的变成 simple、维持多久由共享结算决定，留给完整装配的人工试玩；Simple 本身的加倍效果属于特性内容线，不在本招范围内。维持时长随等级与特攻、光环数随特攻、光束粗细随特攻、扩散半径随体型分别变化。", {
