 ...
 *
 * 一句话：刃尖在身前拉出一条极细的白缝，划过目标的一瞬只闪一下近白的收手标记——血留住，人没倒。
 * 色相家族：近白与冷银（softswipe／cut／impact_normal／smallsparkle）＋中性尘（tinydust）。
 * 拍子：起（windup 聚刃）→ 切（cut 细缝、strike 命中）→ 留手（spare 收手标记）→ 空挥（miss）。
 * 范围：cut 用 `data.path`（与服务端 WorldGeometry.lane 同一组四角）铺成窄缝；缝多长多宽，画面就是那块。
 * 运动：粒子从刃根沿缝扫向远端，命中处向外炸出细碎白光，收手标记在原地停一瞬再淡去。
...
const FalseswipeDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        cut: {
            ...
            emitters: [
                {
                    name: "seam_fill", bind: "path", offset: [0, 0.55, 0],
                    particle: "world_combat_core:cobblemon/generic/softswipe",
                    shape: { kind: "polygon" }, rate: { data: "hold", fallback: 12 }, direction: "shape", speed: [0.02, 0.1],
                    lifetime: [6, 12], size: [0.24, 0.05],
                    color: 0xEAF0F8, alpha: [0.3, 0], light: "full", maxParticles: 90
                },
                {
                    name: "seam_edge", bind: "path", offset: [0, 0.6, 0],
