 ...
 * 拍子：起（brace 架式）→ 行（drive 推进）→ 击（impact 顶实）→ 收（grind 碾推 / settle 站定 / miss 推空）。
 * 范围：impact 绑命中点，grind 绑被顶住的目标、随它一起被推走，画出的就是被顶开的那段路。
 * 运动：drive 的尘土沿历史拖尾，impact 是贴地外扩的钝击环，grind 是目标脚边持续被犁开的土。
 * 数：`data.clods`（顶推距离 × 8，取整）决定架上与站定时掀起的土块数，`data.scale` 放大范围，
 * `data.intensity`（本击威力 / 100）抬高撞击的密度与亮度，`data.ratio` 只是顶推进度。
 * 参照节：视觉语言第二、三、四、七、九节。
 */
