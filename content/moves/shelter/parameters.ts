 ...
 *   plates   壳板数：基础 6 + 防御×0.006 + 等级×0.02；夹 6..8。壳板是固定可数的一层（6–8 块），每块代表初始承伤额度的一份。
...
namespace PokemonSkills {
    actionParameters.define("shelter", {
        ...
        /** 壳板数：固定可数的 6–8 块，每块代表初始承伤额度的一份。 */
        plates: formula(
            F.base(6).plus(F.stat("defence").times(0.006)).plus(F.level().times(0.02)).clamp(6, 8).round(0),
            "壳板数", {
                unit: " 块",
                description: "拼成这层壳的固定壳板数（6–8 块）；防御与等级越高板越多。每块代表初始承伤额度的一份，被打掉一份就熄灭一块。"
            }),
        /** 壳半径：体型越宽包得越开。 */
        shell: formula(
