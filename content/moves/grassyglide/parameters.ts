 ...
 *   **本招不自己种草**：落点只扬一撮短草做落痕，青草场地仍由环境（青草场地招式、青草制造者特性）提供。
 *
 * 与场上最像的招分开：电光一闪是一道贴地的直线影子，不留东西；水流喷射是水柱、只浇透。
 *   青草滑梯的身份是**草浪与条件先制**：滑过处翻起草叶，站在真青草上才瞬发。
 *
 * 数据分散（每个参数各吃不同的精灵数据，落到不同参数上）：
 *   slide           滑撞威力：物攻给份量、速度给冲势、等级给成长。
 *   dash            滑行距离：速度与等级决定滑多远，也是射程来源；在青草场地上 ×1.3。
 *   pace            每刻位移：速度决定滑得多急；青草场地上再快一成半。
 *   collisionRadius 判定半径：身高决定身体多宽。
 *   push            顶开距离：体重决定把目标铲开多远。
 *   tufts           草叶数量：速度与等级驱动，表现按它发射。
 *   tempo/settle/recharge 速度决定节奏；青草场地让起手归零。
 *
...
namespace PokemonSkills {
    ...
    actionParameters.define(grassyglideId, {
        /** 滑撞威力：55 +（物攻 − 55）× 0.22 [−11,28] +（速度 − 55）× 0.18 [−5,18] +（等级 − 20）× 0.3 [0,9]；夹 34..120。 */
        slide: formula(
            F.base(55)
            ...
            "滑撞威力", {
                unit: "威力",
                description: "贴地滑过去撞实的那一下；物攻给份量、速度给冲势、等级给成长。对手防御、相性与暴击在命中时另算。"
            }),
        /** 滑行距离：4.0 +（速度 − 55）× 0.02 [−0.5,1.4] +（等级 − 20）× 0.02 [0,0.8]；草地 ×1.3；夹 3.2..6.8。 */
        dash: formula(
            F.base(4.0)
            ...
            "滑行距离", {
                unit: "格",
                description: "从起步到撞上最多滑多远，也是本招的实际射程来源；腿快的个体滑得更远，脚下有草时再远三成。"
            }),
        ...
        tufts: formula(
            ...
            "草叶数量", {
                unit: "点",
                description: "滑行翻起与命中扬开的草叶数量，也直接驱动画面的发射量；速度与等级越高翻得越密。"
            }),
