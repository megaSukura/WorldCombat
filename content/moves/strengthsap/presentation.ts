 ...
 *
 * 一句话：施法者脚边聚起一圈根须，猛地搭上贴身对手；一条绿光把对手与施法者连住，把对手身上的力气
 *   一股股扯回施法者身体，施法者身上亮起被喂饱的绿光，对手身上则散出被抽空的苍白丝。
 * 色相家族：草绿（0x8FC63F／0x3E7A1F）与嫩绿（0xC7E86A）撑起根须与回流，近白（0xF2FBE0）只给被抽走的力气与命中核心。
 * 拍子：起（windup 聚根）→ 缠（latch 根须搭上、连线成形）→ 回（drain 力气回流到施法者）→ 虚（weaken 对手被抽空）。
 * 范围：latch／weaken 绑在目标身上，按体型适配；连线沿 `data.path` 把目标与施法者连成实线，读到的是这一次抽取的两端。
 * 运动：根须自施法者脚下向目标缠上；回流沿 `data.direction` 从目标流向施法者；被抽走的丝从目标身上向外散后上浮。
 * 数：力量丝数量由 `data.motes`（特攻派生）驱动；回流强度由 `data.healed`（实际回血）体现在施法者一侧的亮度。
        ...
        latch: {
            ...
            emitters: [
                ...
                },
                {
                    ...
                    color: 0x8FC63F, alpha: [0.7, 0], light: "full", maxParticles: 70
                },
                {
                    name: "knot", bind: "target", height: 0.5,
                    particle: "world_combat_core:cobblemon/generic/impact/impact_grass",
                    burst: { count: 2 }, shape: { kind: "sphere_surface", radius: 0.3 },
        ...
        drain: {
            ...
            emitters: [
                ...
                },
                {
                    ...
                    color: 0xC7E86A, alpha: [0.85, 0], light: "full", bloom: 0.35, maxParticles: 26
                }
            ]
        },
        weaken: {
            ...
            emitters: [
                {
                    ...
                    burst: { count: { data: "motes", fallback: 12 } }, shape: { kind: "sphere", radius: 0.36 },
