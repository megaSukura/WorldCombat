 ...
 *
 * 翻译：保留「向对手喷射水炮」，把原生的单体命中翻成即时战斗里**一发压缩的高压水炮**：
 * 水炮以很快的速度直线射向目标，只打第一处接触、撞墙即在墙面散水；命中时把目标沿水炮方向顶开、并按等级泼上
 * 共享身份 `world_combat:status/soaked`（本单元效果）——被浸湿的活体再吃一发水炮会多受一份 `drenchBonus`，
 * 所以连续两发会越打越重。选取为 `kind: "aim"`：朝方向或点都能放，空放也照常力竭。
