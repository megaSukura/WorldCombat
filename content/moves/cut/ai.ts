 ...
 * 它是一趟贴地的宽弧，所以「值不值得挥」取决于身前挤着几个敌人：`ai.cluster`（默认开）在射程内可及的非友方
 * 达到两个以上时抬高优先级，把这一记当成一次清场；关掉后只把它当最便宜的单体平A排序。
 * 放完之后：弧内的敌人都挂了彩、草也被割走，交回共享顺序决定继续贴上去还是走位。
 */
namespace PokemonSkills {
    function cutCrowd(item: WorldBehavior.Capability, context: WorldBehavior.Context, self: CompanionBehavior.Entity): number {
    ...
    CompanionBehavior.registerUse(cutId, {
        ...
        priority: function (context, capability, target) {
            ...
            const crowd = cutCrowd(capability, context, self);
