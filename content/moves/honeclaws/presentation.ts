 ...
 * 磨爪 / honeclaws 的客户端表现。
 *
 * 一句话：爪边先聚起一圈冷光 → 施术者原地交叠着刮几下，两侧爪前的短线一起汇入身体、迸出几点暖火星 →
 * 锋口定住的一刻，爪上亮起一小簇金点；此后锋口还在的时间里，爪尖只留极小的冷点轻轻闪。
 ...
 * 运动：draw 冷光向内聚拢；hone 两侧爪前短线向身体汇入、火星带一点重力下落、金点向外一亮；hum 冷点极慢上浮；fade 火星向下沉。
 * 数：两侧短线与火星的数量绑 `data.scrapes`（物攻与速度派生），强调的金点数与亮度绑 `data.gain`／`data.shine`
 *   （本次实际抬起的级数派生）；刮得越密、抬得越多，画面里的粒子越多；上限未增时 `shine` 为 0，不播完整升级。
 * 持续：hum 绑在真正的锋口窗口上（服务端 `WorldFeedback.onEffect`），窗口关闭会同步收回。
 * 参照节：视觉语言第二、三、四、七、九节。
 */
const HoneClawsDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        hone: {
            ...
            emitters: [
                {
                    name: "claw_left", bind: "source", offset: [-0.32, 0.5, 0], height: 0.35,
                    particle: "world_combat_core:cobblemon/generic/scratch",
                    burst: { count: { data: "scrapes", fallback: 14 }, at: 1 },
                    shape: { kind: "point" },
                    direction: "inward", speed: [0.16, 0.36], spin: 10,
                    lifetime: [6, 11], size: [0.28, 0.05], sizeMode: "index",
                    color: 0xE8F2FF, alpha: [0.9, 0], light: "full", bloom: 0.2, maxParticles: 60
                },
                {
                    name: "claw_right", bind: "source", offset: [0.32, 0.5, 0], height: 0.35,
                    particle: "world_combat_core:cobblemon/generic/scratch",
                    burst: { count: { data: "scrapes", fallback: 14 }, at: 1 },
                    shape: { kind: "point" },
                    direction: "inward", speed: [0.16, 0.36], spin: -10,
                ...
                },
                {
                    ...
                    burst: { count: { data: "shine", fallback: 0 } },
        ...
        hum: {
            ...
            emitters: [
                {
                    name: "claw_left_tip", bind: "source", offset: [-0.28, 0.42, 0], height: 0.3,
                    particle: "world_combat_core:cobblemon/generic/sparkle/smallsparkle",
                    rate: 1.5, shape: { kind: "point" },
                    direction: "up", speed: [0.003, 0.011],
                    lifetime: [14, 24], size: [0.04, 0.01], sizeMode: "sin",
                    color: 0xD8E4F0, alpha: [0.26, 0], alphaMode: "sin", light: "world", maxParticles: 10
                },
                {
                    name: "claw_right_tip", bind: "source", offset: [0.28, 0.42, 0], height: 0.3,
                    ...
                    color: 0xF0F6FF, alpha: [0.28, 0], alphaMode: "sin", light: "full", maxParticles: 10
                }
            ]
        },
        fade: {
