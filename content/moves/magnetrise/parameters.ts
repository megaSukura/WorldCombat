 ...
 * 核心念头：把脚下那块地磁化，用磁场真正把身体托离脚下地表、沿地形低空滑过。身体一离地，地面属性招式
 *   够不到它，贴地的近战敌人撞上来会被同极弹开一步；磁力耗尽时身体失托、交回原生重力。
 * 世界化：命中挂共享身份 world_combat:status/magnetrise 的真实 MobEffect（物品栏可见、/effect 可用），
 *   由本单元的入场伤害规则负责地面招免疫；真实低空托举与弹开由本招自己的 mark 承担。
 ...
 *   liftHeight  基础 0.35 格 + 身高 ×0.15，夹 0.25..0.8；身体真正离地的高度（groundLift 目标离地）。
 ...
 * 内部几何常量（不向玩家展示）：托举响应 response 与额外下探 probe 交给原生 groundLift 每刻维持离地。
 */
namespace PokemonSkills {
    ...
    actionParameters.define(magnetriseId, {
        ...
        liftHeight: formula(
            ...
            "悬浮高度", { unit: " 格", description: "身体真正离地的高度；身板越高抬得越高，原生托举照它维持。" }),
        sparks: formula(
