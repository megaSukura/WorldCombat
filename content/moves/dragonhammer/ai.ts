 ...
 *   `ai.opening` 选「只对没被砸趴的目标」时跳过已经趴着的敌人，把这一锤留给还站着的目标。
 * 够不到交给共享接近逻辑；走进抡击距离就砸下去。
 */
namespace CompanionBehavior {
    ...
    PokemonSkills.addPreferences("dragonhammer", {}, [
        PokemonSkills.field(PokemonSkills.pathOf("heavy"), "重锤式", "boolean", {
            help: "开启：威力约 ×1.15、砸趴约 ×1.3、击飞收到约 ×0.8、起手 +3 刻、冷却 +8 刻，砸得更重更久、把人钉在原地。关闭（疾锤式）：击飞约 ×1.25、射程约 ×1.1、起手更快，代价是威力约 ×0.9、砸趴更短。"
        }),
        ...
        PokemonSkills.field(PokemonSkills.pathOf("ai.opening"), "出手时机", "choice", {
            ...
            help: "选「只对未被砸趴的目标」时跳过已经趴着的敌人，把这一锤留给还站着的目标。"
        })
    ]);
}
