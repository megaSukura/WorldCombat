 ...
 *   再整副砸下；砸实的一下把目标砸得趴伏一阵，走不动几步，自己也被这一下的反冲顶得顿一顿。
 *   没有反震、不裂地，代价是起手慢、只砸一个点。
 * 与同为「身体当武器」的招分开：
 *   泰山压顶 —— 跃起到落点、范围压中一圈、概率麻痹；
 *   木槌     —— 抬身砸下、裂开地表、反震自己、压速度；
 *   龙锤     —— 自上而下砸单个目标、不裂地不反震，把目标**砸趴**一小段（速度大幅下降）。
 *
 * 数值来源（每项依赖不同的精灵数据，分散到不同参数上）：
 *   hammer  锤击威力 76 + 物攻偏移 + 体重偏移 + 等级偏移（身体越沉、物攻越高砸得越狠）。
 *   reach   抡击距离 2.7 格 + 身高偏移 + 速度偏移。
 *   lunge   扑身距离 0.9 + 速度偏移（快的人补得上一步）。
 *   shove   击飞距离 0.7 + 体重偏移 + 物攻偏移；重锤式收得更短（压在原地）。
 *   downTicks 砸趴时长 26 刻 + 自身体重偏移 − 目标质量偏移（自己越沉砸得越久，目标越重越难被砸趴）。
 *   dust    龙气碎屑数量 20 + 体重偏移 + 物攻偏移（同时驱动画面密度）。
 *   tempo/aftercast/recharge  速度决定起手／收招／冷却；重锤式更慢。
 *
 * 配置 heavy（重锤式）双向取舍：开＝威力 ×1.15、砸趴 ×1.3、击飞 ×0.8、起手 +3、冷却 +8（砸得更重更久、
 *   把人钉在原地）；关（疾锤式）＝击飞 ×1.25、射程 ×1.1、起手更快，代价是威力 ×0.9、砸趴 ×0.85。
 *
...
namespace PokemonSkills {
    ...
    actionParameters.define("dragonhammer", {
        ...
        lunge: formula(
            ...
            "扑身距离", {
                unit: "格",
                description: "砸下去之前朝目标补上的那一步；速度快的个体补得更远。"
            }),
        ...
        /** 砸趴时长：26 + 自身体重偏移[−4,30] − 目标质量偏移[−3,9]；重锤 ×1.3 / 疾锤 ×0.85；夹 12..70。 */
        downTicks: seconds(
            F.base(26)
                ...
                .clamp(12, 70).round(0),
            "砸趴时长", "被砸中后带着 world_combat:status/knocked_down 走不动几步的时间；**自己越沉砸得越久，目标越重越难被砸趴**，重锤式更久。"),
