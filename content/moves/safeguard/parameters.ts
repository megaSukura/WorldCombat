 ...
 * 世界化：施法者挂一枚来源标记 world_combat:safeguard_mark，每 20 刻按来源租约把共享身份
 *   world_combat:status/safeguard 的真实 MobEffect（物品栏可见、/effect 可用）补给自己与半径内的友方；
 *   同一位受护者身上多个来源各记一份独立贡献。带该身份的活体上，任何经共享状态路由
 *   （CombatStatus.inflict／招式次要状态）落下的异常都会被本单元的 gate 拒绝，并当场播「守护弹开」。
 *   守护跟着施法者走；离开半径的人按这一份贡献的剩余时间短暂保留，来源结束只撤自己名下的贡献。
 *
 * 数值来源（每个参数读不同的个体数据）：
