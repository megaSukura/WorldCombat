 ...
 * （默认 11）以内，且身上还没有被夹住（`partiallytrapped`）。`ai.preferMovers` 打开时只对正在移动或逃跑的
 * 威胁埋夹（用 `ai.lead` 提前量埋在前面），对站着不动的目标留给普通攻击。
 * 对谁出手：当前威胁；冲过来、正在逃跑的优先，焦点目标另加一档。
...
namespace PokemonSkills {
    ...
    function snaptrapWants(context: WorldBehavior.Context, item: WorldBehavior.Capability, target: CompanionBehavior.Entity): boolean {
        ...
        if (CompanionBehavior.ai<boolean>(item, "preferMovers", false)
            && !snaptrapMoving(target) && !CompanionBehavior.fleeing(context, target)) return false;
    ...
    CompanionBehavior.registerUse("snaptrap", {
        ...
        priority: function (context, capability, target) {
            ...
            let base = 20;
            if (snaptrapMoving(target)) base += 14;
            if (CompanionBehavior.fleeing(context, target)) base += 22;
    ...
    addPreferences("snaptrap", {}, [
        ...
        field(pathOf("ai.preferMovers"), "只埋移动目标", "boolean", {
            help: "开启：只对正在移动或逃跑的威胁埋夹，站着不动的目标留给普通攻击；关闭：对任何范围内的威胁都愿意埋。"
        })
    ]);
}
