 ...
 * 扎根 / ingrain 的客户端表现。
 *
 * 一句话：施法者脚下一沉，尘土先被压开，一丛根须从脚边钻进地里、把地面顶成深褐色；此后每隔一会儿，
 * 绿色的生机从根须间贴着地面升起、汇进身体，脚边泛起一圈浅浅的土环；拔根时根须缩回、碎土落下。
 * 色相家族：草绿 0x6FA83A 为主体，亮绿 0x9CD44A 作涌起的生机，土褐 0x6B4A2B 作地面与根须细节。
 * 拍子：下沉（sink 0–10t）→ 扎定（root 0–30t）→ 抽养（pulse 0–22t）→ 根须（roots，持续低密度）→ 拔根（fade 0–24t）。
 * 范围：地面根环绑脚点、fit none，半径按 `data.scale`（实际根域半径 / 1.2）推出；判定与表现同径。
 * 运动：尘土被压开 → 根须向地下钻 → 绿光沿根须向上汇集 → 拔根时根须缩回、土屑落下。
