 ...
 * 拍子：起（charge 聚电）→ 掷（cast 电矢）→ 驻（enclose 立笼 / cage 电栅 / arc 越界 / zap 笼内电击）→ 收（release）。
 * 范围：enclose 与 cage 都是 `bind: "point"`、`fit: "none"`，用 `data.radius` 画电栅半径——画出来的圈就是栏杆的位置。
 * 运动：电栅沿局部 +Y 竖直竖起并自转，粒子在笼壁上下窜动；越界时电弧从栏杆朝目标炸出。
