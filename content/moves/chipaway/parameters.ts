...
namespace PokemonSkills {
    ...
    actionParameters.define(chipawayId, {
        ...
        strike: formula(
            ...
            "每拍威力", {
                unit: "威力",
                description: "每一拍真正命中时的基础威力；物攻定拳劲、等级给稳。抢攻式每拍更轻、稳步式每拍更重。对手防御、相性与暴击在命中时另算，**目标本段的防御能力等级被这一招忽略，但装备护甲仍照常减伤**。"
            }),
        ...
        /** 拳锋半径：0.32 + 体宽偏移[−0.04,0.16] ×0.12；夹 0.26..0.55。 */
        half: formula(
            F.base(0.32).plus(F.body("width").minus(0.9).times(0.12).clamp(-0.04, 0.16)).clamp(0.26, 0.55).round(2),
            "拳锋半径", {
                unit: "格",
                description: "每一拍拳路的接触半径；身板越宽拳面越开。首个碰到它的人或方块就是这一拍打到的地方。"
            }),
