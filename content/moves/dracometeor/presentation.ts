 ...
 *
 * 一句话：施法者抬头，落点先亮起一圈标记、天上聚起一点将坠的光 → 陨石拖着紫绿的龙焰尾迹垂直砸下 →
 *   每颗落点炸开一圈龙属性能量、把地面砸出焦黑的坑。
 * 色相家族：龙焰的紫绿（0x9A8AF0 / 0x6FE0C8）为主体，燃石的橙（0xD98A4A）与深褐烟（0x2E2824）衬托。
 * 拍子：起 summon（聚落点）→ 标 mark（地面圈）→ 落 fall（下坠尾迹）→ 击 impact（命中）与 burst（坑与余烬）。
 * 范围：mark 用 `data.radius`（流星式是 spread、坠星式是落点半径）画出会被砸到的地；burst 用 `data.impactRadius`。
 * 运动：陨石垂直向下、尾迹沿自身路径拖出；落点向外崩碎星与碎石；焦痕贴地不动。
 * 数：碎片数绑定 `data.shards`（特攻派生），强度绑定 `data.intensity`（单颗威力 / 120），
 *   落星数量由服务端逐颗触发 fall／burst 表达。
 * 参照节：视觉语言第二、三、四、七、九节。
 */
