 ...
 * 翻译：把“下跪让对手大意后发起袭击劈向对手”翻成先制造破绽再兑现的两段——跪拜给目标打上 dropguard
 * （空门）身份并降防，随后的劈砍若落在还带着空门的目标身上就加重。劈砍直接结算，不掷命中，所以必中。
 * 数据分散：物攻定劈砍威力、空门加成、卸防等级、空门时长与击退，速度定欺身距离与蓄拜时长，
...
namespace PokemonSkills {
    actionParameters.define("kowtowcleave", {
        ...
        /** 欺身距离：基础 2.5 格，速度每比 60 快 1 加 0.02，夹在 2..4。 */
        lunge: formula(
            F.base(2.5).plus(F.stat("speed").minus(60).times(0.02)).clamp(2, 4).round(2),
            "欺身距离", { base: 2.5,
                unit: "格",
                description: "跪拜后一步欺近目标的距离；速度快的个体扑得更近。"
            }),
