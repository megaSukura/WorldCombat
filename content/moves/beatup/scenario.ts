 ...
 * 必然事实：本招被提交过（`stage.casts`）；目标受到过伤害；同伴就在身边，因此首次命中之后同一轮内还会再落一段
 *   （隔 25 刻读取，此时冷却 60 刻未到，不会有第二轮施放，读数只会来自同一轮的后续段落）。实际召集到几名、
 *   打了几段写进 note。
 * 真正的「后备宝可梦」名单需要共享入口，本招读到的是当前站在场上的同伴（见报告共享前置/翻译说明）。
 */
Smoke.scenario("beatup", function (stage) {
    ...
    stage.until(1200, function () {
        ...
        stage.after(25, function () {
            ...
            stage.note("每下基准威力随领队物攻与等级、召集半径与上限随等级、每下间隔随速度；同伴那一下按它自己的物攻相对领队缩放。实际召集到几名、打了几段由场上站位与同伴数决定。真正的后备名单替换需要共享入口，本招读的是当前在场的同伴。", {
