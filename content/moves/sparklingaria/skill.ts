 ...
 *
 * 核心念头：唱起一支歌，一圈亮晶晶的气泡从身周荡开。站在这圈里的敌人被水压轰中，越靠中心越重；
 * 同一圈里所有**别人**身上的灼伤被气泡洗净，还回一点血——唱歌的人自己不算在内。它必须站进人群里
 * 才能同时打到人和洗到人，这是它与别的歌（爆音波、轮唱）最大的不同。
 *
 * 三幕：
 *   起（windup，提交前）：吸气、气泡在身前收成一簇，可免费打断。
 *   爆（burst → hit）：气泡浪整圈炸开；圈内每个非友方按到中心的距离衰减后各挨一记 `burst`，被气泡碰到
 *       的人身上的灼伤由共享状态入口洗净（`CombatStatus.cure`）并回一点血。
...
namespace PokemonSkills {
    ...
    define({
        id: "sparklingaria",
        name: "Sparkling Aria",
        description: "唱起一支歌，一圈亮晶晶的气泡从身周荡开：圈内的敌人被水压轰中，越靠中心越重；圈内所有别人身上的灼伤被气泡洗净并回一点血（唱歌的人自己不算）。高音配置范围更广、单点更轻，收束咏叹调更近更重。",
        ...
        execute: function (action, move, config, done) {
            ...
            WorldGeometry.select(world, WorldGeometry.ring(centre, 0, radius, { below: 3, above: 4 }), function (other, facts) {
                ...
                if (CombatStatus.has(world, other, "burn") && CombatStatus.cure(world, other, "burn")) {
                    cured++;
                    heal(world, other, cureHeal, "sparklingaria");
                    WorldFeedback.emit(world, sparklingariaScene, 1, facts.position(),
                        { moment: "wash", target: String(other.ref()), intensity: Math.max(0.5, Math.min(1.6, cureHeal / 0.06)) }, 24);
