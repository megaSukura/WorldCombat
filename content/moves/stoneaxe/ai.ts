 ...
 * 对谁出手：`ai.preferCluster`（默认开）打开时，目标身边还挤着别人就抬价——石阵设在敌人必经处能一块块砸到进出的多个目标，
 *   包括飞在低空的；关闭则只按普通近身攻击排序。
 * 够不到怎么办：3.2 格左右的短射程交给 `reach`，共享任务把身位收进射程后再出手。
 * 放完之后：落点留下有限石阵（崩解式只砸第一次），每个新进入的敌人消耗一块；静止不动的目标不会被反复砸。
...
namespace PokemonSkills {
    ...
    addPreferences(stoneaxeId, {}, [
        ...
        field(pathOf("ai.preferCluster"), "优先扎堆", "boolean", {
            help: "开启后，目标身边还挤着别人时优先劈它，在敌人必经处留下的石阵能一块块砸到进出的多个目标（包括飞在低空的）；关闭则只按普通近身攻击排序。"
        })
    ]);
}
