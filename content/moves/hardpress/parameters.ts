 ...
 *   pressRadius 掌面半宽：碰撞箱宽度给掌面左右多宽；双腕式把掌面摊开一倍多。
 *   sink        压沉深度：体重决定把目标向下按多深。
 *   shove       顶开距离：体重决定沿背离掌心推多远。
 *   reach       伸手距离：身高给臂长，也是掌面铺开的长度与实际射程来源。
 *   motes       碎屑数：物攻与体重换算，驱动画面的下压柱与碎屑量。
 *   tempo／aftercast／recharge：速度定节奏；两种腕式共用同一节奏。
 *
 * 配置 `brace`（双腕式，默认关）双向取舍：开＝掌面半宽 ×1.7（一次罩住并排站的几个目标），代价是单点威力 ×0.86；
 *   起手与冷却不变；关（单腕式）＝单点更重，但掌面更窄。
 *
...
namespace PokemonSkills {
    ...
    /** 表现里的参考半宽（格）：服务端传 scale = 实际掌面半宽 / 这个值。 */
    ...
    actionParameters.define(hardpressId, {
        ...
        /** 掌面半宽：0.55 + (宽度−0.9)×0.45[−0.12,0.5]；双腕 ×1.7；夹 0.4..1.9。 */
        pressRadius: formula(
            ...
            "掌面半宽", {
                unit: "格",
                description: "掌面在脚前沿瞄准方向铺开时左右各占多宽；身板越宽掌面越宽，双腕式把这一面摊得更开。判定与表现同宽。"
            }),
        ...
        /** 顶开距离：0.28 + (体重−50)×0.003[−0.06,0.5]；夹 0.12..1.0。 */
        shove: formula(
            F.base(0.28).plus(F.body("weight").minus(50).times(0.003).clamp(-0.06, 0.5)).clamp(0.12, 1.0).round(2),
            "顶开距离", {
                unit: "格",
                description: "命中后把目标沿背离掌心方向推开多远；身体越沉推得越远。"
            }),
        /** 伸手／掌长：2.1 + (高度−1.4)×0.35[−0.2,0.6]；夹 1.7..3.0。 */
        reach: formula(
            F.base(2.1).plus(F.body("height").minus(1.4).times(0.35).clamp(-0.2, 0.6)).clamp(1.7, 3.0).round(2),
            "伸手距离", {
                unit: "格",
                description: "掌面从脚前沿朝向铺开多远，也是本招的实际射程来源；体型越高大铺得越远。"
            }),
        ...
        /** 起手：7 − (速度−55)×0.02[−1.5,2]；夹 4..13。 */
        tempo: seconds(
            F.base(7).minus(F.stat("speed").minus(55).times(0.02).clamp(-1.5, 2)).clamp(4, 13).round(0),
            "起手", "抬臂到手臂落下的时间；速度越快越短；两种腕式共用同一节奏。"),
        ...
        /** 冷却：22 − (等级−20)×0.08[0,4]；夹 14..34。 */
        recharge: seconds(
            F.base(22).minus(F.level().minus(20).times(0.08).clamp(0, 4)).clamp(14, 34).round(0),
            "冷却", "再压一记前的等待；等级越高回得越快；两种腕式共用同一冷却。")
    });
    ...
    describe(hardpressId, [
        ...
        { key: "description.1", values: ["reach","pressRadius","sink","shove"] },
