 ...
 *   身上的冰冻被化开，还被熏得湿透。烈日下蒸汽更烫（×1.5）、张得更开、白汽更浓。
 *
 * 两幕：
 *   起（boil，提交前）：体内水沸、口鼻与身侧翻起白汽，只播预告。
 *   喷（burst）：提交后朝目标方向张出 `reach` 长、`angle` 宽的扇面（WorldGeometry.sector，
 *       判定与表现共用同一组顶点），最多罩住 `maxTargets` 个非友方，各自结算 steam、化开冰冻、
 *       挂上湿透身份并被顶开 `push`；始终先解掉自己身上的冰冻（原生 defrost）。
 *
 * 与同族分开：热水抛出的沸水会烫伤并留下烫池；水蒸气是一条向前张开、专化冰冻的扇面，且只随强日照变强。
 */
    ...
    define({
        ...
        windup: function (action, config, prepare) {
            ...
            action.present("hydrosteam:boil", hydrosteamScene, 1, action.origin(),
                JSON.stringify({ moment: "boil", reach: reach, bellow: config && config.bellow === true ? 1 : 0 }));
            return prepare;
        },
        execute: function (action, move, config, done) {
            ...
            // 原生 defrost：烧开这炉蒸汽的一刻先解掉自己身上的冰冻。
            ...
            WorldGeometry.selectEnemies(world, region, function (victim, facts) {
                ...
                WorldFeedback.emit(world, hydrosteamScene, 1, facts.position(),
                    { moment: "scald", target: String(victim.ref()), thawed: wasFrozen ? 1 : 0, push: push,
                        scale: scale, sunlight: sunlit ? 1 : 0, intensity: sunlit ? 1.6 : 1 }, 24);
