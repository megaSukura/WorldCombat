 ...
 *
 * 一句话：施法者沉肩、钢甲上掠过一层冷光 → 整个人腾空翻了半圈、身后甩下碎石与速度线 → 以身躯砸在落点上，
 * 地面炸开一圈灰白冲击与碎石 → 被砸中的目标身上爆出钢色钝击，坑里余尘久久不散。
 * 色相家族：冷灰与钢白（impact_steel / groundquake / large_rock）为主体，土褐（earth / tinydust）作坑尘，无饱和色。
 * 拍子：起（windup 蓄势）→ 行（leap 腾空）→ 击（crash 落地、impact 命中）→ 收（crash 的余尘）。
 * 范围：crash 绑落点、fit none，地面环半径按 `data.scale`（实际落点半径 / 2.0）铺开，画出的就是被罩住的地块。
