 ...
 *
 * 一句话：施法者周身念力上涌、目标头顶压出一圈下压的印记 → 一整块深紫重物从上方追着目标砸落、拖着下坠的光屑 →
 *   落地炸成一圈白边冲击面与飞散的碎块，目标身上再闪过一道裂特防的紫纹。
 * 色相家族：深紫蓝（0x6A3FD0 主 / 0x9B7BEE 亮 / 0xE4D8FF 近白核心），近白只给砸落的核心与冲击面；无第二个色相。
 * 拍子：起 conjure/mark 0–22t ／ 落 descend 0–80t ／ 砸 crush 0–30t ／ 裂 sunder ／ 场 shock ／ 空 miss。
 * 范围：crush 的贴地大环与 shock 的外扩环都按 `data.scale`（判定半径 / 0.5）铺开，就是这一压盖住的那块地方。
 * 运动：conjure 向上涌；mark 的印记缓缓下压；descend 绑 projectile 沿下落方向甩出下坠的光屑；
 *   crush 由内向外炸、碎块受重力落下；shock 环贴地向外扩。
...
const PsystrikeDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        mark: {
            ...
            emitters: [
                {
                    name: "seal", bind: "target", offset: [0, 1.4, 0], height: 0.4, orient: "fixed",
                ...
                {
                    name: "weight", bind: "target", offset: [0, 0.6, 0], height: 0.6,
        ...
        crush: {
            ...
            emitters: [
                {
                    name: "core", bind: "target", offset: [0, 0.16, 0], height: 0.45,
                ...
                {
                    name: "shock_ring", bind: "target", offset: [0, -0.5, 0], height: 0, orient: "fixed",
                ...
                {
                    name: "debris", bind: "target", offset: [0, 0.24, 0], height: 0.5,
