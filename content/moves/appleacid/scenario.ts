 ...
 * 并给目标留下发酵身份（共享身份 world_combat:status/sour，命中后必然施加）。
 * 叠酸（第二颗 −2）、酸浆每跳、苹果是否落空都是随机/位置结果，写进 note 供读轨迹判断。
 */
Smoke.scenario("appleacid", function (stage) {
    ...
    stage.until(900, function () {
        ...
        stage.expect(stage.hadMobEffect(machop, "world_combat:status/sour"), "目标被酸到后带上了发酵身份");
        stage.note("叠酸（第二颗改为 −2）、酸浆每跳伤害、以及苹果是否落空都是随机/位置结果，只作记录。",
