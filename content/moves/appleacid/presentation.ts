 ...
 * 苹果酸 / appleacid —— 客户端表现。
 *
 * 一句话：手里掂着一颗酸苹果 → 苹果沿弧线抛出去 → 砸中炸成一圈酸浆、主目标与周围一起被泡软 →
 * 落点留下一摊冒泡的酸浆，走进去的人先被浸到发酵、留在里面反复被咬。
 * 色相家族：酸苹果的黄绿（0x9EC44A / 0xCFE07A），近白只给砸中的那一下。
 * 拍子：起 windup（掂果）→ 掷 throw（脱手）→ 爆 burst（炸开）→ 击 hit／叠酸 stack（主目标）→ 留 patch（冒泡）→ 浸 soak／patch_hit（迟到者）。
 * 范围：burst 的溅射圈半径直接绑定服务端的 `data.splash`，patch 的覆盖绑定 `data.splash`（酸浆半径同量级），
 *   玩家一眼看出酸浆盖住哪一圈。
 * 运动：苹果沿弧线飞向目标（原生投射物锚点同步）；酸浆的酸泡在原地升起。
...
const AppleAcidDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        throw: {
            ...
            emitters: [
                {
                    name: "throw_trail", bind: "source", fit: "none", offset: [0, 0.3, 0], orient: "toward",
                    particle: "world_combat_core:cobblemon/generic/goo/acidsplash",
                    burst: { count: { data: "cores", fallback: 12 }, interval: 2, repeats: 2 },
                    shape: { kind: "cone_volume", radius: 0.5, length: 1.8, angleDegrees: 34 },
                    direction: "shape", speed: [0.2, 0.5], spread: 12, gravity: 0.05,
                    lifetime: [8, 16], size: [0.1, 0.02],
                    color: 0x9EC44A, alpha: [0.85, 0], light: "world", maxParticles: 50
                }
            ]
        },
        burst: {
        ...
        patch: {
            ...
            emitters: [
                ...
                },
                {
                    ...
                    rate: 4, shape: { kind: "circle", radius: 1.4 },
