 ...
 *
 * 翻译：把「将２根针刺入对手，连续２次给予伤害」落成一记**先后两下**的双刺——两针按出手时间 `gap` 先后射出、
 *   不等前一根飞到；第一针先扎开伤口，第二针只有命中第一针实际打中的同一目标，才更容易把毒带进去（`woundBonus`）。
 *   它不是一次伤害结算两次，而是一段有前后因果、两针各自结算的两拍。属性取原生的虫，毒只是它附带的。
 *
