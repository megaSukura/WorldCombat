 ...
 * 挡了几下、壳挡满没有写进 note 供读轨迹判断。壳开的当刻只解除本招自己那条 root。
 */
Smoke.scenario("withdraw", function (stage) {
    ...
    stage.until(1000, function () {
        ...
        stage.after(60, function () {
            stage.note("the shell shows one large petal per remaining block (count from level and the deep-shallow choice) and spends one whole-charge ward per accepted hostile attack; the shell and this cast's own root share one instance, so when the last block lands or the window ends the shell opens at once and only this cast's root is released, with the Defence taken back.", {
