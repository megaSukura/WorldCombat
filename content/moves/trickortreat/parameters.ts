 ...
 *
 * 世界化：即时战场没有第六个属性槽，所以把这招落到共享的临时属性层 NativeModifiers.types 上——**给目标套上
 *   一件幽灵外壳**，只在这件外套还没穿满三层属性时套得上（目标已到第三属性或已是幽灵时套不上，预检直接
 *   拒绝，不浪费 PP）。外壳期间 `NativeEffects.types` 带着 ghost，于是本系、受击相性、AI 与 ready 一起变化；
 *   壳到期或提前被解掉时解除属性层，属性随原生个体本身恢复。壳的属性层若先被单独解除，记录层会把外壳一起
 *   脱掉，画面不留一件空壳。反制：属性层已满的目标套不上；识破／气味侦测也能把这件壳的幽灵免疫再摘掉。
 *
 * 瞄准：kind:aim。敌我实体都可指定——对敌是打开幽灵／恶弱点，对友是替它挡下一般／格斗；空点不套壳（走 fizzle），
 *   照付同样的 PP 与冷却。非宝可梦没有属性可追加，明确拒绝而不编造。
 *
