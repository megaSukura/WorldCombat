 ...
 *
 * 什么局面有意义：附近有一个生命低于 ai.healBelow、且身上带着主异常的伙伴（不含自己——这招治的是别人）。
...
namespace CompanionBehavior {
    ...
    function purifyAfflicted(context: WorldBehavior.Context, target: Entity): boolean {
        for (let index = 0; index < PokemonSkills.purifyMalaise.length; index++)
            if (status(context, target, PokemonSkills.purifyMalaise[index])) return true;
        return false;
    }
