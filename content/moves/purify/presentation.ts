 ...
 * 净化 / Purify 的粒子语言。
 *
 * 一句话：手心先拢起一点净光（windup）→ 施法者朝目标探手，目标身上的病痛被抽成暗紫的雾、沿一条线飞回
 *   施法者（draw）→ 暗雾在施法者身上落地，化作一圈薄荷色的回血光（absorb）。
 ...
 * 数：absorb 的回血光量绑 data.motes 并以 data.intensity（由回复比例派生）缩放，抽出病痛数绑 data.removed（抽掉的项数）。
 */
const PurifyDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        fizzle: {
            ...
            emitters: [
                {
                    name: "fizzle_puff", bind: "source", offset: [0, 0.4, 0], height: 0.2,
