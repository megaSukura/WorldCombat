 ...
 *
 * 翻译：把「大量水流」落成一记**朝准线锁死、持续 6 刻的高压水柱**——不是加农水炮那种又细又准的一击，而是
 *   从喷口到真实受阻点始终连着的一整柱：正前方第一个敌人被浇透一次并沿准线被分步顶开；漫灌式再在受阻端点
 *   向周围看得见的一小圈溅一次。它慢、贵、看得见起手（蓄水很久），所以给对手留出走位窗口；命中 80 就翻成
 *   「准线附近会散」加「起手长」这两件看得见的事，而不是一次暗掷。
 *
 * 与场上最像的招分开：加农水炮是笔直单点高压柱、放完施法者力竭；喷水（waterspout）是从脚下整圈漫出；
 *   喷射火焰是会转向、会变长的火舌。水炮是**方向锁死、只持续 6 刻、只推一次的单体压力柱**。
 *
 * 数据分散（每项读不同的精灵数据）：
 ...
 *   blow      推距：体重与特攻共同决定沿准线把首个敌人顶开的总距离。
 *   velocity  水柱流速：速度换算的水柱粒子流速，驱动客户端水柱运动。
 *   reach     射程：特攻与等级决定能轰多远，也是本招实际射程来源。
 *   soakTicks 湿身时长：等级决定被浇得多透。
 *   spread    散射角：特攻越低散得越开，释放那一刻一次性决定整柱方向（命中 80 的落地方式之一）。
...
namespace PokemonSkills {
    actionParameters.define("hydropump", {
        ...
        splash: formula(
            ...
            "回溅威力", {
                unit: "威力",
                description: "水柱撞上硬面或身体那一点，漫灌式向周围看得见的一圈敌人各溅一下的威力；特攻越高回溅越重，冲压式不回溅。"
            }),
        /** 水柱判定：基础 0.55 格，碰撞箱每比 1.4 高 0.1（夹 −0.08..0.35）；
         *  漫灌 ×1.2 / 冲压 ×0.9；夹 0.38..1.2。 */
        radius: formula(
            F.base(0.55).plus(F.body("height").minus(1.4).times(0.1).clamp(-0.08, 0.35))
                .times(F.when(F.pref("deluge"), F.const(1.2), F.const(0.9)))
                .clamp(0.38, 1.2).round(2),
            "水柱判定", {
                unit: "格",
                description: "水柱飞行与命中的判定粗细；体型越高水头越大，越容易正面撞上目标。漫灌更粗、冲压更窄。画面里水柱的宽度就是它。"
            }),
        ...
        blow: formula(
            ...
            "推距", {
                unit: "格",
                description: "水柱把正前方第一个敌人沿准线顶开的总距离，分摊到 6 刻里逐刻推动；身体越沉、水势越猛顶得越远，冲压式更集中。实际位移受原生碰撞限制。"
            }),
        /** 水柱流速：基础 1.0，速度每比 55 快 1 加 0.006（夹 −0.15..0.4）；漫灌 ×0.9 / 冲压 ×1.05；夹 0.7..1.8。 */
        velocity: formula(
            F.base(1.0)
            ...
            "水柱流速", {
                unit: "格/刻",
                description: "水柱粒子沿准线流动的速度，由速度数据换算、驱动客户端水柱的运动；它不是独立伤害。"
            }),
    ...
    describe("hydropump", [
        ...
        { key: "description.1", values: ["radius","reach","spread"] },
