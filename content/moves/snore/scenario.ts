 ...
 *
 * 场面：只会打鼾的卡比兽站在一侧，对面一只关掉 AI 的铁傀儡当靶子（不逃不还手，施术者的睡不会被伤害打断）。
 * 先让施术者醒着：打鼾以共享睡眠身份为门槛，这时它不该被提出；随后按周期（按自身位置选中自己）用 /effect
 * 把睡眠续上，施术者就会在睡梦里喷出鼾声。
 *
 * 断言只取必然事实：醒着时没有施放；睡着后提交过打鼾；靶子挨到过伤害。
 * 每一声约 30% 的畏缩掷骰、靶子的走向与暴击都是随机／位置项；声线止于实际接触点、回响第二声只在仍睡着时
 * 才响，写进 note 供读轨迹判断。
 */
Smoke.scenario("snore", function (stage) {
    ...
    // 早于任何外来睡眠出现前核验清醒门槛，并清掉可能落下的睡眠，保证这一段确实是醒着的。
    stage.after(80, function () {
        stage.command("effect clear @e[type=cobblemon:pokemon,distance=..6,limit=1,sort=nearest] world_combat:sleep");
        ...
        stage.until(700, function () {
            ...
            stage.note("每声约 30% 的畏缩掷骰、靶子的走向、暴击都是随机／位置项；声线只画到实际接触点，回响第二声需仍处于睡眠才响", {
