 ...
 *
 * 翻译：把「发射虹色光束」落成一条**会跑起来的虹色光束**——在身前把冷光折成一段棱镜，沿瞄准线冲出去，
 * 落点结出一小片虹色霜；挨到的人偶尔被这道冷光刺得攻击下降。它是本组唯一带颜色的光束：与冰冻光束
 * （瞬发贯穿的白蓝线、留冰线、冰冻）分开——极光束是看得见它一路跑到哪的彩虹缎带、只打最前面一个、
 * 留的是一小块霜斑、压的是攻击。
 *
 * 数据分散（每项读不同的精灵数据）：
 *   beam        光束威力：特攻定冷光的压强，等级让虹色更烈。
 *   reach       射程：特攻决定这道光能照多远。
...
namespace PokemonSkills {
    ...
    describe(aurorabeamId, [
        { key: "description.0", values: ["beam", "shimmer"] },
