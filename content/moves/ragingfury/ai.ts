/**
 * 大愤慨 的伙伴 AI 用途：一套「连续几口火舌扫过去、把地面点着」的出手计划。
 *
 * 什么局面有意义：威胁可见、敌对、存活，且在 ai.maxChase（默认 9）格以内；更远交给共享接近逻辑。
 * 对谁出手：当前威胁。**正处于恍惚（共享身份 confusion）时不出手**——刚喷完的自己就是乱的。
 * 够不到怎么办：reach 就是火舌推进距离，够不到就由共享任务走近。
 * 放完之后：目标被点着又推开，伙伴会跟着压上去一小段，把对方留在刚点起的余火里。
 * 执行时每口沿目标的当刻身位重瞄（AI 沿已锁定目标，玩家按住技能键可自行换向），段内方向固定。
