 ...
 *
 * 翻译：把「用有毒的牙齿咬住对手」落成**咬开伤口、把毒液注进去**——本族里咬得最轻、最准的一口，没有畏缩，
 * 卖的完全是那管毒。独有部分在**注毒**：咬中不立刻发作，隔 `pump` 刻毒液才在伤口里渗开，掷出则加重为剧毒
 * （共享身份 world_combat:status/toxic，宝可梦同步为原生剧毒）；若伤口本来就带着毒，这一口会**确保**把它加深成剧毒——
 * 因此它是本族里唯一给持久伤害收尾的牙。
 *
 ...
 *   reach/lunge  扑出距离与速度：速度派生；浓毒式收一点、注毒更慢。
 *   grip         獠牙判定：身高派生。
...
namespace PokemonSkills {
    actionParameters.define("poisonfang", {
        ...
        /** 扑出距离：2.0 + 速度偏移[−0.35,1.0]；浓毒 ×0.94 / 快毒 ×1.05；夹 1.5..3.1。 */
        reach: formula(
            F.base(2.0).plus(F.stat("speed").minus(55).times(0.013).clamp(-0.35, 1.0))
                ...
                .clamp(1.5, 3.1).round(2),
            "扑出距离", {
                unit: "格",
                description: "从起步到咬到的总位移，也是本招的实际射程来源；腿快的个体扑得更远。浓毒式收得更短。"
            }),
        /** 扑咬速度：0.72 + 速度偏移[−0.12,0.3]；浓毒 ×0.92 / 快毒 ×1.06；夹 0.5..1.05。 */
        lunge: formula(
            F.base(0.72).plus(F.stat("speed").minus(55).times(0.005).clamp(-0.12, 0.3))
                .times(F.when(F.pref("venom", text("worldcombat.skill.poisonfang.preference.venom")), F.const(0.92), F.const(1.06)))
                .clamp(0.5, 1.05).round(2),
            "扑咬速度", {
                unit: "格/刻",
                description: "扑出时每刻前进的距离；浓毒式沉一点，快毒式起得更急。"
            }),
    ...
    describe("poisonfang", [
        ...
        { key: "description.pounce", values: ["lunge","reach"] },
