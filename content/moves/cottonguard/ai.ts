 ...
 *
 * 什么局面有意义：有威胁且在 ai.maxChase 内时，先裹上绒衣再扛；身边暂时安全时（驻守／自主／工作）也先裹好。
 * 什么时候最想出手：血量掉到 ai.panic 以下（正在挨压）时 priority 110 抢在共享次序前——防招要在被打崩之前裹上；
 *   血量还行就退回普通次序，先输出。
 * 对谁出手：自己；不需要接近，由共用任务直接施放。
 * 放完之后：防御等级已经写进公共能力阶梯，绒衣窗口内不再重复；绒衣被磨掉后才重新考虑。
 */
namespace PokemonSkills {
    ...
    addPreferences("cottonguard", {}, [
        ...
        field(pathOf("ai.panic"), "紧急血量", "number", {
            ...
            help: "血量比例低于这个值时抢在共享次序前裹身；调高更早进入防守姿态，调低只在濒危时才裹。"
        })
    ]);
}
