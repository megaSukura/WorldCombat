 ...
 *   鸣（wave，提交后）：以自身为顶点朝瞄准方向压出一道锥形声波，贴地扩散；选中敌人就朝它、没有选中实体也能
 *       对着空点振鸣。锥内每个敌人各结算一次伤害（整道声波只伤一次），威力随距离从满额衰减到 `falloff`；
 *       每人各掷一次 10% 起的碾防（共享 `NativeEffects.boost(..., "spd", -1)`）。
 *
 * 声波没有飞行物、瞬时展开。选敌只按锥形区域（`WorldGeometry.sector` + `selectEnemies`），不做视线遮挡——
 * 石头墙挡不住声音，这是本招明确保留的策略，不是漏掉了去遮挡。与同族分开：唯一锥形、近强远弱、可绕墙、
...
namespace PokemonSkills {
    ...
    define({
        id: "bugbuzz",
        name: "Bug Buzz",
        description: "朝瞄准方向振动身体压出一道锥形声波，贴地扩散、可以穿过墙面：锥内每个敌人各承受一次特殊伤害，越近越重，并可能被震得特防下降 1 级；没有选中敌人也能对着空点振鸣。",
