 ...
 * 必然事实：本招被提交过；它命中过靶子（`damageTo(foe) > 0`）；施法者沿竖向弧线移动过（`travelled > 0.1`）。
...
Smoke.scenario("axekick", function (stage) {
    ...
    stage.until(1400, function () {
        ...
        stage.expect(stage.travelled(caster) > 0.1, "the user hopped along the chop arc");
        stage.note("daze is rolled per hit at the computed chance, so a false reading is possible; a self-hurt appears only on a whiff", {
