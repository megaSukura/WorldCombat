/**
 * 原始之力的可执行设计说明：让会这一招的菊石兽（Omastar，真实学习者）走向一只睡眠的对手并轰开脚下的古力。
 *
 * 必然事实：本招被提交过（`stage.casts`）；以自身为中心的一圈轰到并造成伤害（`damageTo`）。
 * 反哺是否触发（约 10% 起）、推开多远、托起多高都写进 note 供读轨迹判断；本场景只放一个目标，
 * 多人半球与空中目标由完整装配的人工试玩核对。
 */
Smoke.scenario("ancientpower", function (stage) {
    stage.fill([-8, -1, -6], [10, -1, 6], "minecraft:stone");
    stage.time("day");
    stage.weather("clear");
    var caster = stage.pokemon({ species: "omastar", level: 36, moves: ["ancientpower"], at: [-3, 0, 0] });
