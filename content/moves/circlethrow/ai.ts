 ...
 *   更远由共享接近逻辑走过去，但接近距离由 grip 决定（很短）。
 * 对谁出手：`selectTarget` 只接一个——优先「正在攻击自己或主人」的那个（借它冲上来的力摔最顺），
 *   并列时挑最近的。玩家「关注」的焦点目标直接 honored；`accepts` 排除友方、已死、不可见与已经带着「溃退」的目标。
 * 什么时候最想出手：对手已经贴得很近、正在攻击时 priority 抬高——它是一记贴身反击，不是远攻。
 * 够不到怎么办：reach 是抓取距离，共享任务先贴上去再抓。
 * 放完之后：目标挨一记摔击、被摔到背后并逐出交战圈，伙伴交回共享交战计划。
