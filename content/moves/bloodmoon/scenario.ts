 ...
 *
 * 一句话：一只血月月月熊（L70，原生学习者，血月形态）从 11 格外召出一轮血月，月柱垂落砸中站桩的卡比兽；
 *   砸完之后它短时间内不能再次召月——这就是原生「无法连续使出2次」。
 *
 * 场面：血月形态的月月熊（ursaluna，aspect=bloodmoon）只会血月，卡比兽（snorlax）只会跃起、原地站桩，
 *   相隔 11 格——在射程内，月月熊不用先靠近；地面铺平、白天晴天。
 *
 * 必然事实：本招被提交过（`stage.casts`）；目标受到过伤害（`stage.damageTo`）；首次召月后 60 刻内没有再被使出
 *   （禁复门禁——它在 60 刻内只有血月一招可用，重试会被顶回去）。主目标/溅射的伤害数值、暴击、焦痕格子随地面与
 *   随机而定，写进 note 供读轨迹判断。
 */
Smoke.scenario("bloodmoon", function (stage) {
    ...
    stage.until(3000, function () {
        ...
        stage.after(60, function () {
            ...
            stage.note("the first cast is counted; within the spent window the only move it knows is rejected, so the count stays 1", {
    ...
    }, "blood moon comes down within 150 s");
});
