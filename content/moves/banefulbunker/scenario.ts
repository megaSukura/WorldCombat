 ...
 * 碉堡的执行设计说明。
 *
 * 场面：一只只会碉堡的精灵站定，6 格外一只卫道士开战并走近。毒壁只守施法者的落脚点，所以这里让施法者停在原地。
 * 必然事实：本招被提交过且碉堡已合拢；碉堡合拢后，一次来自卫道士的接触攻击被挡下，且它被灌上共享身份
 * world_combat:status/poison（与是不是宝可梦无关，任何生物共用同一条主异常）。
 * 用非亡灵的攻击者是有意为之：亡灵受原版 canBeAffected 保护、不吃中毒，灌毒走的是原版规则。
 * 挡下的量、灌毒是否落地写进 note 供读轨迹判断；“离位立即塌壁”需要把施法者移出锚点，AI 场面里不便稳定构造，
 * 同样写进 note 交由人工试玩确认。
 */
Smoke.scenario("banefulbunker", function (stage) {
    ...
    stage.until(600, function () {
        return stage.casts("banefulbunker", caster) > 0 && stage.hasMobEffect(caster, "world_combat:baneful_guard");
    }, function () {
        ...
        stage.after(6, function () {
            ...
            stage.note("banefulbunker block and poison", { before: before, after: stage.damageTo(caster),
                ...
                anchored: stage.hasMobEffect(caster, "world_combat:baneful_guard") });
            stage.done();
        });
