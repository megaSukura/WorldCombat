 ...
 *
 * 一句话：把带毒的壳射出去，命中时按对方哪一面软在钝击（物理）与喷射（特殊）之间自动挑伤害更高的一面。
 *
 * 场面：一只只会臂贝武器的呆壳兽（slowbro，L40；原生的唯一学习者是它的伽勒尔形态，这里只用基础形态承载机制）
 *   对一只只会跃起、站桩的卡比兽（snorlax，L30），相隔 8 格——在射程内，AI 可以直接发炮；地面铺平、白天晴天。
 *
 * 必然事实：本招被提交过（`stage.casts`）；目标受到过毒壳伤害（`stage.damageTo`）。
 * 随机量写进 note：这一发按对手物防/特防对比选中的分类、中毒概率、暴击，供读轨迹判断。
 */
