 ...
 *   锁定方向做直线重撞；命中第一个敌人时先把它沿冲撞方向压出去，再按它实际停住的身体做一次短距方块
 *   碰撞射线——身体真的贴到墙面才算墙撞，威力 ×slamBonus、rooted slamStun 刻撞乱节奏，并把贴着的那几格
 *   墙凿成一个临时缺口（world.terrain 的 linger 租约，换成空气、breachTicks 后原地形自己放回）。
 *
 * 场面：石面平地、晴空正午。一只只会火箭头锤的 squirtle（技能表只给这一招，私有冒烟装配里别的动作
 *   都不在，AI 就只会用它）在目标正后方立一面石墙（offset x=4），目标是一只 NoAI 的僵尸（普通拳靶，
 *   原版生物代表任意对手），贴着墙面站在 x=3.6，施术者从 x=-3 起手。两者敌对后 AI 自己走位、缩头、
 *   沿直线冲撞。僵尸冻结在原地，正对上这招「惩罚站桩目标」的用途，让「贴墙被钉」可复现。
 *
...
Smoke.scenario("skullbash", function (stage) {
    ...
    // A plain vanilla punching bag under NoAI, standing flush against the wall face (x=4): the charge's actual
    // body-to-wall probe reads a real pin, and the slam bonus and breach only appear when the body is truly
    // pressed to the wall.
    var target = stage.mob({ type: "minecraft:zombie", at: [3.6, 0, 0] });
    stage.noai(target);
    ...
    stage.until(1200, function () {
        ...
        stage.after(6, function () {
            ...
            stage.note("a charge hit a foe whose back was against the recorded wall: it took the slam multiplier and the wall was leased to air as a gap that closes again. Random here: damage roll and crit, whether the zombie survives, how many charges it took to pin it, how many wall cells the breach actually carved, and how long the gap stayed open. rooted/GuardEffects during the tuck are shared script effects, not MobEffects, so the stage cannot see them.", {
