 ...
 *
 * 核心念头：朝对手撒一把会改写身体的魔法粉——粉末团飘过去罩住它，把它整个改写成超能力属性。
 *   草属性生物把粉从身上抖掉，完全免疫；已经是纯超能力的也撒不上去。
 *
 * 幕：
 *   起（windup，提交前）：手里抖匀这把粉，只观察与预告，可被打断且不花代价。
 *   撒（throw，提交后）：粉团带轻微追踪飘向目标，撞上时在它身上炸开一团粉。
 *   改（coat）：目标的属性被写进共享 NativeModifiers types 层（单一超能力，到期自动还原原生属性），
 *     并挂共享身份 `world_combat:status/magicpowder` 的标记；粉粒与超能闪点在它身上亮一下。
 *   散（wipe）：改写到期时粉末从身上飘散，告诉玩家这层已经过去。
 *
 * 反制：草属性免疫粉末（预检直接拒绝，不浪费 20 发 PP）；已经是纯超能力的撒不上去；粉团飘到之前走开即可。
 */
        ...
        execute: function (action, move, config, done) {
            ...
            function coat(current: CombatAction, impact: CombatImpact): void {
                ...
                if (hit === null || !scope.valid(hit) || scope.friendly(hit)) {
                    WorldFeedback.emit(scope, magicpowderScene, 1, at, { moment: "miss", motes: motes, scale: scale }, 22);
                    finish(current);
                    return;
                }
            ...
            const flight = LivingActions.projectile(action, {
                ...
                direction: direction,
                appearance: { sprite: "cobblemon:particle/generic/powder", scale: Math.max(0.7, scale), tint: 0xE86CC8,
                    homing: { target: targetRef, turn: 6, delay: 1, range: action.range() } },
                impact: coat
            }, function (current) {
                WorldFeedback.emit(current.world(), magicpowderScene, 1, current.targetPosition(), { moment: "miss", motes: motes, scale: scale }, 22);
                finish(current);
            });
            WorldFeedback.emit(world, magicpowderScene, 1, from,
                { moment: "throw", projectile: flight, target: targetRef, motes: motes, cloud: cloud, scale: scale }, 34);
