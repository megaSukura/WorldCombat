 ...
 * 运动：晶片由体内向外铺、贴体停住；晶环向外推开；持壳时冷光贴着棱面明灭；被打时碎屑向外崩落；
 *   裂纹沿 `data.direction`（真实来袭接触侧）朝向，越接近碎壳阈值 `data.depth` 越深、碎晶越多越大；碎裂时受重力落下。
 * 数：晶面量绑 `data.facets`（防御与等级派生），崩落碎晶量绑 `data.shards`（按 `data.depth` 放大），
 *   深度绑 `data.depth`，削伤程度绑 `data.intensity`，尺寸绑 `data.scale`。
...
const HardenDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        crack: {
            ...
            emitters: [
                {
                    name: "crack_shard", bind: "source", fit: "none", height: 0.4, orient: "direction",
                    particle: "world_combat_core:cobblemon/generic/ice/iceshard",
                    burst: { count: { data: "shards", fallback: 14 } },
                    shape: { kind: "arc", radius: 0.6, arcDegrees: 140 },
                    direction: "shape", speed: [0.04, 0.2], gravity: 0.035, drag: 0.92, spin: 22,
                    lifetime: [10, 18], size: [{ data: "shardSize", fallback: 0.4 }, 0.03],
                    color: 0x9FB8B3, alpha: [0.7, 0], light: "world", maxParticles: 120
                },
                {
                    name: "crack_flash", bind: "source", fit: "none", height: 0.45, orient: "direction",
        ...
        shatter: {
            ...
            emitters: [
                {
                    name: "shatter_fall", bind: "source", fit: "none", height: 0.4, orient: "direction",
                    particle: "world_combat_core:cobblemon/generic/ice/iceshard",
                    burst: { count: { data: "facets", fallback: 14 } },
                    shape: { kind: "arc", radius: 0.5, arcDegrees: 180 },
                    direction: "shape", speed: [0.03, 0.12], gravity: 0.05, drag: 0.9, spin: 24,
                    lifetime: [12, 22], size: [0.18, 0.04],
                    color: 0x9FB8B3, alpha: [0.65, 0], light: "world", maxParticles: 140
                },
                {
                    name: "shatter_flash", bind: "source", fit: "none", height: 0.45, orient: "direction",
                    particle: "world_combat_core:cobblemon/generic/impact/impact_steel",
                    burst: { count: 1, at: 1 },
                    shape: { kind: "arc", radius: 0.35, arcDegrees: 150 },
                    direction: "shape",
                    lifetime: [10, 12], size: [0.5, 0.9],
