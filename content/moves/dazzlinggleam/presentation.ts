 ...
 *
 * 一句话：施法者把身周飘散的粉色微光收拢到身上，脚下浮起一圈将亮未亮的光环；随后一片强光整圈炸开，
 * 光浪贴着地面向外推、外圈射出几道亮光，最后身周只剩几粒慢慢升起的余辉。
 * 色相家族：粉白（glowingsparkle_pink / shinesparkle_rainbow / impact_fairy / largefadeorb）为主体，
 *   近白只做炸开核心的强调（flash / giantring_white）。
 * 拍子：起（charge 微光内收、地面光环将亮）→ 闪（flash 整圈炸开、hit 逐个目标被闪花）→ 收（miss 落空 / 余辉）。
 * 范围：charge / flash 的地面圈按服务端传的 `data.radius`（真实光浪半径）画出，玩家看到的圈就是会被闪到的地。
 * 运动：起手微光与光环向内收束，炸开时整圈向外推、外圈光道沿半径射出。
 * 数：`data.rays`（特攻与等级派生）决定外圈光道条数，`data.motes`（特攻派生）决定光尘量，
 *   `data.count`（本次威力×距离派生）决定每个命中目标身上的爆点量。
 * 参照节：视觉语言第二、三、四、七、九节。
 */
    ...
    moments: {
        charge: {
            ...
            emitters: [
                ...
                },
                {
                    ...
                    rate: 14, shape: { kind: "circle", radius: { data: "radius", fallback: 3.4 }, thickness: 0.85 },
            ...
            emitters: [
                {
                    ...
                    lifetime: [5, 9], size: [2.4, 3.4], sizeMode: "linear",
                    color: 0xFFFFFF, alpha: [0.9, 0], light: "full", bloom: 0.7, maxParticles: 4
                },
                {
                    ...
                    direction: "outward", speed: [0.05, 0.18], spread: 6,
                    lifetime: [12, 20], size: [0.8, 1.6], sizeMode: "linear",
                    color: 0xFFE9F6, alpha: [0.7, 0], light: "full", bloom: 0.4, maxParticles: 60
                },
                {
                    ...
                    color: 0xFFFFFF, alpha: [0.9, 0], light: "full", bloom: 0.5, maxParticles: 40
                },
                {
        ...
        hit: {
            ...
            emitters: [
                ...
                {
                    name: "petal", bind: "target", height: 0.55,
                    particle: "world_combat_core:cobblemon/generic/sparkle/glowingsparkle_pink",
