 ...
 *
 * 一句话：施法者伏地低头，身上坠下一层暗影、乱发竖起；随后一束凌乱的黑发从最低处贴着地面窜出去，
 * 在目标护架下方炸开一道暗色裂口，把它撞得一顿。
 * 色相家族：近黑与暗紫（obscuringsmoke／scalingshaded／impact_dark），发梢用一点苍白的紫灰挑亮。
 * 拍子：起（feign 伏低）→ 击（lash 发刺窜出、hit 命中、stagger 撞顿）→ 收（miss 落空）。
 * 范围：lash 用 `data.path` 画出服务端从施法者到目标的同一条发刺轨迹，发梢够到哪就是判定到哪。
 * 运动：暗影从身上沉下，发束沿低处的轨迹窜出（不是直线落下，而是贴地掠过），命中处暗色向外裂开。
...
const FalsesurrenderDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        lash: {
            ...
            emitters: [
                {
                    name: "strand", bind: "path", offset: [0, 0.35, 0],
                    particle: "world_combat_core:cobblemon/generic/wrap",
                    shape: { kind: "polyline" },
                ...
                {
                    name: "lacerate", bind: "path", offset: [0, 0.35, 0],
