 ...
 *
 * 翻译：把「朝对手快速踢出三脚、一脚比一脚重」落成**朝释放方向的三脚直踢**——每脚从身体中心沿当刻 aim
 * 取一段真实短三维首碰，只打第一个挡在脚前的敌体，第 n 脚威力 = kick × (1 + ramp × 已中脚数)。
 * 每脚独立掷命中，落空或撞墙这串就停；前两脚只轻推、最后一脚把整串余下的推力一次送出。
 * 与三旋击分开：三连踢是**定身、直线、贴地、快而准**的单体直踢（脚数间隔更短、命中率更高、冷却更短）；
 ...
 *   halfWidth 踢击接触半径：碰撞箱宽度决定这一脚的首碰有多粗（脚会先够到多宽的目标）。
 *   gap       脚间隔：速度决定三脚连得多紧。
 *   accuracy  每脚命中率：速度提高它；抽射式略降。
 *   push      整串总击退：物攻决定踢开后总共顶多远；抽射式顶得更远。前两脚各出四分之一，最后一脚补足余量。
...
namespace PokemonSkills {
    ...
    actionParameters.define(triplekickId, {
        ...
        /** 踢击接触半径：0.4 + 宽度偏移[−0.04,0.28]；夹 0.34..0.72。 */
        halfWidth: formula(
            F.base(0.4).plus(F.body("width").minus(0.9).times(0.3).clamp(-0.04, 0.28)).clamp(0.34, 0.72).round(2),
            "踢击接触半径", {
                unit: "格",
                description: "这一脚伸出去时首碰有多粗——脚会先够到多远的目标；身体越宽的个体接触越宽，画面里弹出的足形大小与它一致。"
            }),
        ...
        /** 整串总击退：(0.2 + 物攻偏移[−0.03,0.35]) × 抽射 1.5；夹 0.12..0.7。 */
        push: formula(
            ...
            "整串总击退", {
                unit: "格",
                description: "这一串三脚命中后目标被顶开的总距离；前两脚各只出四分之一，最后一脚补足余量，所以目标不会在第二脚前就被推离踢线。力量越大顶得越远，抽射式顶得更远。"
            }),
