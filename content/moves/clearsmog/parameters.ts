/**
 * 清除之烟 / clearsmog —— 参数与伤害段。
 *
 * 原生事实（Cobblemon 1.8 / Showdown）：Poison／特殊／威力 50／命中 必中（accuracy true）／PP 15／优先度 0／单体，
 *   非接触（protect/mirror/metronome）；`onHit` 清空目标的能力等级（clearBoosts）。
 *   描述「向对手投掷 特殊的泥块进行攻击。 使其能力变回原点。」学习者 38。
 *
 * 翻译：把「投出特殊泥块、使其能力变回原点」落成**掷出一团清浊之泥、砸中后炸开成一片清浊之烟**——
 *   泥块本身是特殊伤害；炸开的烟裹住目标，把它抬起来与压下去的能力等级全部冲回原点；烟还在目标身上黏一小会儿，
 *   这段时间里它**再怎么给自己加等级也会被冲散**，等于一段短暂的「留不住能力变化」的窗口。
 *   原生的「必中」落成泥块会小幅修向目标、几乎不落空；PP 15 落成中等冷却。
 *
 * 与同族分开：黑雾是从自己身上漫开、把场上所有人（包括自己）一起归零；清除之烟是**掷一团泥打一个点**，
 *   只冲掉烟里那几个对手的等级，再把他们短暂压在原点上——它是针对性的、单边的，不会动到术者自己。
 *
 * 数据分散（每个参数读不同的精灵数据）：
 *   mud        泥块砸击威力：特攻定泥的温度，速度定甩出的劲；漫烟式 ×0.85。
 *   reach      掷出距离：等级与特攻决定。它也是本招实际射程。
 *   cloudRadius 烟团半径：特攻与等级决定；漫烟式 ×1.5。
 *   linger     黏烟时长：防御与等级决定目标被压在原点多；漫烟式 ×1.4。
 *   interval   冲散周期：速度越快，烟越快把新加上的等级冲掉。
 *   flight     泥块速度：速度。
 *   clayRadius 泥块判定：碰撞箱高度。
 *   fumes      烟量：特攻换算，驱动烟团与冲散表现的数量。
 *   tempo／aftercast／recharge：速度定节奏；漫烟式更慢更久。
 *
 * 配置 `billow`（漫烟式）双向取舍（默认关，聚泥式）：
 *   开（漫烟式）：烟团半径 ×1.5、黏烟时长 ×1.4，但威力 ×0.85、起手 +1 刻、冷却 +8 刻——罩得更广更久。
 *   关（聚泥式）：威力 ×1.15、出手快、冷却短，但烟团小、黏烟短——一次精准的重泥。
 *
 * 伤害段 `mud` 与参数同名，走共享换算（原生类别 Special，Poison 属性，非接触）。
 */
