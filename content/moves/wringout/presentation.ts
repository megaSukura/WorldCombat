 ...
 * 绞紧 / wringout 的客户端表现。
 *
 * 一句话：施法者身侧先盘起一圈紫晶螺旋气，随即这股力缠上目标、由脚到头越拧越紧，一道道流光被从身体里绞出、
 *   向上散开；双绞式会再拧一记，第二圈更亮更密。
 * 色相家族：紫晶（0x9A7BFF）主体、淡紫（0xD8CCFF）细节、深紫（0x5A3FA8）余韵；单一色相。
 * 拍子：起 coil（盘螺旋）→ 击 squeeze（第一拧）→ 再击 squeeze2（双绞式第二拧）→ 空 whiff。
 * 范围：squeeze 的螺旋环半径按 `data.scale`（实际螺旋半径 / 1.2）铺开，画出的圈就是被绞到的范围。
 * 运动：螺旋贴目标向外旋紧；被绞出的流光沿目标向上飞出；收拢时环向内收。
            ...
            emitters: [
                {
                    ...
                    particle: "world_combat_core:cobblemon/generic/psychic/psyspiral",
                    rate: 40, shape: { kind: "torus", radius: { data: "scale", fallback: 1 }, thickness: 0.5 },
            ...
            emitters: [
                {
                    ...
                    particle: "world_combat_core:cobblemon/generic/psychic/psyspiral",
                    rate: 60, shape: { kind: "torus", radius: { data: "scale", fallback: 1 }, thickness: 0.6 },
