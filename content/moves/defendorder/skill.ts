 ...
 *     按**活着的手下数量**把等级写进公共能力阶梯，挂上共享身份 world_combat:status/defendorder 的甲壳窗口。
 * 反制：手下是活物，会被打掉；每少一只，甲壳立即薄一级（死亡当刻核对，另有周期核对兜底）。
 * 结束：甲壳到期或被清除时，召来的手下一起散去，等级一次收回。
 */
namespace PokemonSkills {
    ...
    // 记号：记录这一批手下的 ref 与已经供给的等级数，被清掉时按差额增减。amplifier 只给图标看。
    ...
    function defendorderWrite(world: CombatWorld, owner: CombatActor, state: any, ticks: number): void {
        const marks = world.effects(owner, defendorderMark);
        for (let i = 0; i < marks.length; i++) world.operation(marks[i].id(), "world_combat:dispel", "{}");
        world.effect(defendorderMark, owner, JSON.stringify(state), ticks);
    }
    function defendorderClear(world: CombatWorld, owner: CombatActor): void {
        const marks = world.effects(owner, defendorderMark);
        for (let i = 0; i < marks.length; i++) world.operation(marks[i].id(), "world_combat:dispel", "{}");
    }
    ...
    function defendorderReconcile(world: CombatWorld, owner: CombatActor): void {
        ...
        if (body !== null) WorldFeedback.keep(world, "defendorder:shell:" + String(owner.ref()), defendorderScene, 1, body.position(),
            { moment: "cling", owner: String(owner.ref()), guards: desired, size: 0.14 * (Number(state.scale) || 1) }, 22);
    }
    ...
    WorldBodies.define(defendorderUnderling, {
        ...
        tick: { every: 2, handler: function (brain) { defendorderHover(brain); } },
            ...
            // 少一只就立即核对，甲壳当场薄一级。
            if (owner !== null) defendorderReconcile(world, owner);
        }
    });
    ...
    // 兜底核对：手下死亡当刻已经核对过，这里每 20 刻再对一次，防止任何漏网。
