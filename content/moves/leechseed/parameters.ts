 ...
 *   world_combat:status/leechseed，物品栏可见、/effect 可用、牛奶可解）；旁边挂一枚托管标记，标记的 source
 *   就是施法者，所以伤害、权限与归属真实来自种植者，并由标记自己的钟排每一口。走完自己的时间或被外力解除通向同一幕收尾。
 * 输入：自由瞄准（kind: "aim"），沿方向投掷；墙与空放只让种子落地，真正首碰的活体敌人才进入种植判定。
 *
