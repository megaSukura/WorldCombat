 ...
 * 层次：凑近（起手）、落吻（心爆＋妖精冲击＋内收环）、余韵（头顶飘心）、打偏（心散的一顿）、
 *       亲空（没贴到时散开的一点心）。
 * 起击收：windup（凑近）→ kiss（亲上）→ linger（还在发懵）。
 * 数：落吻的心数与余韵的亮度按服务端 data.hearts 派生，亲密度越高画面越满；爆发强度按 data.scale。
 */
const SweetkissDefinition: ParticleDefinition = {
    ...
    moments: {
        ...
        kiss: {
            ...
            emitters: [
                {
                    name: "kiss_heart", bind: "target", height: 0.72,
                    particle: "world_combat_core:cobblemon/generic/status/infatuation_heart",
                    burst: { count: { data: "hearts", fallback: 14 }, interval: 3, repeats: 2 },
                    shape: { kind: "sphere_surface", radius: 0.3 },
                    direction: "outward", speed: [0.05, 0.22], drag: 0.94,
                    lifetime: [14, 24], size: [0.26, 0.08], sizeMode: "sin",
                    color: 0xFF8FB8, alpha: [0.95, 0], light: "full", maxParticles: 120
                },
                {
                    name: "kiss_core", bind: "target", height: 0.7,
                    ...
                    color: 0xFFD0E4, alpha: [1, 0], light: "full", bloom: 0.3
                },
                {
                    name: "kiss_ring", bind: "target", height: 0.55,
                    particle: "world_combat_core:cobblemon/generic/ring/smallring",
                    burst: { count: 34 }, shape: { kind: "ring", radius: 0.42 },
                    direction: "inward", speed: [0.05, 0.11],
                    lifetime: [12, 18], size: [0.32, 0.14],
                    color: 0xE0508A, alpha: [0.55, 0], light: "full", maxParticles: 60
                },
                {
                    name: "kiss_white", bind: "target", height: 0.8,
                    particle: "world_combat_core:cobblemon/generic/fadeheart_white",
                    burst: { count: 8, interval: 4, repeats: 2 }, shape: { kind: "sphere", radius: 0.2 },
                    direction: "up", speed: [0.02, 0.08],
                    lifetime: [14, 24], size: [0.18, 0.06], sizeMode: "sin",
