 ...
 * 拍子：起 windup（口边聚光）→ 扑 pounce → 咬 bite（峰值）／ miss → 磨 grind（持续碎屑）→ 塌 crack（收口强调）。
 * 范围：bite／grind／crack 都绑命中目标或命中点，画出的就是这一口咬中的位置与大小（data.scale 来自咬合判定）。
 * 运动：pounce 的尘迹沿施法者实际走过的直线铺开；grind 的碎屑贴接触点向内翻卷；crack 的碎片从咬点向外崩开。
            ...
            emitters: [
                {
                    ...
                    particle: "world_combat_core:cobblemon/generic/tinydust",
                    rate: 24, shape: { kind: "sphere", radius: 0.26 },
