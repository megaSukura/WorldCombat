 ...
 * - 即时翻译：一条「痛线」——施法者在两者之间牵起一条细红线，把两团生命的和沿这条线拉直，再从中间分开，
 *   低的一方被填上一截、高的一方被抽走一截；无论是对手还是自己，都要分担同一个平均值，所以它既可能是回复也可能是自损。
 *   一分不能致死：两边都被夹在至少 1 点生命上。
 * - 参数分散到精灵数据：牵线射程取特攻与等级，起手与收手取速度，冷却取等级；痛线上的流动粒子数由本次真实的
 *   生命差与体型、等级一起派生（`data.flow`），两边血量差得越多，画面里的流动越密。
 *
 * 没有伤害段：这是变化招式，结算只把双方生命拉向同一个平均值，不经过属性、相性与暴击。
 */
namespace PokemonSkills {
    actionParameters.define("painsplit", {
        ...
        motes: formula(
            ...
            "流动粒子数", {
                unit: "颗",
                description: "痛线上流动的光点基准数量；等级越高、身板越高越多，实际发射量再按本次生命差放大。"
            }),
