 ...
 * 打草结的可执行设计说明。
 *
 * 场面：一只只会打草结的草系精灵（Snivy），面对 3 格外一只笨重缓慢的 Snorlax（脚下有土）。两者开战，AI 只有这一招可用。
 * 必然事实：本招被提交过；目标被缠中并受到伤害（根结收拢时同一次判定）；目标身上出现过 tripped 身份。
 * 是否在延迟内走出范围、有没有因为腾空而躲开，都是位置结果，写进 note 供读轨迹判断。
 * 本招不再改动世界方块，所以不检查 changedBlocks。
 */
