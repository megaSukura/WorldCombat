 ...
 *
 * 世界化：把「三束光线」翻成一排三发的扇形齐射。集束式（默认）三束依次锁同一合法目标，三发各结算，
 *   三束各掷一次元素余痕；广域式让三束各自找身边最多三个不同的有效敌，覆盖更广，代价是每束更轻、余痕机会略低；
 *   点选方向/地点而没有敌人（或目标中途离场）时，三束按 `fan` 张角朝那个方向扇出空放，不强寻敌。
 *   三束按火→冰→电（红蓝黄）次序离手，画面与判定读同一组元素身份。
 *
