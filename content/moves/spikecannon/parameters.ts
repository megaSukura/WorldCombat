 ...
 *
 * 翻译：把回合制的「2～5 连击」翻成**架炮直贯**——施法者稳住架势，首发把炮身方向固定下来，
 *   之后每发都沿同一条线打出去；每发穿透一线上的目标，并把命中的对象**顶退**。
 *   它是本族最慢、最重、射程最长的一梭，也是唯一会把人推开的。可射空，墙会截断；移动中断时未发的钉取消、已付 PP 不退。
 ...
 *   飞弹针   —— 追踪细针，钉在目标身上；
 *   冰锥     —— 齐排平行直飞，靠整排宽度覆盖；
 *   尖刺加农炮 —— 固定准线重钉，穿一排、把人顶退；无追踪、无残留，只有「贯」与「推」。
 *
 * 数据分散（每项读不同的精灵数据）：
