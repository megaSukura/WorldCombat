 ...
 *   所以它不需要瞄准，但必须站进人堆里、还要让别人看得见。命中后先调用 NativeEffects.boost 下降攻击；
 *   再尝试把对方的世界 native target 归零，成功才挂共享身份 world_combat:status/befriended 的真实 MobEffect
 *   与一只托管效果，并在存续期间维持停手，让追上来的东西先停手。原生 Boss 拒绝改目标时只留降攻，不硬控。
 *   这份和睦很脆：一旦受到来自施法者阵营的敌对伤害，平息结束、握手断开，已降的攻击等级保留。
 *   宝可梦那一层照旧走原生攻击等级。「摊手」铺得广而浅，一次劝住更多人；「作揖」把手势收小，但降得更深、
 *   平静更久，起手与冷却更长。
 *
 * 数值来源（每个参数读不同的个体数据）：
