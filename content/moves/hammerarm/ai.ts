 ...
 *
 * 什么局面下出手：一记近身单体的过顶重砸。目标可见、敌对、存活，且在 `ai.maxChase`（默认 6）格内；
 *   更远交给共享接近逻辑。这一记会让自身速度下降，所以它只在够得到时用，不拿它去追人。
 * 对谁出手：`ai.finish`（默认开）打开时，残血目标多一档分——用一记最重的原地打击收掉。
 * 够不到怎么办：reach 就是本招射程，不够先走近；目标在起手期间跑掉就只留扑空的尘。
 * 放完之后：命中才付自身速度 −1；交回共享交战计划等冷却。
 */
