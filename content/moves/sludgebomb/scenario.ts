 ...
 *
 * 一句话：把一枚延时污泥炸弹丢到对手脚边，引信烧完时爆心一圈一起挨伤、被推开、可能中毒。
 *
 * 场面：一只臭臭泥带着这一招，站在 5 格外对一只慢吞吞的卡比兽投弹；卡比兽走得慢，大概率留在爆心内，
 * 让"引信烧完→炸到"这条链可复现。卡比兽用撞击还手。
 *
 * 断言只取必然事实：这招被放过、目标挨到过爆心伤害。引信期间是否走出爆心、中毒（约 30% 的随机掷）与暴击写进 note。
 */
Smoke.scenario("sludgebomb", function (stage) {
    ...
    stage.until(1400, function () {
        ...
        stage.expect(stage.damageTo(target) > 0, "the delayed burst dealt damage");
        stage.note("whether the target stayed inside the fuse radius, the poison roll and crits are random/positional", {
