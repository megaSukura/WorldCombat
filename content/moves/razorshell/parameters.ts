 ...
 *
 * 翻译：把「用锋利贝壳切斩对手」落成一记**沿真实外缘划过的 crescent 壳刃**——壳缘亮起一道水光，刀刃中心
 * 沿 `reach` 半径的弧在 4 刻内横扫过 `arc` 度的范围；只有站在刃厚 `edge` 那一圈外缘的目标才会被削到，
 * 贴着身体的内圈不自动挨刀。切中的目标各吃一记切斩，各自按几率被削掉一级防御；壳缘带水，被切开的还会
 * 被溅湿（共享身份 `world_combat:status/soaked`，与水流尾、波动冲、水流裂破的湿身是同一件事），
 * 为别的招（水流炮、冰冻干燥等）留一段水湿窗口。墙会截住对应的刃段，不补穿墙命中。
 *
 * 与同族分开：咬碎是单点研磨压塌护甲、撕裂爪是一道窄走廊的交叉撕抓、劈开是竖直重劈；强力鞭打填满整个扇面，
 * 而只有贝壳刃用一道**薄而亮的壳缘**在固定距离带上切人——站位比覆盖面更重要。
 *
 * 数值来源（每项依赖不同的精灵数据，分散到不同参数上）：
 *   carve       切斩威力 75 + 物攻偏移；揽月式 ×0.88 / 凿刃式 ×1.15。
 *   reach       外缘半径 2.3 + 速度偏移；也是实际射程来源。
 *   arc         扫过的张角 110 度 + 碰撞箱宽度偏移；揽月式 ×1.45 / 凿刃式 ×0.7。
 *   edge        刃厚 0.5 格 + 碰撞箱宽度偏移；揽月与凿刃共用同一锋刃厚度，只改扫过的角度与威力。
 ...
 *
 * 配置 `wide`（揽月式）双向取舍：开启＝扫过的弧更宽、削甲几率更高、湿身更久、顶得更开，代价是单下威力 ×0.88、冷却 +6；
 * 关闭＝凿刃式，弧收窄但单下威力 ×1.15、冷却更短、削甲几率略低——对单点更狠，对人群更弱。两者锋刃厚度相同。
 *
...
namespace PokemonSkills {
    actionParameters.define("razorshell", {
        ...
        arc: formula(
            ...
            "扇面张角", {
                unit: "度",
                description: "刀刃中心沿外缘扫过的总张角；身体越宽的个体扫得越开，揽月式张得更开、凿刃式收成一条窄刃。两者共用同一锋刃厚度。"
            }),
        /** 刃厚：基础 0.5 格，碰撞箱每比 0.9 宽 1 格加 0.5（夹 −0.15..0.35）；夹 0.35..0.95。 */
        edge: formula(
            F.base(0.5).plus(F.body("width").minus(0.9).times(0.5).clamp(-0.15, 0.35)).clamp(0.35, 0.95).round(2),
            "刃厚", {
                unit: "格",
                description: "壳刃有多厚：只有身体落在外缘这一圈里才会被削到，贴身的内圈不自动挨刀。揽月式与凿刃式共用同一厚度。"
            }),
    ...
    describe("razorshell", [
        ...
        { key: "description.1", values: ["reach", "arc", "edge"] },
