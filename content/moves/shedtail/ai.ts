 ...
 * `ai.release` 决定撤向哪：默认背离威胁，也可以向主人身边靠（对应原生「返回」）。
...
namespace PokemonSkills {
    CompanionBehavior.registerUse("shedtail", {
        ...
        available: function (context, capability, purpose, target) {
            ...
            return CompanionBehavior.ratio(self) > 0.5 + reserve;
        },
        ...
        target: function (context, capability, target) {
            ...
            var owner = context.facts.owner, distance = capability.data.range;
            var destination: number[];
            if (CompanionBehavior.ai<string>(capability, "release", "away") === "owner" && owner)
                destination = owner.point.slice();
            else if (threat) {
                var dx = self.point[0] - threat.point[0], dz = self.point[2] - threat.point[2];
                var length = Math.sqrt(dx * dx + dz * dz) || 1;
                destination = [self.point[0] + dx / length * distance, self.point[1], self.point[2] + dz / length * distance];
            } else destination = [self.point[0], self.point[1], self.point[2] + distance];
            var copy: any = JSON.parse(JSON.stringify(self));
            copy.point = destination;
            return copy;
        },
