 ...
 *
 * 翻译：把「先制」与「连续 2～5 次」翻成**甩出一串水做的手里剑**：起手在掌中搓出旋转的水盘，随后一枚接一枚
 *   沿直线切向目标，每枚独立结算一次 `shuriken` 特殊伤害并挂上短暂的浇透；命中几枚由精灵数据决定（等级、特攻、速度），
 *   所以两只精灵甩出的枚数可以在场上看出不同，而不是纯掷骰。它是全族唯一的特殊招与唯一的连发水星。
