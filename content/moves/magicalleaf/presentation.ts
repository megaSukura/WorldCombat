 ...
 * 拍子：起（gather 聚叶）→ 击（launch 迸叶、seek 追叶、hit 命中）→ 收（fade 散尽）。
 * 范围：hit 的命中环半径用 `data.scale`（判定半径 / 0.3）给出，玩家看出每片叶能咬住多大一圈。
 * 运动：gather 的叶由外向内旋聚；launch 的叶向外迸开；seek 的叶沿 projectile 锚点飞行、逐刻转向对手。
 * 数：`data.count`（叶数）绑定 launch 的迸射数量，`data.trail`（单叶威力换算的尾迹量）绑定 seek 的发射率，
 *   `data.notes`（威力换算的碎叶数）绑定 hit 的爆开数量，`data.intensity` 抬高亮度。
