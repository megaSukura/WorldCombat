 ...
 *
 * 翻译：把「从两眼发射出神奇的光线攻击，连续 2 次给予伤害」落成一记**双目并射**——两只眼睛各从自己的真实眼位
 *   射出一道灵光，两道射线朝同一个瞄点收拢、再各自沿原直线穿过交点飞完射程。每道光只打它碰到的第一个接触；
 *   只有两道光确实打到同一个目标，那一点才会共鸣、第二道才更强。
 *   它是四招里唯一远程、特殊、不接触的一招：两条发光的线从两个眼位收拢到同一点，与两只手/两根针完全不同的读法。
 *
 * 数据分散（每项读不同的精灵数据）：
...
namespace PokemonSkills {
    ...
    actionParameters.define(twinbeamId, {
        ...
        resonance: percent(
            ...
            "第二道共鸣加成", "两道光确实打到同一个目标时，第二道顺着那一点共鸣射入，威力最多再抬这么多；特攻越高共鸣越强。并射式没有这道加成，两道打中不同目标也加不上。"),
