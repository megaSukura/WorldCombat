 ...
 *
 * 翻译：把「影子做成的锐爪」落成一条从施法者脚下**贴着真实地表**铺到落点**身后**的暗带，由那一端
 * 反向伸出一只影爪，从对手照不到的一面抓进要害。它的形状是「影先到、爪后到」：对手在看正面，影子
 * 已经绕到它背后。暗带走的是真实地面与通路，实墙会把影带裁断；落点没有地表时，只收成一记近处短爪。
 ...
 *   shade   影铺距离：体重与等级定影子绕到落点身后多远；深影式再往前铺。
 *   claw    爪面半宽：碰撞箱宽度定判定带有多宽；贴影式更宽。
 *   depth   爪的竖直覆盖：身高定这一爪从脚上抓多高。
 *   lag     回抓延时：速度定影带铺到锚点后、爪再探出来之间隔几刻；贴影式更急。
 *   shred   崩屑量：物攻换算，驱动表现。
 *   ambush  暗算加成：对手没在看自己时额外增加的伤害比例；等级越高越狠，深影式再高一档。
 *   tempo／aftercast／recharge：速度定节奏；深影式以更长的起手与冷却换更重更远的影。
 *
 * 配置 `deep` 双向取舍（默认关）：
 *   开（深影式）：威力 ×1.12、影铺 +0.6 格、暗算 +0.1，代价是起手 +3 刻、冷却 +8 刻。
 *   关（贴影式）：出手更快、爪面宽 0.12 格、回抓更急、冷却更短，代价是威力 ×0.94、暗算略低。
 */
namespace PokemonSkills {
    ...
    actionParameters.define(shadowclawId, {
        ...
        reach: formula(
            ...
            "抓距", {
                unit: "格",
                description: "影带先够到的落点距离，也是本招的实际射程；速度快的个体铺得更远。"
            }),
        ...
        shade: formula(
            ...
            "影铺距离", {
                unit: "格",
                description: "影子绕过落点、铺到它身后多远处出爪；身重的个体影子拖得更长，深影式再往前铺。"
            }),
        ...
        claw: formula(
            ...
            "爪面半宽", {
                unit: "格",
                description: "暗带与回爪判定的横向半宽；身体越宽的个体爪面越宽。贴影式贴身出爪，爪面更宽。"
            }),
        ...
        depth: formula(
            ...
            "竖直覆盖", {
                unit: "格",
                description: "这一爪从脚下沿落点向上抓多高；高大的个体抓得更高。"
            }),
        /** 回抓延时：基础 6 刻，速度每比 55 快 1 减 0.02（夹 −1..1.5）；深影 +1 / 贴影 −1；夹 3..10。 */
        lag: seconds(
            F.base(6).minus(F.stat("speed").minus(55).times(0.02).clamp(-1, 1.5))
                .plus(F.when(F.pref("deep"), F.const(1), F.const(-1))).clamp(3, 10).round(0),
            "回抓延时", "影带铺到落点身后、影爪再反向探出来之间隔几刻；速度越快收得越急。"),
