 ...
 *   accuracy 每趟命中率：速度决定石球贴得准不准；重滚式更难控。
 *   reach    滚击距离：速度与身高决定贴上去的短距，也是本招的实际射程来源。
 *   radius   判定半径：碰撞箱宽度与身高决定石球扫过多大一圈。
 *   lunge    冲进距离：速度决定这一趟向前垫多远。
 *   step     每刻位移：速度决定滚得多快（表现里的滚动速度）。
 *   push     击退：物攻决定撞开后顶多远。
 *   recoil   弹回：体重决定撞上之后弹回来多少。
 *   window   连滚窗口：等级决定层数能维持多久不散。
 *   grains   碎石点数：物攻派生，表现按它发射。
 *   tempo／recover／recharge：速度与配置共同决定起手、收招与冷却。
 *
 * 配置 `heavy`（重滚式）双向取舍（默认关）：
 *   开（重滚）：每次命中翻倍幅度 2.35 倍（上限更高），代价是每趟命中率 −5%、滚击距离 ×0.92、冲进 ×0.9、
 *     收招 +2 刻、冷却 +8 刻——滚得更重但更难接住。
 *   关（轻滚，原生形态）：翻倍 2.0 倍、每趟命中率 +2%、滚得更远更顺，收招与冷却更短，代价是天花板较低。
 *
...
namespace PokemonSkills {
    ...
    actionParameters.define(rolloutId, {
        ...
        accuracy: percent(
            ...
            "每趟命中率", "一趟滚击掷的命中率（原生 90% 起）；速度提高它，重滚式把它压低。这一趟落空，连滚层数当场散尽。"),
        ...
        /** 冲进距离：1.0 + 速度偏移[−0.1,0.35]；重滚 ×0.9；夹 0.7..1.6。 */
        lunge: formula(
            F.base(1.0).plus(F.stat("speed").minus(50).times(0.005).clamp(-0.1, 0.35))
                .times(F.when(F.pref("heavy", { key: "worldcombat.skill.rollout.preference.heavy", fallback: "重滚式" }), F.const(0.9), F.const(1)))
                .clamp(0.7, 1.6).round(2),
            "冲进距离", {
                unit: "格",
                description: "这一趟贴着地面向前垫进去的距离；速度快的个体垫得远。"
            }),
