 ...
 * 上菜 / orderup —— 可执行设计说明。
 *
 * 一句话：以潇洒的身手端出一记不接触的精准下手，拍伤目标、震碎下手点的屏障；身边带着小个子伙伴时，
 * 按它的样子给自己补上一项能力。
 *
 * 场面：一只只会这个动作的怪力（36 级，身旁跟着一只小个子同伴）对一只只会撞击的卡比兽（45 级）开战。
 * 厚血对手让它活到第二记；AI 只有这一招，必然会被放出来。
 * 同行的「菜」用一只原版小生物（蠹虫）站在队里——它比施法者明显小，能触发按伙伴补能力的判定；同伴若是
 * 另一只宝可梦，本隔离装配里两只同队宝可梦的共享 AI 会互相影响，反而不出手，所以这里刻意用原版小生物。
 * 必然事实：本招被提交过、目标受到过伤害（下手拍实）。
...
Smoke.scenario("orderup", function (stage) {
    ...
    const dish = stage.mob({ type: "minecraft:silverfish", at: [-1, 0, 0] });
