 ...
 *
 * 一句话：拳头收在腰间攥亮，随后一支笔直的冲击窄道贴着地面捅出去，环状冲击沿拳路一道道推远，
 * 被正中的目标身上炸开一记钝击、随即拖着尘屑沿同一条线被轰飞。
 * 色相家族：暖白（0xFFF2DA）与沙金（0xC9B37A），中性尘灰作余韵；饱和色只出现在拳面核心的小面积。
 * 拍子：起 charge（收拳蓄势）→ 击 thrust（窄道推进）与 hit（命中钝击）→ 收 launch（被轰飞）与 whiff（破风）。
 * 范围：thrust 的窄道用 `data.path`（与判定同一组顶点）填成多边形带；玩家一眼看出站在这条带里会挨打。
 * 运动：窄道沿 `data.direction` 从施法者朝目标方向推出，冲击环沿同一条带向前推进。
 * 数：窄道粒子量绑 `data.flows`，冲击环数绑 `data.rings`（物攻换算），命中强度绑 `data.intensity`。
 */
            ...
            emitters: [
                {
                    ...
                    shape: { kind: "polygon" }, burst: { count: { data: "flows", fallback: 110 } },
                    direction: "shape", orient: "direction", speed: [0.02, 0.1], spread: 14,
                    lifetime: [6, 10], size: [0.42, 0.06], sizeMode: "index",
                    color: 0xFFF2DA, alpha: [0.55, 0], light: "full", bloom: 0.3, maxParticles: 160
                },
                {
                    name: "edge", bind: "path", fit: "none",
                    particle: "world_combat_core:cobblemon/generic/speedlines",
                    shape: { kind: "polyline", closed: true }, burst: { count: 26 },
                    direction: "shape", orient: "direction", speed: [0.08, 0.24], spread: 8,
                    lifetime: [5, 9], size: [0.22, 0.05], sizeMode: "index",
                    color: 0xC9B37A, alpha: [0.7, 0], light: "world", maxParticles: 40
                },
                {
                    ...
                    lifetime: [6, 11], size: [0.34, 0.08], sizeMode: "index",
