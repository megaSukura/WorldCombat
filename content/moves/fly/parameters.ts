 ...
 * 就会落空。两个方向各有取舍，收招与冷却也不同。选取是 aim：可以点实体，也可以点一个空点；点空点
 * 走固定落区支路，落点不漂移，真实俯冲到空地。
 *
 ...
 * 伤害段名就是参数名 power；实际轻重再乘一个高度系数（见下），由真实取得的悬停高度决定。
 *   heightFactor = 0.55 + 0.45 × 实际取得高度 / 计划高度：天花板压顶或中途被挡时这一击随之变轻，最低一半多。
 */
