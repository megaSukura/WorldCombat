 ...
 *   相隔 6 格。AI 只有这一招可用。
 * 必然事实：本招被提交过、目标受过伤害，且这一梭脱鳞后自身速度上升、防御下降（整梭只结算一次）。
 *   发数（2～5，随物攻／速度／等级变化）、每片威力、3D 方向与散布只写进 note，供读轨迹判断。
 */
Smoke.scenario("scaleshot", function (stage) {
    ...
    stage.until(900, function () {
        ...
        stage.after(45, function () {
            ...
            stage.expect((stages.spe || 0) > 0, "the shed granted a real Speed stage");
            stage.expect((stages.def || 0) < 0, "the shed took a real Defense stage");
            stage.note("the burst length (2-5 shards), per-shard power, 3D aim and spread follow Attack/Speed/level and the spray/聚鳞 choice; the shed grants Speed +1 / Defense -1 exactly once per volley (design facts verified in the full assembly)", {
                casts: stage.casts("scaleshot", caster),
                damage: Math.round(stage.damageTo(foe) * 10) / 10,
                speedStage: stages.spe || 0,
