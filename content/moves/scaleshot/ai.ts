 ...
 * 对谁出手：`ai.finish`（默认关）打开时，残血目标多一档分；散鳞式（配置或 `ai.spread`）打开时，
 *   目标身旁挤着 2 个以上敌人会把这一梭排到前面——鳞片能分给一群人。
 * 够不到怎么办：reach 就是本招射程，不够先走近；连射之间目标全倒下这一梭自然收住、照常脱鳞。
 * 放完之后：这一梭射完（或目标先倒）就脱鳞结算，交回共享交战计划等冷却。
 */
namespace PokemonSkills {
    ...
    function scaleshotSpray(context: WorldBehavior.Context, capability: WorldBehavior.Capability): boolean {
        ...
        return !!(config && (config.spray === true || CompanionBehavior.ai<boolean>(capability, "spread", false)));
    }
    ...
    addPreferences("scaleshot", {}, [
        ...
        field(pathOf("ai.spread"), "优先散鳞目标群", "boolean", {
            help: "开启（配合散鳞式）：目标身旁挤着两个以上敌人时优先射，鳞片能多分一个；关闭则只按普通中程攻击排序。"
        }),
