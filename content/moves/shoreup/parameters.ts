 ...
 *   取材只读取地面的沙（环境可用量），**不挖走脚下的承重沙块**，地表保持连续；只有明确落在脚边的松散沙**掉落物**
 *   会被消耗，那才是真正修改世界的一步。这条「不挖地基、只收浮沙」的取舍让它与光合作用（只读光照、不改世界）分开。
 ...
 *   grains       集沙量：3 + 等级(≥20)偏移[0,0.6]，厚结档 ×1.5，夹 2..10 粒，也是取景与消耗掉落物的上限。
...
namespace PokemonSkills {
    ...
    actionParameters.define(shoreupId, {
        ...
        grains: formula(F.base(3).plus(F.level().minus(20).max(0).times(0.06))
            ...
            .clamp(2, 10).round(),
            "集沙量", { unit: " 粒", description: "从探沙范围内卷起多少粒散沙糊到身上；等级越高、厚结档越多。地面沙块只作取材来源、不被挖走，只有落在脚边的松散沙掉落物才真的被消耗。" }),
