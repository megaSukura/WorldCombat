 ...
 * 核心近白，钢屑用低饱和灰蓝。
 * 拍子：起 gather（收钢）→ 铸 lance（钢梁走廊）→ 撞 impact（命中钢花）／ 空 fizzle（尽头散光）→ 剥 shed（落屑）。
 * 范围：lance 用 `data.path` 画出服务端走廊的同一组四个顶点——梁有多长多宽，画面就是那条钢梁。
 * 运动：钢梁沿走廊由近及远铺开、边缘同时向前扫过；命中钢花在命中点炸开；落屑在施法者身上垂直下坠。
 * 数：`data.notes`（威力换算）与 `data.intensity`（威力 / 140）决定钢梁与命中钢花的密度，`data.shards`（体重与
 * 身高派生）决定边缘强调与落屑数量，`data.scale`（半宽 / 0.85）放大钢梁与冲击的尺寸。
    ...
    moments: {
        ...
        lance: {
            ...
            emitters: [
                ...
                {
                    name: "lane_grit", bind: "path", offset: [0, 0.5, 0],
