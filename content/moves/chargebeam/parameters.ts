 ...
 * 翻译：把「向对手发射电击光束，由于蓄满电流有时提高自己的特攻」落成**一道按住不放、从喷口连到实际碰撞点的细束**——
 *   起手把电向身前收拢、压成一道束；开火第一刻就沿自由准心判定首碰，命中的敌人吃下主击；之后每刻准心跟随，
 *   细束始终连着当刻的真实碰撞点。只有把准心压在同一个受击者身上直到收束，尾端才会闪一下余流、再咬一口。
 *
 * 与同族分开：火之舞是贴着自己跳、覆盖全身、扫一圈；充电光束是按住一条细束、要求持续瞄准同一个点。
 ...
 *   residualShare 余流占比：特攻；收束时那一口的轻重。
 *   beamDuration  束持续：速度；越快把电收得越干脆，需要保持接触的窗口也越短。
 ...
 * 关闭（速射）＝更快更便宜，射程与回灌几率都较低。两个方向对应「远处一发狠的」与「贴身速点」。
 */
namespace PokemonSkills {
    actionParameters.define("chargebeam", {
        ...
        surgeChance: percent(
            ...
            "回灌几率", "开火第一刻的主击命中后，余流回灌、特攻提升的几率；原生约 70%，特攻与等级把它抬得更稳，过充式更足。"),
        ...
        residualShare: percent(
            ...
            "余流占比", "细束收束时余流再咬一口，这一口是主击威力的几成。"),
        /** 束持续：7 − 速度偏移[−2,2]；夹 5..10（原余流延迟 +1 刻）。 */
        beamDuration: seconds(
            F.base(7).minus(F.stat("speed").minus(60).times(0.02).clamp(-2, 2)).clamp(5, 10).round(0),
            "束持续", "从命中到收束、细束保持的刻数；只有把准心继续压在同一个受击者身上直到收束，尾端才会闪一下余流。速度越快收得越干脆。"),
    ...
    describe("chargebeam", [
        { key: "description.0", values: ["beam","reach"] },
        { key: "description.1", values: ["radius","surgeChance","surgeStages"] },
        { key: "description.2", values: ["residualShare","beamDuration"] },
