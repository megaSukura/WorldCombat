 ...
 * 拍子：起 windup（攒火星）→ 开火 ignite（点起火炉）→ 添柴 stoke（挨打上窜）→ 守炉 aura（低密度长明）→ 熄 fade。
 * 范围：ignite / aura / fade 绑施法者自身，strike 绑命中点——画出的就是火炉与这一记挥到的位置。
 * 运动：windup 的火星向内收拢，ignite 的火从脚边向上窜，stoke 的火苗从身体四周升起，aura 的余烬贴地缓慢上飘。
 * 数：`data.plumes`（本次怒火档数与封顶派生）决定火星数量，`data.bright`（档数派生）抬高亮度，
 * `data.gain` 是这一记实际涨的档数，`data.scale`（判定半径 / 0.42）放大挥击轮廓。
