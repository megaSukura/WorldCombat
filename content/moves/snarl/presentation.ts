 ...
 * 拍子：起 gather（聚声，可反复）→ 击 bark（每一声一锥）→ 中 hush（落到人身上）→ 收 linger（被斥余韵）/ fizzle（喝空）。
 * 范围：bark 的锥形顶点就是判定用的扇面（`data.path`），长度 `data.reach`、张角 `data.halfArc` —— 铺到哪就是会被喝到哪；
 *   一个脉冲一个 bark，画面里连推几次就是机制里连喝几声。
            ...
            emitters: [
                {
                    ...
                    color: 0xD8CCFF, alpha: [0.65, 0], light: "full", maxParticles: 36
                },
                {
                    ...
                    color: 0x8A6BC8, alpha: [0.7, 0], light: "full", bloom: 0.25, maxParticles: 30
                }
            ]
        },
        bark: {
