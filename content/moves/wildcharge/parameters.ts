 ...
 * 笔直撞上去；撞实的一刻把电流灌进对方身体：干燥的目标看概率，湿透的目标被必然传导一次；回路也顺着自己回来，
 * 全身湿透时漏电更狠，起冲就能看出。
 * 原生没有次要效果；本招把「电流灌进对方」做成一次传导，作为它在场上与猛撞分开的身份，写进报告说明。
 *
 * 数据分散（每项依赖不同的精灵数据与当前处境）：
 *   surge    电击威力：物攻与速度共同打底；自己湿透漏电打折、对方湿透导电加成、过载再抬一档。
 *   paralyze 麻痹概率：对干燥目标，速度给出基础、过载提高、控流式降低；湿透目标直接传导，不掷这个概率。
...
namespace PokemonSkills {
    actionParameters.define("wildcharge", {
        ...
        /** 麻痹概率：基础 0.12，速度每比 60 快 1 加 0.0015（夹 -0.04..0.10），过载 +0.06 / 控流 -0.03；夹 0.05..0.5。目标湿透时改用必然传导，不掷此概率。 */
        paralyze: percent(
            F.base(0.12).plus(F.stat("speed").minus(60).times(0.0015).clamp(-0.04, 0.1))
                ...
                .clamp(0.05, 0.5),
            "麻痹概率", "对干燥目标，“把电流灌进去、让它麻痹”的机会；速度越快越容易灌透，过载再加一档，控流式降低。目标湿透时不再掷这个概率：湿身直接传导一次麻痹（除非免疫）。"),
