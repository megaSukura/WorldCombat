 ...
 *
 * 一句话：拳上按拳印聚起暗红怒气 → 每一记鬼拳都在命中的一瞬间甩出一只大大的虚影拳头、炸开鬼色冲击
 *   与一簇怒气 → 挨打时拳上再窜起一簇怒气（拳印 +1）→ 拳印散尽时余怒褪去。
 * 色相家族：血怒红（0xB23A4E 主体 / 0xE8B0C0 高光）压着鬼紫（0x8A3A6A 作虚影拳与冲击），暗底近黑 0x2E1A28。
 * 拍子：起 coil（0–8t 聚怒）→ 拳 punch × `fists`（一记 Event 一只拳）→ 攒 stack／守 aura → 收 fade／miss。
 * 范围：每一记拳绑命中者、拳影与冲击画在被打中的那一身上；这是一串贴身拳，没有铺开的地面区域。
 * 运动：punch 的虚影拳从命中者身上向外一探、冲击向外炸；stack 的怒气向上窜起。
