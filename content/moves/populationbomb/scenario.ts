 ...
 * 卡比兽（Snorlax，30 级）叫伙伴；晴天平地。必然事实：本招被提交过（`stage.casts`）；至少一只伙伴扑中并
 * 造成伤害（默认「鼠海」下命中率约 85%、每 60 秒内会多次施放）。实际连了几只、断在第几只、是否集满
 * 写进 note 供读轨迹判断。
 */
Smoke.scenario("populationbomb", function (stage) {
    ...
    stage.until(1600, function () {
        ...
        stage.note("每只伙伴独立掷命中，扑空一只这一串就断；默认「鼠海」上限十只、命中率约 85%，实际只数随机。配置 swarm 关闭后上限五只、每下更重更准。连段长度与命中分布由完整装配的人工试玩核对。", {
