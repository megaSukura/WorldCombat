 ...
 *
 * 一句话：身侧先收拢一圈铃音（windup）→ 一声清亮的声波以身体为心向外扩成圆环，扫过之处伙伴身上的病痛
 *   化成暗紫碎屑被震散（peal，连着响数声）→ 余韵在身侧轻轻落下（fade）。
 * 色相家族：清透蓝白 0xCFE8FF 作主体，近白 0xF2FBFF 作高光，被震散的病痛用低饱和暗紫 0x8A6BB0 画小面积。
 * 拍子：起 windup 0–14t ／ 击 peal 0–26t（每声一次）／ 收 fade 0–30t。
 * 范围：peal 的圆环绑 point、fit none，几何按 data.scale = 实际铃声半径 / 5.0 缩放，玩家站在环外就明白不会被洗到。
 * 数：peal 的铃光数绑 data.motes（特防与体型派生），被震散的病痛碎屑数绑 data.cleansed（本声实际洗掉的项数）。
 */
    ...
    moments: {
        ...
        peal: {
            ...
            emitters: [
                ...
                {
                    name: "peal_malaise", bind: "source", offset: [0, 0.45, 0], height: 0.2,
                    particle: "world_combat_core:cobblemon/generic/orb/smallfadeorb",
                    burst: { count: { data: "cleansed", fallback: 1 }, interval: 2, repeats: 5 }, shape: { kind: "sphere", radius: 0.42 },
