 ...
 *   每一声都洗掉当下范围内所有伙伴的主异常。它不回复生命、不留持续状态：这是**当场的一下**，和「芳香治疗」
 *   那片会停留、反复净化的香云分开。
 *
 * 两幕：
 *   起（windup，提交前）：铃音在身侧收拢，只观察与预告；可被打断，不花任何代价，准备期不能移动。
 *   响（提交后）：每一声铃响都洗一次——先洗自己，再洗 chimeRadius 内每个友善伙伴的全部主异常；
...
namespace PokemonSkills {
    ...
    /** 清除一个战斗者身上的全部主异常，返回实际洗掉的项数。 */
    function healbellCleanse(world: CombatWorld, actor: CombatActor): number {
        if (!world.valid(actor)) return 0;
        var removed = 0;
        for (var index = 0; index < healbellMalaise.length; index++)
            if (CombatStatus.cure(world, actor, healbellMalaise[index])) removed++;
        return removed;
    }

    function healbellAfflicted(world: CombatWorld, actor: CombatActor): boolean {
        for (var index = 0; index < healbellMalaise.length; index++)
            if (CombatStatus.has(world, actor, healbellMalaise[index])) return true;
        return false;
    }

    define({
        id: healbellId, name: "治愈铃声",
        description: "以自己为心敲响一圈铃声，一次施放连响数声；每一声都洗掉半径内自己与所有伙伴身上的全部主异常（中毒／剧毒、灼伤、麻痹、睡眠、冰冻）。它不回复生命，也不留下任何持续状态。",
