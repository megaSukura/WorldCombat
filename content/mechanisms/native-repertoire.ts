    ...
    export function create(options: Options) {
        ...
        function installChannel(): void {
            CobblemonCombat.channel(options.channel || options.namespace + ":skills", function (request) {
                ...
                const roster = request.roster ? JSON.parse(String(request.roster())) : [];
                const menu = menus.resolve({ pokemon: pokemon, skills: result, roster: roster,
                        world: request.world ? request.world() : null, actor: request.actor ? request.actor() : null });
                request.reply(JSON.stringify({ pokemon: { id: String(pokemon.id()), species: String(pokemon.species()), level: pokemon.level() },
                    skills: result, requested: requested, dependencies, menu: menu, supportedMoves: Object.keys(skills) }));
            });
        }
