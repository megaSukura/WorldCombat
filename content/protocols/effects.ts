...
WorldCombat.on("world_combat:effects_incoming", "world_combat:damage_incoming", "", function (event) {
    var target = event.target();
    if (target !== null) {
        if (JSON.parse(event.data()).bypassesInvulnerability) return;
        var world = event.world(), data = JSON.parse(world.signal("world_combat:incoming", 1, target, event.data()));
